 ...
 * @return boolean
 */
function handle_invoice_attachment($id)
{
    if (isset($_FILES['file']) && _perfex_upload_error($_FILES['file']['error'])) {
        header('HTTP/1.0 400 Bad error');
        echo _perfex_upload_error($_FILES['file']['error']);
    ...
    if (isset($_FILES['file']['name']) && $_FILES['file']['name'] != '') {
        ...
        $path = get_upload_path_by_type('invoice') . $id . '/';
        // Get the temp file path
        $tmpFilePath = $_FILES['file']['tmp_name'];
        // Make sure we have a filepath
        if (!empty($tmpFilePath) && $tmpFilePath != '') {
            _maybe_create_upload_path($path);
            $filename    = unique_filename($path, vn_to_str($_FILES['file']['name']));
            $newFilePath = $path . $filename;
            // Upload the file into the company uploads dir
            if (move_uploaded_file($tmpFilePath, $newFilePath)) {
                $CI           = & get_instance();
                ...
                $file = $CI->misc_model->add_attachment_to_database($id, 'invoice', $attachment);
...
function handle_task_attachments_array($taskid, $index_name = 'attachments')
{
    ...
    if (isset($_FILES[$index_name]['name'])
        ...
        for ($i = 0; $i < count($_FILES[$index_name]['name']); $i++) {
            ...
            $_FILES[$index_name]['name'][$i] = vn_to_str($_FILES[$index_name]['name'][$i]);
            $_FILES[$index_name]['name'][$i] = vn_to_str($_FILES[$index_name]['name'][$i]);
            // Make sure we have a filepath
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
...
function handleid_upload_contracts_sale($id='')
{
    $id_contracts_sale = $id;
    $path = get_upload_path_by_type('contracts_sales') . $id_contracts_sale . '/';
    $CI =& get_instance();
    if (isset($_FILES['file']['name'])) {
        // Get the temp file path
        $tmpFilePath = $_FILES['file']['tmp_name'];
        ...
        {      
            ...
            {
                if($id_contracts_sale != "")
                {
                    $value['id_contracts_sale']=$id_contracts_sale;
                    $value['create_by']=get_staff_user_id();
                    $value['date']=date('Y-m-d H:i:s');
                    $CI->db->insert('tblcontracts_sales_file',$value);
                    if($CI->db->insert_id())
                    {
                        $value['id']=$CI->db->insert_id();
                        $result[]=$value;
                    }
                }
            }
...
function handle_evaluate_attachments_array($evaluate_id, $index_name = 'attachments')
{
    $uploaded_files = [];
    $path = get_upload_path_by_type('evaluate') . $evaluate_id . '/';
    if (!file_exists($path)) {
        mkdir($path);
        fopen($path. 'index.html', 'w');
    }
...
function get_upload_path_by_type($type)
{
    ...
    switch ($type) {
        ...
        case 'contracts_sales':
        $path = 'uploads/contracts_sales/';
        break;

        case 'check_quality':
        $path = 'uploads/check_quality/';
        break;

        case 'evaluate':
        $path = 'uploads/evaluate/';
        break;
        
        case 'production_report':
        $path = 'uploads/production_report/';
        break;
        
        case 'contracts_supplier':
        $path = 'uploads/contracts_supplier/';
        break;
    }
