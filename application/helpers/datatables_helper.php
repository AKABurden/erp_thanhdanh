 ...
 * @return array
 */
function data_tables_init($aColumns, $sIndexColumn, $sTable, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $searchAs = [], $cs = [], $sHaving = '')
{
    $CI          = &get_instance();
    ...
    if (!empty($cs['union_all'])) {
        $sQuery = '
        SELECT COUNT(' . $sIndexColumn . ")
        FROM $sTable " . $join . ' ' . $where;

        $_query = $CI->db->query($sQuery)->result_array();
        $iTotal = $_query[0]['COUNT(' . $sIndexColumn . ')'];
    } else {
        $sQuery = '
 ...
 * @return string
 */
function render_datatable_tfoot_ch($headings = [], $class = '', $additional_classes = [''], $table_attributes = [])
{
    $_additional_classes = '';
    $_table_attributes   = ' ';
...
function render_datatable($headings = [], $class = '', $additional_classes = [''], $table_attributes = [], $classBody = '')
{
    $_additional_classes = '';
    $_table_attributes   = ' ';
    if (count($additional_classes) > 0) {
    ...
    $table .= '</tr>';
    $table .= '</thead>';
    $table .= '<tbody class="' . (!empty($classBody) ? $classBody : '') . '"></tbody>';
    $table .= '</table></div>';
    echo $table;
}
function render_datatable_hau($headings = [], $class = '', $additional_classes = [''], $table_attributes = [])
{
    $_additional_classes = '';
    $_table_attributes   = ' ';
    if (count($additional_classes) > 0) {
    ...
    $table = '<div class="' . $IEfix . '"><table' . $_table_attributes . 'class="table table-' . $class . '' . $_additional_classes . '">';
    $table .= '<thead>';
    $table .= '<tr>';
    foreach ($headings as $heading) {
        if (!is_array($heading)) {
    ...
    $table .= '</thead>';
    $table .= '</table></div>';
    $table .= '<div class="' . $IEfix . '"><table class="dt-table-loading table table-' . $class . '">';
    $table .= '<thead>';
    $table .= '<tr class="hide">';
    foreach ($headings as $heading) {
        if (!is_array($heading)) {
