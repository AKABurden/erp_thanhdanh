...
function _dhau($date)
{
    ...
    return hooks()->apply_filters('after_format_date', $formatted[0], $date);
}
...
function substr_string25($value = '')
{
    $count = strlen($value);
    $data = $value;
    if($count > 25) {
        $data = substr($value,  0, 25);
        $data = $data.'...';
    }
    return $data;
}
function getQuantity_Status($table = '')
{
    $count = 0;
    if($table == 'tblstaff') {
        $count = count(get_table_where('tblstaff',array('active'=>1,'month(birthday)'=>date('m'))));
    }
    else if($table == 'tblclients') {
        $count = count(get_table_where('tblclients',array('month(birtday)'=>date('m'))));
    }
    else if($table == 'tblpurchases') {
        $count = count(get_table_where('tblpurchases',array('status'=>1)));
    }
    else if($table == 'tblrfq_ask_price') {
        $count = count(get_table_where('tblrfq_ask_price',array('status'=>1)));
    }
    else if($table == 'tblsupplier_quotes') {
        $count = count(get_table_where('tblsupplier_quotes',array('status'=>1)));
    }
    else if($table == 'tblpurchase_order') {
        $count = count(get_table_where('tblpurchase_order',array('status'=>1)));
    }
    else if($table == 'tblimport') {
        $count = count(get_table_where('tblimport',array('status'=>1)));
    }
    return $count;
}