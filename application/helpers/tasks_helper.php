...
function get_sql_select_task_assignees_ids($next_create = false)
{
	if(!empty($next_create)) {
//		return '(
//			SELECT GROUP_CONCAT(staffid SEPARATOR ",")
//			FROM ' . db_prefix() . 'task_assigned
//			WHERE taskid=' . db_prefix() . 'tasks.id
//			AND tbltask_assigned.staffid != tbltasks.addedfrom
//			ORDER BY ' . db_prefix() . 'task_assigned.staffid
//		)';
        return '(
			SELECT GROUP_CONCAT(staffid SEPARATOR ",") 
			FROM ' . db_prefix() . 'task_assigned 
...
function get_sql_select_task_asignees_full_names($next_create = false)
{
	if(!empty($next_create)) {
//		return '(
//				SELECT GROUP_CONCAT(CONCAT(firstname, \' \', lastname) SEPARATOR ",")
//				FROM '.db_prefix().'task_assigned
//				JOIN '.db_prefix().'staff ON '.db_prefix().'staff.staffid = '.db_prefix().'task_assigned.staffid
//				WHERE taskid='.db_prefix().'tasks.id
//				AND tbltask_assigned.staffid != tbltasks.addedfrom
//				ORDER BY '.db_prefix().'task_assigned.staffid
//			)';
...
{
	$_tasks_where = '('.db_prefix().'tasks.id IN (SELECT taskid FROM '.db_prefix().'task_assigned WHERE staffid = ' . get_staff_user_id() . ')';
