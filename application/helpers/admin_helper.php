...
function init_head_noMenu($aside = true)
{
    $CI =& get_instance();
...
function init_head($aside = true)
{
    $CI = &get_instance();
    if(!is_admin()) {
        if(empty($CI->session->userdata('staff_ip')) && !empty(get_option('phone_login_active'))) {
            ktLoginIP();
        }
    }
    $CI->load->view('admin/includes/head');
...
// }
function has_permission_ch($permission)
{
    ...
    $CI->db->select('obj');
    $CI->db->where('obj', $permission);
    $permission = $CI->db->get('tbl_permission_v2')->row();
    if (!$permission) {
        return false;
    }
    $CI->db->select('1');
    $CI->db->where('obj_parent_permission', $permission->obj);
    $CI->db->where('id_staff', $_userid);
...
function has_permission_parent($permission, $staffid = '')
{
    $_permission = $permission;
    $CI =& get_instance();
    // if (function_exists($permission) && is_callable($permission)) {
    ...
    if($permission == 'parent_reports')
    {
        $CI->db->select('1');
        $CI->db->from('tbl_staff_parent_permission_v2');
        $CI->db->group_start();
        $CI->db->or_where('obj_parent_permission', 'parent_debt_customer');
        $CI->db->or_where('obj_parent_permission', 'parent_warehouse_reports');
        $CI->db->or_where('obj_parent_permission', 'parent_fund_balance');
        $CI->db->or_where('obj_parent_permission', 'parent_reports_purchase');
        $CI->db->or_where('obj_parent_permission', 'parent_reports');
        $CI->db->group_end();
        $CI->db->where('id_staff', $_userid);
        $CI->db->where('can_view', 1);
        $perm = $CI->db->get()->row();
    ...
    $CI->db->select('1');
    $CI->db->from('tbl_staff_parent_permission_v2');
    $CI->db->where('obj_parent_permission', $permission);
    $CI->db->where('id_staff', $_userid);
    $CI->db->where('can_view', 1);
    $perm = $CI->db->get()->row();
    if ($perm) {
...
function has_permission($permission, $staffid = '', $can = '')
{
    ...
    $CI->db->select('1');
    $CI->db->from('tbl_staff_child_permission_v2');
    $CI->db->where('obj_permission', $permission->obj);
    $CI->db->where('id_staff', $_userid);
...
function render_admin_js_variables()
{
    ...
    $lang = [
        ...
        'comfim_delete_all_list'                                  => _l('cong_comfim_delete_all_list'),
    ];
    ...
    if(function_exists('get_tags_color'))
    {
        echo 'app.available_tags_color = ' . json_encode(get_tags_color()) . ';';
    }
    if(function_exists('get_tags_background_color'))
    {
        echo 'app.available_tags_background_color = ' . json_encode(get_tags_background_color()) . ';';
    }
...
function get_table_where($table, $where = array(), $order_by = "", $result = 'result_array', $group_by = "", $select = "")
{
    $CI =& get_instance();
...
function get_table_query($_query = "", $result = "result_array")
{
    $CI =& get_instance();
    if(!empty($_query))
    {
        $_result = $CI->db->query($_query)->$result();
        return $_result;
    }
    return false;
}