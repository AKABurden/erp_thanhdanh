...
class Tasks_model extends App_Model
{
	...
	public function do_kanban_query($status, $search = '', $page = 1, $count = false, $where = [], $where_false = [])
	{
		...
		if (!empty($where_false)) {
			foreach ($where_false as $k => $v) {
				$this->db->where($v, false, false);
			}
		}
		if ($search != '') {
	...
	public function copy($data, $overwrites = [])
	{
		...
		if (!empty($task->duedate)) {
			...
			$_new_task_data['duedate'] = date('Y-m-d H:i:s', strtotime(date('Y-m-d H:i:s', strtotime('+' . $dDiff->days . 'DAY'))));
		}
    ...
    public function copyNext($data, $overwrites = [])
    {
        $task = $this->get($data['copy_from']);
        $fields_tasks = $this->db->list_fields(db_prefix() . 'tasks');
        $_new_task_data = [];
        ...
        $_new_task_data['dateadded'] = $data['dateadded'];
	...
	public function copy_task_checklist_items($from_task, $to_task)
	{
		...
		foreach ($checklists as $list) {
			...
			$process_child = $this->db->get_where('tblcategory_tasks_process_child', ['id_category_tasks_process' => $list['process_id']])->result_array();
			foreach ($process_child as $kk => $vv) {
				$this->db->insert('tbl_tasks_process_child', [
					'task' => $to_task,
					'id_category_tasks_process' => $vv['id'],
					'id_category_tasks' => $list['process_id'],
					'name' => $vv['name'],
	 ...
	 * @return mixed
	 */
	public function add($data, $clientRequest = false, $list_category_tasks = true)
	{
		...
		unset($data['rel_append_id']);

		
        $ktStartdate = explode('/', $data['startdate']);
        if (strlen($ktStartdate[0]) == 4) {
		...
		$ktDuedate = explode('/', $data['duedate']);
		if (strlen($ktDuedate[0]) == 4) {
		...
		if (isset($data['_addedfrom'])) {
			$data['addedfrom'] = $data['_addedfrom'];
			$staffNow = $data['_addedfrom'];
			unset($data['_addedfrom']);
		}
		...
		if (isset($data['copy_attachments'])) {
			$copy_attachments = $data['copy_attachments'];
			unset($data['copy_attachments']);
		}
		...
		if (isset($data['repeat_every']) && $data['repeat_every'] != '') {
			...
			} else if ($data['repeat_every'] == 'custom_day') {
				$data['custom_recurring'] = 2;
				$data['repeat_every'] = 1;
			} else {
		...
		if (!empty($data['custom_recurring']) && $data['custom_recurring'] == '2') {
			$custom_day = $data['custom_day'];
		}
		unset($data['custom_day']);
		...
		if ($insert_id) {
			...
			$this->db->update('tbltasks', ['order_by' => $insert_id]);

			if (!empty($data['category_tasks']) && !empty($list_category_tasks)) {
				$process = $this->db->get_where('tblcategory_tasks_process', ['id_category_tasks' => $data['category_tasks']])->result_array();
				if (!empty($process)) {
					if (!empty($data['rel_type']) && $data['rel_type'] == 'work_plan_task' && !empty($data['rel_id'])) {
						$process_work_plan_items = get_table_where('tbl_work_plan_items', array('work_plan_task_id' => $data['rel_id']));
					}
					foreach ($process as $key => $value) {
						$staff_id = NULL;
						if (!empty($process_work_plan_items)) {
							foreach ($process_work_plan_items as $kk => $vv) {
								if ($value['name'] == $vv['category_tasks_process_name']) {
						...
						$process_child = $this->db->get_where('tblcategory_tasks_process_child', ['id_category_tasks_process' => $value['id']])->result_array();
						foreach ($process_child as $kk => $vv) {
							$this->db->insert('tbl_tasks_process_child', [
								'task' => $insert_id,
								'id_category_tasks_process' => $vv['id'],
								'id_category_tasks' => $value['id'],
								'name' => $vv['name'],
								'approval_standards' => $vv['approval_standards'],
								'completion_control_standards' => $vv['completion_control_standards']
							]);
			...
			if (!empty($custom_day)) {
				foreach ($custom_day as $key => $value) {
					$this->db->insert('tbltasks_repeat_day', [
						'taskid' => $insert_id,
						'day' => $value,
					]);
				}
			...
			if ($clientRequest == false) {
				...
				if (!empty($copy_attachments)) {
					$task_path = get_upload_path_by_type('task') . $insert_id . '/';
					_maybe_create_upload_path($task_path);
					foreach ($copy_attachments as $attachment) {
						$path = FCPATH . $attachment['file_name'];
						if (file_exists($path)) {
							$f = fopen($path, FOPEN_READ);
							if ($f) {
								$filename = unique_filename($task_path, $attachment['file_name']);
								$fpt = fopen($task_path . $filename, 'w');
								if ($fpt && fwrite($fpt, stream_get_contents($f))) {
									$this->db->insert(db_prefix() . 'files', [
										'rel_id' => $insert_id,
	...
	public function add_Internal_proposal($data, $clientRequest = false, $list_category_tasks = true)
	{
		$ticket_to_task = false;
		if (isset($data['ticket_to_task'])) {
			$ticket_to_task = true;
	...
	public function update($data, $id, $clientRequest = false)
	{
		...
		$this->db->where('id', $id);
		$success = $this->db->update(db_prefix() . 'tasks', $data);
		...
		if (!empty($success)) {
			if (!empty($data['custom_recurring']) && $data['custom_recurring'] == '2') {
				...
				$this->db->delete('tbltasks_repeat_day');
			}
			if (!empty($custom_day)) {
				foreach ($custom_day as $key => $value) {
					$this->db->insert('tbltasks_repeat_day', [
			...
			$department_NotDelete = [];
			if (!empty($department_id)) {
				foreach ($department_id as $key => $value) {
					$this->db->where('task_id', $id);
					$this->db->where('department_id', $value);
					$kt_task_department = $this->db->get('tbltask_department')->row();
					if (empty($kt_task_department)) {
						$successDe = $this->db->insert('tbltask_department', [
							'department_id' => $value,
							'task_id' => $id,
							'create_by' => get_staff_user_id(),
						]);
						if (!empty($successDe)) {
							$department_NotDelete[] = $this->db->insert_id();
						}
					} else {
						$this->db->where('id', $kt_task_department->id);
						$this->db->update('tbltask_department', [
							'department_id' => $value,
							'task_id' => $id,
						]);
						$department_NotDelete[] = $kt_task_department->id;
					}
				}
			}
			$this->db->where('task_id', $id);
			if (!empty($department_NotDelete)) {
				$this->db->where_not_in('id', $department_NotDelete);
			}
			$this->db->delete('tbltask_department');
	...
	public function add_task_followers($data)
	{
		...
		if ($this->db->affected_rows() > 0) {
			...
			$this->_send_task_responsible_users_notification($description, $data['taskid'], $data['follower'], 'task_added_as_follower_to_staff', $additional_notification_data);
			return true;
		}
		return false;
	}
	...
	public function add_task_assignees($data, $cronOrIntegration = false, $clientRequest = false)
	{
		...
		} elseif (!empty($data['assigned_from'])) {
		...
		if ($assigneeId) {
			...
			if (get_staff_user_id() != $data['assignee'] || $clientRequest) {
				...
				@send_mail_task_assignees($member->email, $data['taskid']);
			}
	...
	{
		$this->db->select('rel_type,rel_id,name,visible_to_client,status, addedfrom');
		$this->db->where('id', $task_id);
		$task = $this->db->get(db_prefix() . 'tasks')->row();
		...
		if ($status == self::STATUS_COMPLETE) {
			...
			$update['staff_finished'] = get_staff_user_id();
		}
		$this->db->where('id', $task_id);
		...
		if ($this->db->affected_rows() > 0) {
			...
			// yct start
			$arrStaffReceive = array();
			$arrStaffCanAccess = $this->get_staff_members_that_can_access_task($task_id);
			foreach ($arrStaffCanAccess as $key => $staff) {
				$arrStaffReceive[] = $staff['staffid'];
			}
			if ($status == self::STATUS_COMPLETE) {
				notificationTaskComplete($task_id, $task->addedfrom, $arrStaffReceive);
			}
			// yct end
			return true;
		}
	 ...
	 * @return boolean
	 */
	public function _send_task_responsible_users_notification($description, $taskid, $excludeid = false, $email_template = '', $additional_notification_data = '', $comment_id = false)
	{
		$this->load->model('staff_model');
        if(!empty($excludeid)) {
            $staff = $this->db->get_where('tblstaff', ['staffid' => $excludeid, 'active' => 1])->result_array();
        }
        else {
...
//				}
			...
			if ($this->should_staff_receive_notification($member['staffid'], $taskid)) {
				...
				if ($email_template != '' && !empty($member['email'])) {
	...
	public function timer_tracking_api($task_id = '', $timer_id = '', $note = '', $adminStop = false, $staffid = '')
	{
		if ($task_id == '' && $timer_id == '') {
			return false;
		}
		if ($task_id !== '0' && $adminStop == false) {
			if (!$this->is_task_assignee($staffid, $task_id)) {
				return false;
			} elseif ($this->is_task_billed($task_id)) {
				return false;
			}
