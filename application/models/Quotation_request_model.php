...
class Quotation_request_model extends App_Model
{
    ...
    public function submit($formData, $id = null)
    {
        ...
        if (empty($id)) { //insert
            $formData['code'] = getReference('quotation_request');
        ...
        if (empty($id)) { // insert
            $this->db->insert('tblquotation_request', $submitData);
            $submitId = $this->db->insert_id();

            if (getReference('quotation_request') == $formData['code']) {
                updateReference('quotation_request');
            }
        } else { //update
            if ($this->db->update('tblquotation_request', $submitData, ['id' => $id])) {
                $submitId = $id;
            } else {
                $submitId = false;
            }
        ...
        if (!empty($submitId)) {
            ...
            foreach($arrItems as $itemIndex => $itemValue) {
                ...
                if (empty($submitItemData['id'])) { // insert
                    ...
                    $this->db->insert('tblquotation_request_item', $submitItemData);
                    $insertedId = $this->db->insert_id();
                    $ignoreDeleteId[] = $insertedId;
                } else { // update
                    $itemRowId = $submitItemData['id'];
                    unset($submitItemData['id']);
                    if ($this->db->update('tblquotation_request_item', $submitItemData, ['id' => $itemRowId])) {
            ...
            // Xóa bảng chi tiết
            $this->db->where('quotation_request_id', $submitId);
            $this->db->where_not_in('id', $ignoreDeleteId);
            $this->db->delete('tblquotation_request_item');
        }
    ...
    public function get($id) {
        $result = get_table_where('tblquotation_request', ['id' => $id], '', 'row_array');
    ...
    public function getItems($id) {
        ...
        $this->db->where('tblquotation_request_item.quotation_request_id', $id);
    ...
    public function delete($id) {
        $this->db->delete('tblquotation_request', ['id' => $id]);
        $this->db->delete('tblquotation_request_item', ['quotation_request_id' => $id]);
