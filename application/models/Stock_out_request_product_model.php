...
class Stock_out_request_product_model extends App_Model
{
    ...
    public function submit($formData, $id = null)
    {
        ...
        if (empty($id)) { //insert
            $formData['code'] = getReference('stock_out_request_product');
        ...
        if (empty($id)) { // insert
            $this->db->insert('tblstock_out_request_product', $submitData);
            $submitId = $this->db->insert_id();

            if (getReference('stock_out_request_product') == $formData['code']) {
                updateReference('stock_out_request_product');
            }
        } else { //update
            if ($this->db->update('tblstock_out_request_product', $submitData, ['id' => $id])) {
                $submitId = $id;
            } else {
                $submitId = false;
            }
        ...
        if (!empty($submitId)) {
            ...
            foreach($arrItems as $itemIndex => $itemValue) {
                ...
                $submitItemData = [
                    ...
                    'stock_out_request_product_id' => $submitId,
                    'item_type' => $itemType,
                    'item_id' => $itemId,
                    'order_id' => $orderId,
                ...
                if (empty($submitItemData['id'])) { // insert
                    ...
                    $this->db->insert('tblstock_out_request_product_item', $submitItemData);
                    $insertedId = $this->db->insert_id();
                    $ignoreDeleteId[] = $insertedId;
                } else { // update
                    $itemRowId = $submitItemData['id'];
                    unset($submitItemData['id']);
                    if ($this->db->update('tblstock_out_request_product_item', $submitItemData, ['id' => $itemRowId])) {
            ...
            // Xóa bảng chi tiết
            $this->db->where('stock_out_request_product_id', $submitId);
            $this->db->where_not_in('id', $ignoreDeleteId);
            $this->db->delete('tblstock_out_request_product_item');
        }
    ...
    public function get($id) {
        $result = get_table_where('tblstock_out_request_product', ['id' => $id], '', 'row_array');
    ...
    public function getItems($id) {
        ...
        $this->db->join('tbl_materials', 'tbl_materials.id = tblstock_out_request_product_item.item_id AND tblstock_out_request_product_item.item_type = "materials"', 'left');
        ...
        $this->db->where('tblstock_out_request_product_item.stock_out_request_product_id', $id);
    ...
    public function delete($id) {
        $this->db->delete('tblstock_out_request_product', ['id' => $id]);
        $this->db->delete('tblstock_out_request_product_item', ['stock_out_request_product_id' => $id]);
