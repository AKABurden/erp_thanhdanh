...
class Leads_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        if (isset($data['custom_contact_date']) || isset($data['contacted_today'])) {
            ...
            else
            {
                ...
                $data['date_contact'] = to_sql_date($data['custom_contact_date'], true);
            }
        }
        else
        {
        ...
        $data['description'] = !empty($data['description']) ? nl2br($data['description']) : NULL;
        $data['dateadded']   = date('Y-m-d H:i:s');
        $data['addedfrom']   = get_staff_user_id();
        $data['birtday']    = !empty($data['birtday']) ? to_sql_date($data['birtday']) : NULL;
        $data['date_create_company']    = !empty($data['birtday']) ? to_sql_date($data['date_create_company']) : NULL;
        ...
        if(empty($data['zcode']))
        {
            $data['zcode'] = get_option('prefix_leads').'-'.sprintf("%05d",(ch_getMaxID_items('id','tblleads')+1));
        }
        if(!empty($data['zcode']))
        {
            $data['code_type'] = 'TN';
        ...
        if ($insert_id) {
            ...
            CreateCode('lead', $insert_id , $data['date_contact'], ['code_system']);
            ...
            if(!empty($contacts))
            {
                ...
                {
                    $this->db->insert(db_prefix().'contacts_lead', [
                        'id_lead' => $insert_id,
                        ...
                        'note' => $value['note'],
                        'is_primary' => !empty($value['is_primary']) ? $value['is_primary'] : 0,
                        'birtday' => !empty($value['birtday']) ? to_sql_date($value['birtday']) : NULL,
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        if ($this->db->update(db_prefix() . 'leads', $data))
        {
            $this->log_lead_activity($id, 'cong_update_lead', false);
            $affectedRows++;
            ...
            if(!empty($contacts))
            {
                ...
                foreach($contacts as $key => $value)
                {
                    if(!empty($value['id']))
                    {
                        ...
                        if($this->db->update(db_prefix().'contacts_lead', $array_update))
                        {
                    ...
                    else
                    {
                        ...
                        $IdAddContact = $this->db->insert_id();
                        if($IdAddContact)
                        {
                ...
                $this->db->delete(db_prefix().'contacts_lead');
            }
            //Kết thúc thêm liên hệ
                    ...
                    if(is_array($value))
                    {
                        foreach($value as $k => $v)
                        {
                            if(!empty($v))
                            {
                                ...
                                $GetValue = $this->db->get(db_prefix().'lead_value')->row();
                                if(!empty($GetValue))
                                {
    ...
    public function delete($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->delete('tbllead_activity_log');

            $this->db->where('leadid', $id);
            $this->db->delete('tbllead_integration_emails');
           ...
           //Công bổ sung
            ...
            $advisory_lead = $this->db->get('tbladvisory_lead')->result_array();

            foreach($advisory_lead as $key => $value)
            {
                $this->db->where('id_advisory', $value['id']);
                $this->db->delete('tblprocedure_advisory_lead');
            }
            $this->db->where('id_object', $id);
            $this->db->where('type_object', 'lead');
            $this->db->delete('tbllog_advisory_lead');
            //end công bổ sung
    ...
    //Công bổ sung
    public function getInfoGroupLead($id = "")
    {
