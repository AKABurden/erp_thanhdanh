...
class Clients_model extends App_Model
{
	...
	public function get($id = '', $where = [])
	{
		...
		if (is_numeric($id)) {
			...
			if (!empty($client)) {
				...
				$client->contacts = $this->db->get(db_prefix() . 'contacts')->result_array();
			}
		...
		$this->db->order_by('company_short', 'asc');
		if ($this->input->post('limit_search')) {
			$this->db->limit($this->input->post('limit_search'));
		}
	...
	public function add($data, $client_or_lead_convert_request = false)
	{
		...
		if (!empty($data['date_create_company'])) {
			$data['date_create_company'] = to_sql_date($data['date_create_company']);
		}
		if (!empty($data['birtday'])) {
			$data['birtday'] = to_sql_date($data['birtday']);
		}
		...
		$data['prefix_client'] = get_option('prefix-client');
		if (empty($data['zcode'])) {
			$data['zcode'] = get_option('prefix-client') . '-' . sprintf("%05d", (ch_getMaxID_items('userid', 'tblclients') + 1));
		}
		//Mã loại hệ thống
		if (!empty($data['zcode'])) {
			$data['code_type'] = 'TN';
		...
		//End mã loại hệ thống
		$data['email_client'] = !empty($data['email_client']) ? $data['email_client'] : NULL;
		$data['declare_customs'] = !empty($data['declare_customs']) ? 1 : 0;
		$data['vat'] = !empty($data['vat']) ? $data['vat'] : NULL;
		$data['company'] = !empty($data['company']) ? $data['company'] : NULL;
		$data['phonenumber'] = !empty($data['phonenumber']) ? $data['phonenumber'] : NULL;

		if (!empty($data['date_accounting'])) {
			if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $data['date_accounting'])) {
				$data['date_accounting'] = to_sql_date($data['date_accounting']);
			}
		}

		if (!empty($data['status_activity'])) {
			if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $data['status_activity'])) {
		...
		if ($userid) {
			CreateCode('client', $userid, '', ['code_system']);
			if (!empty($data['address'])) {
				$create_by = get_staff_user_id();
				if (!empty($data['phonenumber']) && $data['address']) {
					$this->db->insert(db_prefix() . 'shipping_client', array(
						'client' => $userid,
						'name' => $data['company'],
						'phone' => $data['phonenumber'],
						'address' => $data['address'],
						'code_zip' => !empty($data['zip']) ? $data['zip'] : NULL,
						'address_primary' => 1,
			...
			if (!empty($contacts)) {
				foreach ($contacts as $key => $value) {
					$contact_add = $this->add_contact($value, $userid);
					if (!empty($value['is_primary']) || count($contacts) == 1) {
						$contact_id = $contact_add;
					}
				}
			}
			//Kết thúc thêm liên hệ
			if (isset($custom_fields)) {
				$_custom_fields = $custom_fields;
				// Possible request from the register area with 2 types of custom fields for contact and for comapny/customer
				unset($custom_fields);
				if (!empty($_custom_fields['customers'])) {
					$custom_fields['customers'] = $_custom_fields['customers'];
				}
				if (!empty($_custom_fields['leads'])) {
					foreach ($_custom_fields['leads'] as $key => $value) {
						$this->db->where('id', $key);
						$fields_lead = $this->db->get('tblcustomfields')->row();
						if (!empty($fields_lead)) {
							$this->db->select('tblcustomfields.id');
							$this->db->where('slug', str_replace($fields_lead->fieldto, 'customers', $fields_lead->slug));
							$this->db->where('fieldto', 'customers');
							$file_customer = $this->db->get('tblcustomfields')->row();
							if (!empty($file_customer)) {
								$custom_fields['customers'][$file_customer->id] = $value;
							}
						}
					}
					unset($_custom_fields['leads']);
				}
...
//            }
					...
					if (is_array($value)) {
						foreach ($value as $k => $v) {
							if (!empty($v)) {
								...
								$this->db->insert(db_prefix() . 'client_value', $array_group);
							}
						}
					} else {
						if (!empty($value)) {
	...
	public function update($data, $id, $client_request = false)
	{
		...
		if ($this->db->update(db_prefix() . 'clients', $data)) {
			$affectedRows++;
			//Thêm liên hệ
			$contact_not_delete = array();
			if (!empty($contacts)) {
				foreach ($contacts as $key => $value) {
					if (!empty($value['id'])) {
						$id_contact = $value['id'];
						unset($value['id']);
						if ($this->update_contact($value, $id_contact)) {
							$contact_not_delete[] = $id_contact;
						}
					} else {
						$IdAddContact = $this->add_contact($value, $id);
						if ($IdAddContact) {
			...
			if (!empty($info_detail)) {
					...
					if (is_array($value)) {
						foreach ($value as $k => $v) {
							if (!empty($v)) {
								...
								$GetValue = $this->db->get(db_prefix() . 'client_value')->row();
								if (!empty($GetValue)) {
				...
				$this->db->delete(db_prefix() . 'client_value');
			}
			//end
		}
	...
	public function delete($id)
	{
		...
		$export_different = $this->db->get_where('tblexport_different', ['object' => 1, 'id_object' => $id])->num_rows();
		$other_payslips_coupon = $this->db->get_where('tblother_payslips_coupon', ['objects' => 1, 'objects_id' => $id])->num_rows();
		$other_payslips = $this->db->get_where('tblother_payslips', ['objects' => 1, 'objects_id' => $id])->num_rows();
		if (!empty($orders) || !empty($quotes) || !empty($deliveries) || !empty($export_different) || !empty($other_payslips_coupon) || !empty($other_payslips)) {
			return false;
		}
		$affectedRows = 0;
		...
		if ($this->db->affected_rows() > 0) {
			...
			if (!empty($client->leadid)) {
				...
				$this->db->update(db_prefix() . 'leads', ['status' => 0]);
			}
	...
	// công bổ sung
	public function get_code_next_client()
	{
		$this->db->select_max('userid');
		$id_max = $this->db->get(db_prefix() . 'clients')->row();
		if (empty($id_max)) {
			return '000001';
		}
		...
		return $_code . ($id_max->userid + 1);
	}

	public function getInfoGroup($userid = "")
	{
		...
		foreach ($info_group as $key => $value) {
			if (!empty($userid)) {
				$this->db->select(
                    ...
                    (SELECT group_concat(value) FROM ' . db_prefix() . 'client_value where id_detail = ' . db_prefix() . 'client_info_detail.id AND client = ' . $userid . ') as value
                    '
				);
				$this->db->join(db_prefix() . 'client_value', db_prefix() . 'client_value.id_detail = ' . db_prefix() . 'client_info_detail.id and client = ' . $userid, 'left');
//                $this->db->group_by('id_detail');
	...
	public function searchCustomers($q, $limit = 50)
	{
		// $this->db->select('CONCAT("customers", "__", tblclients.userid) as id, IF(tblclients.fullname IS NOT null, tblclients.fullname, CONCAT(tblclients.prefix_client, "", tblclients.code_client)) as text, COALESCE(tblclients.fullname, "") as fullname, COALESCE(tblclients.phonenumber, "") as phonenumber', false);

		$year = date('Y');

		// AND tblgroup_price.year = '$year'
		$tbPrice = "coalesce((
			SELECT tblgroup_price.id
			FROM tblgroup_price
			WHERE tblgroup_price.client = tblclients.userid 
			ORDER BY tblgroup_price.id DESC
			LIMIT 1
		), 0)";

		$this->db->select('
			CONCAT("customers", "__", tblclients.userid) as id, 
			tblclients.company_short as text, 
			COALESCE(tblclients.zcode, "") as fullname, 
			COALESCE(tblclients.phonenumber, "") as phonenumber, 
			discount as discount, 
			'.$tbPrice.' as table_price_id, 
			tblclients.id_discount_client as id_discount_client, 
			tblclients.allowed_vat as allowed_vat, 
			tblclients.bale_parameters as bale_parameters, 
			tblclients.is_separate_guest as is_separate_guest
		', false);
		$this->db->from('tblclients');
		if (!empty($q)) {
			$this->db->group_start();
			$this->db->like('tblclients.company', $q);
			$this->db->or_like('tblclients.company_short', $q);
			$this->db->or_like('tblclients.fullname', $q);
	...
	public function searchCustomersLeads($q, $limit = 50)
	{
		$this->db->select('CONCAT("leads", "__", tblleads.id) as id, tblleads.company as text, COALESCE(tblleads.zcode, "") as fullname, COALESCE(tblleads.phonenumber, "") as phonenumber, 0 as discount, 0 as table_price_id, 0 as id_discount_client, tblleads.allowed_vat as allowed_vat', false);
		$this->db->from('tblleads');
		if (!empty($q)) {
			$this->db->group_start();
			$this->db->like('tblleads.name', $q);
			$this->db->or_like('tblleads.company', $q);
			$this->db->or_like('tblleads.phonenumber', $q);
			$this->db->or_like('tblleads.zcode', $q);
			$this->db->group_end();
		}
		$this->db->where("(
            SELECT COUNT(*)
            FROM tblclients
            WHERE tblclients.leadid = tblleads.id
            LIMIT 1
        ) = 0");
		$this->db->limit($limit);
		return $this->db->get()->result_array();
	...
	public function getShippingClientByClientId($client)
	{
		$this->db->select('*');
		$this->db->from('tblshipping_client');
		$this->db->where('tblshipping_client.client', $client);
		return $this->db->get()->result_array();
	}

	public function searchShippingClientByClientId($q, $limit = 50, $client_id)
	{
		$area = "(
            SELECT tbldelivery_area.code
            FROM tbldelivery_area
            INNER JOIN tbldelivery_area_detail ON tbldelivery_area_detail.id_delivery_area = tbldelivery_area.id
            WHERE tbldelivery_area.city = tblshipping_client.city_shipping AND tbldelivery_area_detail.id_district = tblshipping_client.district_shipping
        )";
		$this->db->select("tblshipping_client.id as id, CONCAT(tblshipping_client.address, '(Mã KV: ', COALESCE($area, ''), ')') as text", false);
		$this->db->from('tblshipping_client');
		if (!empty($q)) {
			$this->db->group_start();
			$this->db->like('tblshipping_client.address', $q);
			$this->db->or_like('tblshipping_client.phone', $q);
			$this->db->or_like('tblshipping_client.code_zip', $q);
			$this->db->or_like("$area", $q);
			$this->db->group_end();
		}
		$this->db->where('tblshipping_client.client', $client_id);
		$this->db->limit($limit);
		return $this->db->get()->result_array();
	}

	public function searchShippingLeadByLeadId($q, $limit = 50, $lead_id)
	{
		$this->db->select('tblshipping_lead.id as id, tblshipping_lead.address as text', false);
		$this->db->from('tblshipping_lead');
		if (!empty($q)) {
			$this->db->group_start();
			$this->db->like('tblshipping_lead.address', $q);
			$this->db->or_like('tblshipping_lead.phone', $q);
			$this->db->or_like('tblshipping_lead.code_zip', $q);
			$this->db->group_end();
		}
		$this->db->where('tblshipping_lead.lead_id', $lead_id);
		$this->db->limit($limit);
		return $this->db->get()->result_array();
	}

	public function searchOnlyCustomers($q, $limit = 50)
	{
		$this->db->select('tblclients.userid as id, tblclients.company_short as text, COALESCE(tblclients.fullname, "") as fullname, COALESCE(tblclients.phonenumber, "") as phonenumber, tblclients.is_separate_guest as is_separate_guest', false);
		$this->db->from('tblclients');
		if (!empty($q)) {
			$this->db->group_start();
			$this->db->like('tblclients.company', $q);
	...
	public function searchContract($q, $limit = 50, $user_id)
	{
		$this->db->select('tblcontacts.id as id, CONCAT(COALESCE(tblcontacts.firstname, ""), " ", COALESCE(tblcontacts.lastname, ""), "") as text', false);
		$this->db->from('tblcontacts');
		if (!empty($q)) {
			$this->db->group_start();
			$this->db->like('tblcontacts.firstname', $q);
			$this->db->or_like('tblcontacts.lastname', $q);
			$this->db->or_like('tblcontacts.email', $q);
			$this->db->or_like('tblcontacts.id', $q);
			$this->db->group_end();
		}
		$this->db->where('tblcontacts.userid', $user_id);
		$this->db->limit($limit);
		return $this->db->get()->result_array();
	}

	public function searchContractLead($q, $limit = 50, $id_lead)
	{
		$this->db->select('tblcontacts_lead.id as id, CONCAT(COALESCE(tblcontacts_lead.firstname, ""), " ", COALESCE(tblcontacts_lead.lastname, ""), "") as text', false);
		$this->db->from('tblcontacts_lead');
		if (!empty($q)) {
			$this->db->group_start();
			$this->db->like('tblcontacts_lead.firstname', $q);
			$this->db->or_like('tblcontacts_lead.lastname', $q);
			$this->db->or_like('tblcontacts_lead.email', $q);
			$this->db->or_like('tblcontacts_lead.id', $q);
			$this->db->or_like('tblcontacts_lead.phonenumber', $q);
			$this->db->group_end();
		}
		$this->db->where('tblcontacts_lead.id_lead', $id_lead);
		$this->db->limit($limit);
		return $this->db->get()->result_array();
	}
}
