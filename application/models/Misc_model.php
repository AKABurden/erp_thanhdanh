...
class Misc_model extends App_Model
{
	 ...
	 * @return array
	 */
	public function get_user_notifications($read = false, $type_noti = false)
	{
		return false;
		$read = $read == false ? 0 : 1;
		...
		$whereType = '';
		if ($type_noti != false) {
			$whereType = ' AND tblnotifications.type=' . $type_noti . '';
		}
		$sql = 'SELECT COUNT(*) as total FROM ' . db_prefix() . 'notifications WHERE isread=' . $read . ' AND touserid=' . $staff_id . $whereType;
		$sql .= ' UNION ALL ';
		$sql .= 'SELECT COUNT(*) as total FROM ' . db_prefix() . 'notifications WHERE isread_inline=' . $read . ' AND touserid=' . $staff_id . $whereType;
		...
		// In this case we are limiting to 15
	...
	public function _search_color($q, $limit = 0)
	{
		$result = [
			'result' => [],
			'type' => 'color',
			'search_heading' => _l('ch_color'),
		];
		// Staff
		$this->db->select();
		$this->db->from(db_prefix() . '_colors');
		$this->db->like('code', $q);
	...
	public function _search_items($q, $data, $limit = 0, $id_order = '')
	{
		$result = [
			'result' => [],
			'type' => 'items',
		...
		if ($data != -1) {
			$table = get_table_where('tbltype_items', array('type' => $data), '', 'row');
			if (!empty($id_order)) {
				$this->db->select($table->table . '.*,tblpurchase_order_items.quantity_suppliers');
			} else {
				$this->db->select($table->table . '.*');
			}
			$this->db->from($table->table);
			$this->db->where('(code LIKE "%' . $q . '%" OR name LIKE "%' . $q . '%")');
			if (!empty($id_order)) {
				$this->db->join('tblpurchase_order_items', 'tblpurchase_order_items.product_id = ' . $table->table . '.id ');
				$this->db->where('tblpurchase_order_items.id_purchase_order', $id_order);
				$this->db->where('tblpurchase_order_items.type', $table->type);
			}
			if ($limit != 0) {
				$this->db->limit($limit);
			}
			...
			$dem_temp = 0;
			$result['result'][$dem_temp] = array('id' => 'h', 'name' => $table->name, 'type_items' => $table->type);
			$dem_temp++;
			foreach ($data as $key_data => $value_data) {
				$value_data = array_merge($value_data, array('type_items' => $table->type));
				if (!empty($id_order)) {
					$quantity = sum_quantity_import($table->type, $id_order, $value_data['id']);
					if (empty($quantity)) {
						$quantity = 0;
					}
					$value_data['quantity_suppliers'] = $value_data['quantity_suppliers'] - $quantity;
			...
			foreach ($table as $key => $value) {
				...
				$dem_temp++;
				if (!empty($id_order)) {
					$this->db->select($value['table'] . '.*,' . $value['id_type'] . ' as type,tblpurchase_order_items.quantity_suppliers');
					$this->db->from('tblpurchase_order_items');
				} else {
					$this->db->select($value['table'] . '.*,' . $value['id_type'] . ' as type');
					$this->db->from($value['table']);
				}
				if ($value['type'] == 'items') {
					$this->db->where($value['table'] . '.active', 1);
					$this->db->where($value['table'] . '.type', 1);
				}
				$this->db->where('(' . $value['table'] . '.code LIKE "%' . $q . '%" OR ' . $value['table'] . '.name LIKE "%' . $q . '%")');
	...
	public function _search_purchases($q, $data, $limit = 0, $id_order = '')
	{
		$result = [
			'result' => [],
			'type' => 'items',
	...
	public function _search_rfq($q, $data, $limit = 0, $id_order = '')
	{
		$result = [
			'result' => [],
			'type' => 'items',
		...
		$result['result'] = array();
		$this->db->select('tblrfq_ask_price.*');
	...
	public function _search_supplier_quotes($q, $data, $limit = 0, $id_order = '')
	{
		$result = [
			'result' => [],
			'type' => 'items',
		...
		$result['result'] = array();
		$this->db->select('tblsupplier_quotes.*');
	...
	public function _search_purchase_order($q, $data, $limit = 0, $id_order = '')
	{
		$result = [
			'result' => [],
			'type' => 'items',
	...
	public function _search_import($q, $data, $limit = 0, $id_order = '')
	{
		$result = [
			'result' => [],
			'type' => 'items',
	...
	public function _return_supplier($q, $data, $limit = 0, $id_order = '')
	{
		$result = [
			'result' => [],
			'type' => 'items',
		...
		$result['result'] = array();
		$this->db->select('tblreturn_suppliers.*');
	...
	public function _search_items_order_quote($q, $data, $limit = 0, $id_quotes = '')
	{
		$result = [
			'result' => [],
			'type' => 'items',
		...
		if ($data != -1) {
			$table = get_table_where('tbltype_items', array('type' => $data), '', 'row');
			if (!empty($id_quotes)) {
				$this->db->select($table->table . '.*,tblsupplier_quote_items.quantity');
			} else {
				$this->db->select($table->table . '.*');
			}
			$this->db->from($table->table);
			$this->db->where('(code LIKE "%' . $q . '%" OR name LIKE "%' . $q . '%")');
			if (!empty($id_quotes)) {
				$this->db->join('tblsupplier_quote_items', 'tblsupplier_quote_items.product_id = ' . $table->table . '.id ');
				$this->db->where('tblsupplier_quote_items.id_supplier_quotes', $id_quotes);
				$this->db->where('tblsupplier_quote_items.type', $table->type);
			}
			if ($limit != 0) {
				$this->db->limit($limit);
			}
			...
			foreach ($data as $key_data => $value_data) {
				$value_data = array_merge($value_data, array('type_items' => $table->type));
				if (!empty($id_quotes)) {
					$quantity = sum_quantity_order($table->type, $id_quotes, $value_data['id']);
					if (empty($quantity)) {
						$quantity = 0;
					}
					$value_data['quantity'] = $value_data['quantity'] - $quantity;
					if ($value_data['quantity'] <= 0) {
					} else {
						$result['result'][$dem_temp] = $value_data;
						$dem_temp++;
					}
			...
			foreach ($table as $key => $value) {
				...
				$dem_temp++;
				if (!empty($id_quotes)) {
					$this->db->select($value['table'] . '.*,' . $value['id_type'] . ' as type,tblsupplier_quote_items.quantity');
				...
				foreach ($data as $key_data => $value_data) {
					$value_data = array_merge($value_data, array('type_items' => $value['type']));
					if (!empty($id_quotes)) {
						$quantity = sum_quantity_order($value['type'], $id_quotes, $value_data['id']);
						if (empty($quantity)) {
							$quantity = 0;
						}
						$value_data['quantity'] = $value_data['quantity'] - $quantity;
	...
	public function _search_items_order_id_purchases($q, $data, $limit = 0, $id_purchases = '')
	{
		$result = [
			'result' => [],
			'type' => 'items',
		...
		if ($data != -1) {
			...
			if (!empty($id_purchases)) {
				$this->db->select($table->table . '.*,tblpurchases_items.quantity_net');
			} else {
				$this->db->select($table->table . '.*');
			}
			$this->db->from($table->table);
			$this->db->where('(code LIKE "%' . $q . '%" OR name LIKE "%' . $q . '%")');
			if (!empty($id_purchases)) {
				$this->db->join('tblpurchases_items', 'tblpurchases_items.product_id = ' . $table->table . '.id ');
				$this->db->where('tblpurchases_items.purchases_id', $id_purchases);
				$this->db->where('tblpurchases_items.type', $table->type);
			}
			if ($limit != 0) {
				$this->db->limit($limit);
			}
			...
			foreach ($data as $key_data => $value_data) {
				$value_data = array_merge($value_data, array('type_items' => $table->type));
				if (!empty($id_purchases)) {
					$quantity_net = sum_quantity_order($table->type, $id_purchases, $value_data['id']);
					if (empty($quantity_net)) {
						$quantity_net = 0;
					}
					$value_data['quantity_net'] = $value_data['quantity_net'] - $quantity_net;
			...
			foreach ($table as $key => $value) {
				...
				$dem_temp++;
				if (!empty($id_purchases)) {
					$this->db->select($value['table'] . '.*,' . $value['id_type'] . ' as type,tblpurchases_items.quantity_net');
				...
				foreach ($data as $key_data => $value_data) {
					$value_data = array_merge($value_data, array('type_items' => $value['type']));
					if (!empty($id_purchases)) {
						$quantity_net = sum_quantity_order($value['type'], $id_purchases, $value_data['id']);
						if (empty($quantity_net)) {
							$quantity_net = 0;
						}
						$value_data['quantity_net'] = $value_data['quantity_net'] - $quantity_net;
	...
	public function changeRowNew_model($table = '', $id = '')
	{
		if ($table != '' && $id != '') {
	...
	{
		$getData = get_table_where('tblpurchase_order', array('status <>' => 4, 'cancel' => 0));
		foreach ($getData as $key => $value) {
			$dateWarning = strtotime(date('Y-m-d')) - strtotime(date("Y-m-d", strtotime($value['date_create'])));
			$numberDay = $dateWarning / 86400;
			$getPriority = get_table_where('tbltickets_priorities', array('date <>' => NULL), 'date DESC');
			foreach ($getPriority as $keyPriority => $valuePriority) {
				if ($numberDay >= $valuePriority['date']) {
					$this->db->set('id_tickets_priorities', $valuePriority['priorityid']);
