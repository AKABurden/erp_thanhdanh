...
class Purchase_order_request_model extends App_Model
{
    ...
    public function submit($formData, $id = null)
    {
        ...
        // var_dump($formData['date']);die;
        $formData['date'] = (!empty($formData['date']) ? to_sql_date($formData['date'], true) : null);
        if (empty($id)) { //insert
            $formData['code'] = getReference('purchase_order_request');
        ...
        if (empty($id)) { // insert
            $this->db->insert('tblpurchase_order_request', $submitData);
            $submitId = $this->db->insert_id();

            if (getReference('purchase_order_request') == $formData['code']) {
                updateReference('purchase_order_request');
            }
        } else { //update
            if ($this->db->update('tblpurchase_order_request', $submitData, ['id' => $id])) {
                $submitId = $id;
            } else {
                $submitId = false;
            }
        ...
        if (!empty($submitId)) {
            ...
            $arrtimequota = $formData['timequota'] ?? [];
            ...
            foreach($arrItems as $itemIndex => $itemValue) {
                ...
                $submitItemData = [
                    ...
                    'purchase_order_request_id' => $submitId,
                    'item_type' => $itemType,
                    'item_id' => $itemId,
                    'brand_ids' => (!empty($arrItemBrand[$itemIndex]) ? implode(',', $arrItemBrand[$itemIndex]) : null),
                    'link' => $arrItemLink[$itemIndex] ?? null,
                    'timequota' => (!empty($arrtimequota[$itemIndex]) ? number_unformat($arrtimequota[$itemIndex]) : null),
                ];

                if (empty($submitItemData['id'])) { // insert
                    unset($submitItemData['id']);
                    $this->db->insert('tblpurchase_order_request_item', $submitItemData);
                    $insertedId = $this->db->insert_id();
                    $ignoreDeleteId[] = $insertedId;
                } else { // update
                    $itemRowId = $submitItemData['id'];
                    unset($submitItemData['id']);
                    if ($this->db->update('tblpurchase_order_request_item', $submitItemData, ['id' => $itemRowId])) {
            ...
            // Xóa bảng chi tiết
            $this->db->where('purchase_order_request_id', $submitId);
            $this->db->where_not_in('id', $ignoreDeleteId);
            $this->db->delete('tblpurchase_order_request_item');
        }
    ...
    public function get($id) {
        $result = get_table_where('tblpurchase_order_request', ['id' => $id], '', 'row_array');
    ...
    public function getItems($id) {
        ...
        $this->db->where('tblpurchase_order_request_item.purchase_order_request_id', $id);
    ...
    public function delete($id) {
        $this->db->delete('tblpurchase_order_request', ['id' => $id]);
        $this->db->delete('tblpurchase_order_request_item', ['purchase_order_request_id' => $id]);
