...
class Authentication_model extends App_Model
{
     ...
     */
    public function login_mobiapp($phone, $password)
    {
        if ((!empty($phone)) and (!empty($password))) {
            ...
            if ($user) {
                ...
                if (!app_hasher()->CheckPassword($password, $user->password)) {
                    log_activity('Failed Login Attempt [Phone Number: ' . $phone . ', Is Staff Member: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');

                    // Password failed, return
                    return 1;
                }
            } else {
                log_activity('Non Existing User Tried to Login [Phone Number: ' . $phone . ', Is Staff Member: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');

                return 2;
            }

            if ($user->active == 0) {
                log_activity('Inactive User Tried to Login [Phone Number: ' . $phone . ', Is Staff Member: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');
