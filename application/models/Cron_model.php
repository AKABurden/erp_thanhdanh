...
class Cron_model extends App_Model
{
    ...
    {
		if(date('Y-m-d') == "2024-04-29" || date('Y-m-d') == "2024-04-30" || date('Y-m-d') == "2024-05-01") {
			return true;
		}

//        if()

        $this->db->select('id, addedfrom, recurring_type, repeat_every, last_recurring_date, startdate, duedate, recurring, custom_recurring');
        $this->db->where('recurring', 1);
        $this->db->where('(cycles != total_cycles OR cycles=0)');
		$this->db->where('DATE_FORMAT(startdate, "%Y-%m-%d") <= "' . date('Y-m-d') . '"');
		$this->db->where('id > 83790', false, false);
		$this->db->where('(DATE_FORMAT(last_recurring_date, "%Y-%m-%d") < "' . date('Y-m-d') . '" OR last_recurring_date IS NULL)', false, false);
		$this->db->where('(DATE_FORMAT(date_request_recurring, "%Y-%m-%d") < "' . date('Y-m-d') . '" OR date_request_recurring IS NULL)', false, false);
        $recurring_tasks = $this->db->get(db_prefix() . 'tasks')->result_array();
        foreach ($recurring_tasks as $task) {
			$re_create_at = NULL;
			$this->db->where('id', $task['id']);
			$this->db->update('tbltasks', ['date_request_recurring' => date('Y-m-d')]);
			$is_sunDay = date('D') == 'Sun' ? true : false;
			...
			if($task['custom_recurring'] == '2') {
				...
				$this->db->where('FORMAT(DATE_FORMAT(NOW(), "%d"), 0) = tbltasks_repeat_day.day', false, false);
				$tasks_repeat = $this->db->get('tbltasks_repeat_day')->row();
				if(!empty($tasks_repeat)) {
					$re_create_at = date('Y-m-d');
				}
			}
			else {
            ...
            if (date('Y-m-d') >= $re_create_at) {
                ...
                if (!empty($task['duedate'])) {
                    ...
                    $dEnd                        = new DateTime(to_sql_date(_dC($task['duedate'])));
                    $dDiff                       = $dStart->diff($dEnd);
                    $overwrite_params['duedate'] = date('Y-m-d', strtotime('+' . $dDiff->days . ' days', strtotime($re_create_at)));
					$overwrite_params['duedate'] = $overwrite_params['duedate'].' 17:00:00';
                }
                ...
                if ($newTaskID) {
					...
					if(!empty($assigned)) {
						...
						$rowTask = $this->site_model->rowTasks($newTaskID);
						...
						$departments_task_name = $this->db->get_where('tbltask_department', ['task_id' => $task['id']])->row('list_name');
						...
						$dataHtml = '<img src="' . base_url('uploads/foso.png') . '" class="staff-profile-image-small img-circle notification-image pull-left" alt="' . lang('system') . '"> 
										Hệ thống - '.$rowTask['addedfrom'].' Vừa phân công công việc '. $rowName . ' vào ngày ' . _dhau($rowTask['startdate']) . ' Vui lòng theo dõi và tiến hành cập nhật công việc !';
						...
						foreach ($assigned as $assignee) {
							...
							if ($assigneeId) {
								$array_assigned[] = $assignee['staffid'];
								...
								$notification_data = [
									...
									'touserid' => $assignee['staffid'],
									'link' => '#taskid=' . $newTaskID,
									'type' => 6,
									'object_id' => $newTaskID,
									'object_type' => 'tasks',
									'onclick' => 'init_task_modal(' . $newTaskID . '); return false;',
								];
...
//									pusher_trigger_notification($notification_data);
						...
						if(!empty($array_assigned)) {
							notificationTaskAssigned($newTaskID, $rowTask['addedfrom'], $array_assigned);
						}
					}

					$this->db->where('taskid', $newTaskID);
					$task_followers = $this->db->get('tbltask_followers')->result_array();
					if(!empty($task_followers)) {
						...
						$dataHtml = '<img src="' . base_url('uploads/foso.png') . '" class="staff-profile-image-small img-circle notification-image pull-left" alt="' . lang('system') . '"> 
							Hệ thống - '.get_staff_full_name($rowTask['addedfrom']).' Vừa thêm bạn vào người theo dõi công vệc ['.$rowTask['name'].'] '. $rowName . '</b> vào lúc ' . _dhau(date('Y-m-d H:i:s'));
						$list_staff_follower = [];
						$list_notification_data = [];
						foreach($task_followers as $key => $follower) {
							$list_notification_data[] = [
								'date' => date('Y-m-d H:i:s'),
								'description' => $dataHtml,
								'touserid' => $follower['staffid'],
								'link' => '#taskid=' . $newTaskID,
								'type' => 6,
								'object_id' => $newTaskID,
								'object_type' => 'tasks',
							...
							$list_staff_follower[] = $follower['staffid'];
						}
						if (!empty($list_notification_data)) {
							$this->db->insert_batch('tblnotifications', $list_notification_data);
						}
						if(!empty($list_staff_follower)) {
							send_notification_app_c($rowTask['id'], [
								'description' => 'Hệ thống - ' . $rowTask['addedfrom'] . ' vừa thêm bạn vào người theo dõi công việc vào lúc ' . _dt(date('Y-m-d H:i:s')),
								'title' => 'Theo dõi công việc công việc',
								'code' => $rowTask['name'],
								'object_type' => 'tasks'
							], $list_staff_follower, $rowTask['addedfrom']);
						}
					}

					$this->db->where('task_id', $task['id']);
					$task_department = $this->db->get('tbltask_department')->result_array();
					foreach ($task_department as $department) {
						$this->db->insert('tbltask_department', [
    ...
    public function recurring_tasks_week()
    {
        if(date('Y-m-d') == "2024-04-29" || date('Y-m-d') == "2024-04-30" || date('Y-m-d') == "2024-05-01") {
            return true;
        }
        $dateNow = date('Y-m-d');
        // $dateLast = strtotime ( '+1 week' , strtotime ( $dateNow ) ) ;
        $dateLast = strtotime('+2 day', strtotime($dateNow));
		...
		$this->db->where('id > 83790', false, false);
        $this->db->where('(cycles != total_cycles OR cycles=0)');
        $this->db->where('DATE_FORMAT(startdate, "%Y-%m-%d") <= "' . $dateLast . '"');
        $this->db->where('(DATE_FORMAT(last_recurring_date, "%Y-%m-%d") < "' . $dateLast . '" OR last_recurring_date IS NULL)', false, false);
        $this->db->where('(DATE_FORMAT(date_request_recurring, "%Y-%m-%d") < "' . $dateNow . '" OR date_request_recurring IS NULL)', false, false);
        $recurring_tasks = $this->db->get('tbltasks')->result_array();
        $dataResult = [];
        foreach ($recurring_tasks as $task) {
            $dateKeyNow = !empty($task['last_recurring_date']) ? $task['last_recurring_date'] : $task['startdate'];
            if(strtotime($dateKeyNow) >= strtotime($dateLast)) {
                continue;
            }
            $dtStart = new DateTime(to_sql_date(_dC($dateKeyNow)));
            $dtEnd = new DateTime(to_sql_date(_dC($dateLast)));
            ...
            $this->db->where('id', $task['id']);
            $this->db->update('tbltasks', ['date_request_recurring' => $dateNow]);

            $this->db->where('task_id', $task['id']);
            $task_department = $this->db->get('tbltask_department')->result_array();

            $this->db->select('GROUP_CONCAT(tbldepartments.name) as list_name')
                ->join('tbldepartments', 'tbldepartments.departmentid = tbltask_department.department_id');
            $departments_task_name = $this->db->get_where('tbltask_department', ['task_id' => $task['id']])->row('list_name');

            for($iKey = 1; $iKey <= $maxKey; $iKey++) {
                $is_sunDay = false;
                $dateKey = strtotime ( "+$iKey day" , strtotime ($dateKeyNow) ) ;
                $dateKey = date ( 'Y-m-d' , $dateKey );
                $re_create_at = NULL;
                $is_sunDay = date('D', strtotime($dateKey)) == 'Sun' ? true : false;
                if(empty($is_sunDay)) {
                    if(strtotime($dateKey) >= strtotime(date('2026-02-14')) && strtotime($dateKey) <= strtotime(date('2026-02-22'))) {
                        $is_sunDay = true;
                    }
                }
                ...
                if ($task['custom_recurring'] == '2') {
                    $this->db->where('taskid', $task['id']);
                    $this->db->where('FORMAT(DATE_FORMAT("'.$dateKey.'", "%d"), 0) = tbltasks_repeat_day.day', false, false);
                    $tasks_repeat = $this->db->get('tbltasks_repeat_day')->row();
                    if (!empty($tasks_repeat)) {
                        $re_create_at = $dateKey;
                    }
                } else {
                    $re_create_at = date('Y-m-d', strtotime('+' . $repeat_every . ' ' . strtoupper($type), strtotime($last_recurring_date)));
                }
                ...
                if ($dateKey >= $re_create_at) {
                    if(empty($is_sunDay)) {
                        $is_sunDay = date('D', strtotime($dateKey)) == 'Sun' ? true : false;
                    }
                    if (!empty($is_sunDay)) {
                        $this->db->where('id', $task['id']);
                        $this->db->update('tbltasks', ['last_recurring_date' => $dateKey]);
                    ...
                    if (!empty($task['duedate'])) {
                        ...
                        $dEnd = new DateTime(to_sql_date(_dC($task['duedate'])));
                        $dDiff = $dStart->diff($dEnd);
                        $overwrite_params['duedate'] = date('Y-m-d', strtotime('+' . $dDiff->days . ' days', strtotime($dateKey)));
                        $overwrite_params['duedate'] = $overwrite_params['duedate'] . ' 17:00:00';
                    }
                    $copy_task_data['dateadded'] = date('Y-m-d H:i:s');

                    $copy_task_data['startdate'] = $dateKey;
                    $task['last_recurring_date'] = $copy_task_data['startdate'];
                    $overwrite_params['startdate'] = $copy_task_data['startdate'];
                    foreach($overwrite_params as $kk => $vv) {
                        $copy_task_data[$kk] = $vv;
                    }
                    $dataResult[] = $copy_task_data;
                    $newTaskID = $this->tasks_model->copyNext($copy_task_data, $overwrite_params);
                    if ($newTaskID) {
                        $this->db->where('id', $task['id']);
                        $this->db->update('tbltasks', ['last_recurring_date' => $copy_task_data['startdate']]);

                        $this->db->where('id', $task['id'])
                            ->set('total_cycles', 'total_cycles+1', false)
                            ->update('tbltasks');
                        $rowTask = $this->site_model->rowTasks($newTaskID);

                        $this->db->where('taskid', $task['id']);
                        $assigned = $this->db->get('tbltask_assigned')->result_array();
                        if (!empty($assigned)) {
                            $task_rel_data = get_relation_data($rowTask['rel_type'], $rowTask['rel_id']);
                            $task_rel_value = get_relation_values($task_rel_data, $rowTask['rel_type']);
                            $rowName = '';
                            ...
                            $array_assigned = [];
                            foreach ($assigned as $assignee) {
                                $assigneeId = $this->tasks_model->add_task_assignees(['taskid' => $newTaskID, 'assignee' => $assignee['staffid']], true);
                                if ($assigneeId) {
                                    $array_assigned[] = $assignee['staffid'];
                                    $this->db->where('id', $assigneeId);
                                    $this->db->update('tbltask_assigned', [
                                        'assigned_from' => $task['addedfrom']
                                    ]);
                                    $notification_data = [];
                                    $notification_data[] = ['date' => date('Y-m-d H:i:s'), 'description' => $dataHtml, 'touserid' => $assignee['staffid'], 'link' => '#taskid=' . $newTaskID, 'type' => 6, 'object_id' => $newTaskID, 'object_type' => 'tasks', 'onclick' => 'init_task_modal(' . $newTaskID . '); return false;',];
                                    if (!empty($notification_data)) {
                                        $this->db->insert_batch('tblnotifications', $notification_data);
                                    }
                                }
                            }
                        }
                        ...
                        if (!empty($task_followers)) {
                            ...
                            $list_notification_data = [];
                            foreach ($task_followers as $key => $follower) {
                                $list_notification_data[] = ['date' => date('Y-m-d H:i:s'), 'description' => $dataHtml, 'touserid' => $follower['staffid'], 'link' => '#taskid=' . $newTaskID, 'type' => 6, 'object_id' => $newTaskID, 'object_type' => 'tasks', 'onclick' => 'init_task_modal(' . $newTaskID . '); return false;',];
                            }
                            if (!empty($list_notification_data)) {
                                $this->db->insert_batch('tblnotifications', $list_notification_data);
                            }
                        ...
                        if(!empty($task_department)) {
                            $insert_batch_department = [];
                            foreach ($task_department as $department) {
                                $insert_batch_department[] = ['department_id' => $department['department_id'], 'task_id' => $newTaskID, 'create_by' => $department['create_by']];
                            }
                            if (!empty($insert_batch_department)) {
                                $this->db->insert_batch('tbltask_department', $insert_batch_department);
                            }
                        }
                    }
                }
