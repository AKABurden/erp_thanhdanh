...
class App_menu
{
    ...
    public function get($group)
    {
        ...
        foreach ($items as $parent => $item) {
            ...
            foreach($item as $child_parent => $_item)
            {
                $items[$parent]['children'][$child_parent]['children'] = $this->get_child($child_parent, $group);
            }
        }
    ...
    public function filter_item($items, $slug)
    {
        foreach ($items as $item) {
            ...
            {
                if (!empty($child['slug']) && $child['slug'] == $slug) {
                    return $child;
                }
                if(!empty($child['children']))
                {
                    foreach ($child['children'] as $_child) {
                        if (!empty($_child['slug']) && $_child['slug'] == $slug) {
                            return $_child;
                        }
                    }
                }
