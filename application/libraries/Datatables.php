...
class Datatables
{
    ...
    public function select($columns, $backtick_protect = TRUE)
    {
        ...
        if (!empty($this->select_more)) {
            foreach ($this->select_more as $key => $value) {
                $this->columns[] = $value;
                $this->select[$value] = trim(preg_replace('/(.*)\s+as\s+(\w*)/i', '$1', $value));
            }
        }
    ...
    public function select_more($select_more) {
        $this->select_more = $select_more;
    }
    ...
    public function custom_ordering($data = [])
    {
        foreach ($data as $key => $value) {
            $this->columns[$value['index']] = $value['select'];
        }
    }

    public function custom_select($data = [])
    {
        $this->columns_select = $data;
    }
    ...
    private function get_ordering()
    {
        ...
        {
            // print_arrays($mColArray[intval($this->ci->input->post('iSortCol_' . $i))], '-', $mColArray[intval($this->ci->input->post('iSortCol_' . $i))], $columns);
        ...
        // print_arrays($this->ci->db->get_compiled_select('tbl_materials'));
        // print_arrays($this->ci->input->post(), $mColArray[intval($this->ci->input->post('iSortCol_0'))], $this->ci->input->post('sSortDir_0'));
    }
            ...
            for ($i = 0; $i < count($mColArray); $i++){
                if ($this->ci->input->post('bSearchable_' . $i) == 'true' && in_array($mColArray[$i], $columns)){
                    if (!empty($this->select[$mColArray[$i]])) {
                        ...
                        //custom
                        if (!empty($this->columns_select[$i]))
                        {
                            $sWhere .= $this->columns_select[$i] . " LIKE '%" . $sSearch . "%' OR ";
                        }
                    }
                }
            }
