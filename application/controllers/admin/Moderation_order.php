...
class Moderation_order extends AdminController
{
    public function __construct()
    {
        parent::__construct();
        $this->preViewModerationOrder = true;
        $this->preViewOwnModerationOrder = true;
        $this->preAddModerationOrder = true;
        $this->preEditModerationOrder = true;
        $this->preApproveModerationOrder = true;
        $this->preDeleteModerationOrder = true;
    }

    public function index()
    {
        if (!$this->preViewModerationOrder && !$this->preViewOwnModerationOrder) {
            access_denied();
        }
        $data['title'] = _l('ch_moderation_order');
        $this->load->view('admin/moderation_order/index', $data);
    }
    public function getModerationOrder()
    {
        ...
        $aColumns = [
            'tblpurchase_order_request.id as idMain',
            'tblpurchase_order_request_item.id as id',
            'tblpurchase_order_request.date as date',
            'tblpurchase_order_request.code as reference_no',
            ...
            'tblpurchase_order_request_item.link as link',
            'tblpurchase_order_request_item.timequota as quota_time_change_one',
            'tblpurchase_order_request_item.expected_date as expected_date',
            'tblpurchase_order_request_item.start_date as start_date',
            'tblpurchase_order_request_item.end_date as end_date',
            'IF(tbl_products.images IS NOT NULL && tbl_products.images != "", CONCAT("uploads/products/", "", tbl_products.images, ""), "") as image'
        ];
        $sIndexColumn = 'id';
        $sTable = 'tblpurchase_order_request_item';
        $where = [];
        $filter = [];

        $join = [
            'INNER JOIN tblpurchase_order_request ON tblpurchase_order_request.id = tblpurchase_order_request_item.purchase_order_request_id',
            'LEFT JOIN tbl_products ON tbl_products.id = tblpurchase_order_request_item.item_id AND tblpurchase_order_request_item.item_type = "products"',
            'LEFT JOIN ' . $tb_brand . ' ON FIND_IN_SET(tb_brand.id, tblpurchase_order_request_item.brand_ids) > 0',
            ...
            'LEFT JOIN tblclients ON tblclients.userid = tblpurchase_order_request.client_id',
            'LEFT JOIN ' . $tb_customer_group . ' ON tb_customer_group.client_id = tblpurchase_order_request.client_id',
        ];
        if (!$this->preViewModerationOrder) {
            array_push($where, 'AND (tblpurchase_order_request.create_by = ' . get_staff_user_id() . ')');
        }

        if (!empty($start_date_search)) {
            $start_date_search = to_sql_date($start_date_search) . ' 00:00:00';
            array_push($where, "AND tblpurchase_order_request.date >= '" . $start_date_search . "'");
        }

        if (!empty($end_date_search)) {
            $end_date_search = to_sql_date($end_date_search) . ' 23:59:59';
            array_push($where, "AND tblpurchase_order_request.date <= '" . $end_date_search . "'");
        }


        $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [], 'GROUP BY tblpurchase_order_request_item.id', [], '', 'HAVING tblpurchase_order_request_item.id > 0');
        ...
        foreach ($rResult as $key => $aRow) {
            ...
            $row[] = '<div class="text-center" style="width: 80px">' . ($aRow['link']) . '</div>';
            ...
            foreach (getListColumTable() as $kk => $vv) {
                $_data = getDataModeration($aRow['idMain'],$vv['id'],'tblpurchase_order_request');
    ...
    function updateDate()
    {
        ...
        if (!empty($data['id']) && !empty($data['_value']) && !empty($data['name'])) {
            ...
            $result = $this->db->update('tblpurchase_order_request_item', $ins);
            if (!empty($result)) {
                $data['result'] = 1;
    ...
    public function exportExcel()
    {
        ...
        if ($this->input->post('export_excel')) {
            ...
            $inputFileName = 'uploads/import_ch/Phieu_dieu_do_cong_viec_don_dat_hang.xlsx';
            ...
            if (!$this->preViewModerationOrder) {
                $this->db->where('(tblpurchase_order_request.create_by = ' . $staff_id . ')');
            }

            if (!empty($start_date_search)) {
                $start_date_search = to_sql_date($start_date_search) . ' 00:00:00';
                $this->db->where("tblpurchase_order_request.date >= '" . $start_date_search . "'");
            }

            if (!empty($end_date_search)) {
                $end_date_search = to_sql_date($end_date_search) . ' 23:59:59';
                $this->db->where("tblpurchase_order_request.date <= '" . $end_date_search . "'");
            }
            $this->db->order_by('tblpurchase_order_request.id desc');
            ...
            foreach ($items as $key => $value) {
                ...
                $objPHPExcel->getActiveSheet()->setCellValueExplicit($columsExcel[3] . $row, ($value['item_code']), PHPExcel_Cell_DataType::TYPE_STRING);
                $objPHPExcel->getActiveSheet()->setCellValueExplicit($columsExcel[4] . $row, ($value['item_name']), PHPExcel_Cell_DataType::TYPE_STRING);
                $objPHPExcel->getActiveSheet()->setCellValueExplicit($columsExcel[5] . $row, ($value['category_name']), PHPExcel_Cell_DataType::TYPE_STRING);
                $objPHPExcel->getActiveSheet()->setCellValueExplicit($columsExcel[6] . $row, ($value['specie_name']), PHPExcel_Cell_DataType::TYPE_STRING);
                $objPHPExcel->getActiveSheet()->setCellValueExplicit($columsExcel[7] . $row, ($value['brand_name']), PHPExcel_Cell_DataType::TYPE_STRING);
                $objPHPExcel->getActiveSheet()->setCellValueExplicit($columsExcel[8] . $row, ($value['client_code']), PHPExcel_Cell_DataType::TYPE_STRING);
                $objPHPExcel->getActiveSheet()->setCellValueExplicit($columsExcel[9] . $row, ($value['client_name']), PHPExcel_Cell_DataType::TYPE_STRING);
                $objPHPExcel->getActiveSheet()->setCellValueExplicit($columsExcel[10] . $row, ($value['link']), PHPExcel_Cell_DataType::TYPE_STRING);
                ...
                if ($value['image'] != '' && file_exists($images)) {
                    ...
                    $objDrawing1->setCoordinates($columsExcel[11] . $row);
                }
                $objPHPExcel->getActiveSheet()->getRowDimension($row)->setRowHeight(60);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[11] . $row, '')->getStyle($columsExcel[11] . $row)->applyFromArray($BStylenumber)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);


                if (!empty($value['barcode'])) {
                    $code = $value['barcode'];
                } else {
                    $code = 'purchase_order_request||' . $value['id'];
                    $this->db->where('id', $value['id']);
                    $this->db->update('tblpurchase_order_request', ['barcode' => $code]);
                }
                $qr = vn_to_str(str_replace('||', '__', $code));
                $folder = FCPATH . 'uploads/purchase_order_request/';
                ...
                $colStt = 13;
                foreach (getListColumTable() as $kk => $vv) {
                    $_data = getDataModeration($value['idMain'],$vv['id'],'tblpurchase_order_request','',true);
            ...
            $filename = lang('Phieu_dieu_do_cong_viec_don_dat_hang') . '.xls';
            ob_start();
