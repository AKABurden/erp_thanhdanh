...
class Suggest_pccc extends AdminController
{
    public function __construct()
    {
        ...
        $this->preViewSuggestCheck = has_permission('suggest_pccc', '', 'view');
        $this->preViewOwnSuggestCheck = has_permission('suggest_pccc', '', 'view_own');
        $this->preAddSuggestCheck = has_permission('suggest_pccc', '', 'create');;
        $this->preEditSuggestCheck = has_permission('suggest_pccc', '', 'edit');;
        $this->preApproveSuggestCheck = true;
        $this->preDeleteSuggestCheck = has_permission('suggest_pccc', '', 'delete');
    }
    ...
    public function index()
    {
        ...
        $data['title'] = _l('dt_suggest_pccc');
        $this->load->view('admin/suggest_pccc/index', $data);
    }
    ...
    public function getSuggestChecks()
    {
        ...
        $join = [
            'LEFT JOIN tbl_category_recommended ON tbl_category_recommended.name_table = "tbl_suggest_check" and tbl_category_recommended.ballot_type = 1'
        ];
        ...
        array_push($where, "AND tbl_suggest_check.ballot_type = 1");

        $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
            '(SELECT count(tbltasks.id) FROM tbltasks  LEFT JOIN tbl_category_recommended ON tbl_category_recommended.id = tbltasks.category_recommended_id  WHERE suggest_id = tbl_suggest_check.id AND tbl_category_recommended.name_table="tbl_suggest_check" and tbl_category_recommended.ballot_type = 1) as countTask',
        ...
        foreach ($rResult as $key => $aRow) {
            ...
            $suggest_pccc_item = $this->db->get_where('tbl_suggest_check_item')->result_array();
            //			print_arrays($suggest_pccc_item);
            $view_machines = '';
            if (!empty($suggest_pccc_item)) {
                foreach ($suggest_pccc_item as $k => $v) {
            ...
            $row[] = '<div class="text-left"><a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/suggest_pccc/view/' . $aRow['id']) . '" data-toggle="modal" data-target="#myModal">' . $aRow['reference_no'] . '</a></div>';
            ...
            $view = '<a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/suggest_pccc/view/' . $aRow['id']) . '" data-toggle="modal" data-target="#myModal"><i class="fa fa-file-text-o"></i> ' . lang('view') . ' ' . lang('phiếu') . '</a>';

            $edit = $this->preEditSuggestCheck ? '<a class="tnh-modal" href="' . base_url('admin/suggest_pccc/detail/' . $aRow['id']) . '"><i class="fa fa-edit"></i> ' . lang('edit') . ' ' . lang('phiếu') . '</a>' : '';
            ...
            $delete = $this->preDeleteSuggestCheck ? '<a type="button" class="po" data-container="body" data-html="true" data-toggle="popover" data-placement="left" data-content="
                <button href=\'' . base_url('admin/suggest_pccc/delete/' . $aRow['id']) . '\' class=\'btn btn-danger po-delete-json\'>' . lang('delete') . '</button>
    ...
    public function detail($id = 0)
    {
        ...
        if ($this->input->post()) {
            ...
            if (empty($id)) {
                if ($this->form_validation->run() == true) {
                    $reference_no = getReference('suggest_pccc');
                    ...
                    if (!empty($success)) {
                        ...
                        if (getReference('suggest_pccc') == $reference_no) {
                            updateReference('suggest_pccc');
                        }
                        if (!empty($items)) {
                        ...
                        // $attachments = $this->db->get_where('tblfiles', ['rel_id' => $id, 'rel_type' => 'suggest_pccc'])->result_array();
                        ...
                        //     $suggest_pccc_item = $this->db->get('tbl_suggest_check_item')->result_array();
                        //     if (!empty($suggest_pccc_item)) {
                        //         $listAssigned = [];
                        //         $listFollowers = [];
                        //         foreach ($suggest_pccc_item as $key => $value) {
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    if (!empty($counter)) {
                        foreach ($counter as $key => $value) {
                            ...
                            $suggest_pccc_item_id = !empty($this->input->post('suggest_pccc_item_id')[$value]) ? $this->input->post('suggest_pccc_item_id')[$value] : 0;
                            $current_status = $this->input->post('current_status')[$value];
                            $date_check = !empty($this->input->post('date_check')[$value]) ? to_sql_date($this->input->post('date_check')[$value]) : null;
                            $staff_check = $this->input->post('staff_check')[$value];
                            $result_id = $this->input->post('result_id')[$value];
            ...
            if (empty($id)) {
                ...
                $data['title'] = lang('dt_add_suggest_pccc');
            } else {
                ...
                $data['title'] = lang('dt_edit_suggest_pccc');
            }
        }
        $data['employees'] = $this->manufactures_model->getAllStaff();
        $data['id'] = $id;
        $data['reference_no'] = getReference('suggest_pccc');
        ...
        $this->load->view('admin/suggest_pccc/detail', $data);
    }
    ...
    public function view($id)
    {
        ...
        $data = [];
        $data['title'] = lang('dt_view_suggest_pccc');
        ...
        $data['dtData'] = $dtData;
        $data['dtItems'] = $dtItems;
        $this->load->view('admin/suggest_pccc/view', $data);
    }
    ...
    public function search_suggest_repalce($string_id = '')
    {
        ...
        $this->db->where('EXISTS (SELECT 1 FROM tbl_machines_5s WHERE tbl_machines_5s.machines_id = tbl_machines.id AND tbl_machines_5s.ballot_type = 1 LIMIT 1)', false, false);
        $this->db->limit($limit);
        ...
        $this->db->where('tbl_cleaning.ballot_type', 1);
    ...
    public function getMaintenaceMachinesDetail()
    {
        ...
        if ($type == 'cleaning') {
            ...
            $this->db->where('tbl_machines_5s.ballot_type', 1);
    ...
    public function delete($id)
    {
        ...
        $ktTask = $this->db->get_where('tbltasks', ['rel_id' => $id, 'rel_type' => 'suggest_pccc'])->row();
        if (!empty($ktTask)) {
            $data['result'] = 0;
    ...
    public function exportExcel()
    {
        if ($this->input->post('export_excel')) {
            ...
            $this->db->where("tbl_suggest_check.ballot_type", 1);
