...
class Dashboard_debt_purchase extends AdminController
{
    public function __construct()
    {
        parent::__construct();
        $this->perviewPurchase_invoice = has_permission('purchase_invoice', '', 'view');
        $this->perViewownPurchase_invoice = has_permission('purchase_invoice', '', 'view_own');
        $this->perviewPay_slip = has_permission('pay_slip', '', 'view');
        $this->perViewownPay_slip = has_permission('pay_slip', '', 'view_own');
        $this->perviewDebt_suppliers = has_permission('debt_suppliers', '', 'view');
        $this->perViewownDebt_suppliers = has_permission('debt_suppliers', '', 'view_own');
    ...
    public function index()
    {
        ...
        $this->load->view('admin/dashboard_debt_purchase/manage', $data);
        // SUM==
    ...
    public function count_all()
    {
        ...
        if(!$this->perviewPurchase_invoice) {
            if($arrIDStaff != array()) {
                $coverStr = implode(",", $arrIDStaff);
                $whereJoin_purchase_invoice['where'][] = 'tblpurchase_invoice.staff_create = '.$coverStr;
            }
        }
        $whereJoin_purchase_invoice['join']=array();
        $whereJoin_purchase_invoice['field']='id_import';
        $invoice = (sum_from_table_join('tblpurchase_invoice',$whereJoin_purchase_invoice));
        $this->db->select('count(*) as count');

        $count_purchase_invoice = $this->db->get('tblpurchase_invoice')->row();
        $count_purchase_invoices = 0 ;
        if(!empty($count_purchase_invoice))
        {
            $count_purchase_invoices = $count_purchase_invoice->count;
        }

// the end where foin tblpurchase_invoice

            $whereJoin_pay_slip=array();
            $whereJoin_pay_slip['join']=array();
            $whereJoin_pay_slip['field']='total';
            $pay_slip = (sum_from_table_join('tblpay_slip',$whereJoin_pay_slip));
            $this->db->select('count(*) as count');

            if(!$this->perviewPay_slip) {
                if($arrIDStaff != array()) {
                    $coverStr = implode(",", $arrIDStaff);
                    $this->db->where('tblpay_slip.staff_id IN ('.$coverStr.')');
                }
            }
            // else if (!$this->perViewownPay_slip){
            //     if($id_branch != 'main') {
            //         $this->db->where('tblpay_slip.id_branch = '.$id_branch);
            //     }
            // }
            $count_pay_slip = $this->db->get('tblpay_slip')->row();
            $count_pay_slips = 0 ;
            if(!empty($count_pay_slip))
            {
                $count_pay_slips = $count_pay_slip->count;
            }
// the end where join pay_slips

$whereJoin['join']=array();
if(!$this->perviewDebt_suppliers) {
    if($arrIDStaff != array()) {
        $coverStr = implode(",", $arrIDStaff);
        $whereJoin['where'] []=  'tblpurchase_order.staff_create IN ('.$coverStr.')';
    }
}
$whereJoin['field']='totalAll_suppliers';
$totalAll_suppliers_order = (sum_from_table_join('tblpurchase_order',$whereJoin));
$whereJoin1['join']=array();
// if (!$this->perViewownDebt_suppliers){
//     if($id_branch != 'main') {
//         $whereJoin_coupon_invoice['where'][] = 'tblsuppliers.id_branch = '.$id_branch;
//     }
// }
$whereJoin1['field']='COALESCE((SELECT SUM(tbl_outsource.grand_total) FROM tbl_outsource WHERE tbl_outsource.supplier_id=tblsuppliers.id AND tbl_outsource.status_pay < 2 ),0)';
$total_outsource = (sum_from_table_join_v2('tblsuppliers',$whereJoin1));
...
$whereJoin2['field']='amount_paid';
$amount_paid_order = (sum_from_table_join('tblpurchase_order',$whereJoin2));
...
$whereJoin3['field']='price_other_expenses';
$amount_paid = (sum_from_table_join('tblpurchase_order',$whereJoin3));
$whereJoin4['join']=array();
if(!$this->perviewDebt_suppliers) {
    if($arrIDStaff != array()) {
        $coverStr = implode(",", $arrIDStaff);
        $whereJoin['where'] []=  'tblpurchase_invoice.staff_create IN ('.$coverStr.')';
    }
}
$whereJoin4['field']='amount_paid';
$amount_paid_invoice = (sum_from_table_join('tblpurchase_invoice',$whereJoin4));

$debt_suppliers = number_format($totalAll_suppliers_order + $total_outsource - $amount_paid_order + $amount_paid + $amount_paid_invoice);
if ($debt_suppliers < 0){
    $debt_suppliers = 0;
}
// the end where join debt_suppliers
            $data['count_purchase_invoices'] = number_format($count_purchase_invoices).' đơn';
            $data['pay_slips'] = number_format($count_pay_slips).' đơn';
            $data['total_imports'] = $debt_suppliers.' VNĐ';
    ...
    // '.(strlen($name) > 30 ? ' ...' : "").'
                $html.='<div class="wrap_container">
                            <span style="float:left; width:70%; height: 24px; overflow: hidden;"><span class="wrap_number">'.($key + 1).'.</span> '.($name).'</span>
...
// '.(strlen($name) > 44 ? ' ...' : "").'
            $html.='<div class="wrap_container">
                        <span style="float:left; width:70%; height: 24px; overflow: hidden;"><span class="number_top_'.($key + 1).'">Top '.($key + 1).': </span> '.($name).'</span>
    ...
    public function top_supplier_imports()
    {
        ...
        $this->db->order_by('totalcurrency','DESC');
        $this->db->limit(5);
        $data_quantity = $this->db->get('tblpurchase_order')->result_array();
        $html ='';
        foreach ($data_quantity as $key => $value) {
            $total = $value['totalcurrency'];
            $name = $value['name'];
