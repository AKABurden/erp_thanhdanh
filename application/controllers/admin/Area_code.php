...
class Area_code extends AdminController
{
    ...
    function index()
    {
        ...
        $data['type'] = 1;
        $this->load->view('admin/area_code/manage', $data);
    }
    function getAreaCode()
    {
        $aColumns = [
            'tbl_area_code.id as id',
            'tbl_area_code.code as code',
            'tbl_area_code.name as name',
            '"" as actions'
        ];
        $type_search = $this->input->post('type_search');

        $sIndexColumn = 'id';
        $sTable       = 'tbl_area_code';
        $where        = [];
        $filter = [];
        ...
        foreach ($rResult as $key => $aRow) {
            ...
            $row = [];
            $id = $aRow['id'];
            $edit = '<a class="tnh-modal" href="' . base_url('admin/area_code/handling/' . $id) . '"><i class="fa fa-edit"></i> ' . lang('tnh_edit') . '</a>';
            $delete = '<a type="button" class="po" data-container="body" data-html="true" data-toggle="popover" data-placement="left" data-content="
                <button href=\'' . base_url('admin/area_code/delete/' . $id) . '\' class=\'btn btn-danger po-delete-json\'>' . lang('delete') . '</button>
    ...
    function handling($id = '', $type = 1)
    {
        ...
        $data = [];
        $area_code = $id ? $this->area_code_model->getAreaCodeById($id) : [];
        if ($this->input->post()) {
            if (!empty($id)) {
                if ($area_code['code'] != $this->input->post('code')) {
                    $this->form_validation->set_rules('code', lang("Mã quy định"), 'trim|required|is_unique[tbl_area_code.code]');
                }
            } else {
                $this->form_validation->set_rules('code', lang("Mã quy định"), 'trim|required|is_unique[tbl_area_code.code]');
            }
            ...
            if ($this->form_validation->run() == true) {
                ...
                if ($id) {
                    ...
                    $ins = $this->area_code_model->updateAreaCode($id, $option);
                    $standard_id = $id;
                } else {
                    $option['staff_create'] = get_staff_user_id();
                    $option['date_create'] = date('Y-m-d H:i:s');
                    $ins = $this->area_code_model->insertAreaCode($option);
                    $standard_id = $ins;
                }
        ...
        $data['id'] = $id;
        $data['type'] = $type;
        $data['dtData'] = $area_code;
        $title = '';
        $title = $id ? lang('Sửa Danh Sách Nhóm Khu Vực') : lang('Thêm Danh Sách Nhóm Khu Vực');
        $data['title'] = $title;
        $this->load->view('admin/area_code/handling', $data);
    }
    public function delete($id)
    {
        $data = [];
        if ($this->area_code_model->deleteAreaCode($id)) {
            $data['result'] = 1;
            $data['message'] = lang('success');
        } else {
            $data['result'] = 0;
    ...
    function modal_excel($type = 0)
    {
        ...
        $data['title'] = $title;
        $data['type'] = $type;
        $this->load->view('admin/area_code/import_excel', $data);
    }
        ...
        if (!empty($data)) {
            foreach ($data as $key => $value) {
                ...
                if ($checkerrors == 0) {
                    ...
                    $check_id = $this->db->get('tbl_area_code')->row_array();
                    $id = '';
                    if (!empty($check_id)) {
                        $id = $check_id['id'];
                    }
