...
class Suggest_ticket_purchase_products extends AdminController
{
    public function __construct()
    {
        ...
        $this->preViewSuggestTicketPurchaseProduct = true;
        $this->preViewOwnSuggestTicketPurchaseProduct = true;
        $this->preAddSuggestTicketPurchaseProduct = true;
        $this->preEditSuggestTicketPurchaseProduct = true;
        $this->preApproveSuggestTicketPurchaseProduct = true;
        $this->preDeleteSuggestTicketPurchaseProduct = true;
    }

    public function index(){
        if (!$this->preViewSuggestTicketPurchaseProduct && !$this->preViewOwnSuggestTicketPurchaseProduct){
            access_denied();
        }
        $data['title'] = _l('dt_suggest_ticket_purchase_products');
        $this->load->view('admin/suggest_ticket_purchase_products/index', $data);
    }

    public function getSuggestTicketPurchaseProducts(){
        ...
        $aColumns = [
            'tbl_suggest_purchase_product.id as id',
            'tbl_suggest_purchase_product.reference_no as reference_no',
            'tbl_suggest_purchase_product.date as date',
            ...
            'tbl_suggest_purchase_product.date_import as date_import',
            'tbl_suggest_purchase_product.employee_id as employee_id',
            'tbl_suggest_purchase_product.created_by as created_by',
            '1',
        ];
        $sIndexColumn = 'id';
        $sTable = 'tbl_suggest_purchase_product';
        $where = [
        ...
        $join = [
            'INNER JOIN tbl_productions_orders_details ON tbl_productions_orders_details.id = tbl_suggest_purchase_product.pod_id',
            ...
            'LEFT JOIN tbl_category_recommended ON tbl_category_recommended.name_table = "tbl_suggest_purchase_product"'
        ];

        if (!$this->preViewSuggestTicketPurchaseProduct) {
            array_push($where,'AND tbl_suggest_purchase_product.created_by =', get_staff_user_id());
        }

        if (!empty($start_date_search)) {
            $start_date_search = to_sql_date($start_date_search).' 00:00:00';
            array_push($where, "AND tbl_suggest_purchase_product.date >= '" . $start_date_search . "'");
        }

        if (!empty($end_date_search)) {
            $end_date_search = to_sql_date($end_date_search).' 23:59:59';
            array_push($where, "AND tbl_suggest_purchase_product.date <= '" . $end_date_search . "'");
        }


        $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
            'tbl_suggest_purchase_product.status',
            'tbl_suggest_purchase_product.date_status',
            'tbl_suggest_purchase_product.staff_status',
            '(SELECT count(tbltasks.id) FROM tbltasks  LEFT JOIN tbl_category_recommended ON tbl_category_recommended.id = tbltasks.category_recommended_id  WHERE suggest_id = tbl_suggest_purchase_product.id AND tbl_category_recommended.name_table="tbl_suggest_purchase_product") as countTask',
        ...
        foreach ($rResult as $key => $aRow) {
            ...
            $row[] = '<div class="text-left" style="width: 120px"><a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/suggest_ticket_purchase_products/view/' . $aRow['id']) . '" data-toggle="modal" data-target="#myModal">' . $aRow['reference_no'] . '</a></div>';
            ...
            $view = '<a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/suggest_ticket_purchase_products/view/' . $aRow['id']) . '" data-toggle="modal" data-target="#myModal"><i class="fa fa-file-text-o"></i> ' . lang('view') . ' ' . lang('phiếu') . '</a>';

            $edit = $this->preEditSuggestTicketPurchaseProduct ? '<a href="' . base_url('admin/suggest_ticket_purchase_products/detail/' . $aRow['id']) . '"><i class="fa fa-edit"></i> ' . lang('edit') . ' ' . lang('phiếu') . '</a>' : '';

            $delete = $this->preDeleteSuggestTicketPurchaseProduct ? '<a type="button" class="po" data-container="body" data-html="true" data-toggle="popover" data-placement="left" data-content="
                <button href=\'' . base_url('admin/suggest_ticket_purchase_products/delete/' . $aRow['id']) . '\' class=\'btn btn-danger po-delete-json\'>' . lang('delete') . '</button>
    ...
    public function detail($id = 0){
        ...
        if ($this->input->post()){
            if (empty($id)){
                $this->form_validation->set_rules('reference_no', lang("Mã Phiếu"), 'required|is_unique[tbl_suggest_purchase_product.reference_no]');
                ...
                if ($this->form_validation->run() == true) {
                    $reference_no = getReference('suggest_ticket_purchase_product');
                    ...
                    $this->db->insert('tbl_suggest_purchase_product',$fields);
                    $id = $this->db->insert_id();
                    if ($id){
                        if (getReference('suggest_ticket_purchase_product') == $reference_no) {
                            updateReference('suggest_ticket_purchase_product');
                        }

                        foreach ($items as $key => $value) {
                            $value['suggest_purchase_product_id'] = $id;
                            $this->db->insert('tbl_suggest_purchase_product_item',$value);
                        }
                        insertActivityLog([
                            'type_parent_obj' => 'suggest_ticket_purchase_product',
                            'table_obj' => 'tbl_suggest_purchase_product',
                            'id_obj' => $id,
                ...
                $this->db->select('tbl_suggest_purchase_product.*');
                $this->db->from('tbl_suggest_purchase_product');
                $this->db->where('tbl_suggest_purchase_product.id',$id);
                $dtSuggestPurchase = $this->db->get()->row_array();
                if ($dtSuggestPurchase['reference_no'] != $this->input->post('reference_no')) {
                    $this->form_validation->set_rules('reference_no', lang("Mã Phiếu"), 'trim|required|is_unique[tbl_suggest_purchase_product.reference_no]');
                }
                $this->form_validation->set_rules('date', lang("date"), 'required');
                $this->form_validation->set_rules('date_import', lang("Ngày Nhập"), 'required');
                $this->form_validation->set_rules('branch_id', lang("Chi nhánh"), 'required');
                if ($this->form_validation->run() == true) {
                    ...
                    $this->db->where('id',$id);
                    $success = $this->db->update('tbl_suggest_purchase_product',$fields);
                    if ($success){
                        $this->db->where('suggest_purchase_product_id',$id);
                        $this->db->delete('tbl_suggest_purchase_product_item');
                        foreach ($items as $key => $value) {
                            $value['suggest_purchase_product_id'] = $id;
                            $this->db->insert('tbl_suggest_purchase_product_item',$value);
                        }
            ...
            if (empty($id)){
                if (!$this->preAddSuggestTicketPurchaseProduct){
                    accessDenied(true);
                }
                $data['title'] = lang('dt_add_suggest_ticket_purchase_products');
                $data['breadcrumb'] = [array('link' => base_url('admin/suggest_ticket_purchase_products'), 'page' => lang('dt_suggest_ticket_purchase_products')), array('link' => '#', 'page' => lang('dt_add_suggest_ticket_purchase_products'))];
            } else {
                if (!$this->preEditSuggestTicketPurchaseProduct){
                    accessDenied(true);
                }
                $this->db->select('tbl_suggest_purchase_product.*');
                $this->db->from('tbl_suggest_purchase_product');
                ...
                $this->db->select('tbl_suggest_purchase_product_item.*');
                $this->db->from('tbl_suggest_purchase_product_item');
                $this->db->where('tbl_suggest_purchase_product_item.suggest_purchase_product_id',$id);
                ...
                $data['title'] = lang('dt_edit_suggest_ticket_purchase_products');
                $data['breadcrumb'] = [array('link' => base_url('admin/suggest_ticket_purchase_products'), 'page' => lang('dt_suggest_ticket_purchase_products')), array('link' => '#', 'page' => lang('dt_edit_suggest_ticket_purchase_products'))];
            }
        }
        $data['employees'] = $this->manufactures_model->getAllStaff();
        $data['id'] = $id;
        $data['reference_no'] = getReference('suggest_ticket_purchase_product');;
        $this->load->view('admin/suggest_ticket_purchase_products/detail',$data);
    }

    public function view($id){
        $data = [];
        $data['title'] = lang('dt_view_suggest_ticket_purchase_products');
        ...
        $data['dtSuggestPurchase'] = $dtSuggestPurchase;
        $data['dtSuggestPurchaseItems'] = $dtSuggestPurchaseItems;
        $this->load->view('admin/suggest_ticket_purchase_products/view',$data);
    }

    public function agree(){
        if (!$this->preApproveSuggestTicketPurchaseProduct) {
            $data['result'] = 0;
        ...
        $this->db->select('tbl_suggest_purchase_product.*');
        $this->db->from('tbl_suggest_purchase_product');
        $this->db->where('tbl_suggest_purchase_product.id',$suggest_purchase_id);
        $dtSuggestPurchase = $this->db->get()->row_array();
        if (empty($dtSuggestPurchase)) {
            ...
            $up = $this->db->update('tbl_suggest_purchase_product',$options);
            if ($up) {
    ...
    public function delete($id){
        if (!$this->preDeleteSuggestTicketPurchaseProduct){
            $data['result'] = 0;
            $data['message'] = lang('access_denied');
            echo json_encode($data); die();
        }
        ...
        $this->db->where('id',$id);
        $success = $this->db->delete('tbl_suggest_purchase_product');
        if ($success){
            $this->db->where('tbl_suggest_purchase_product_item.suggest_purchase_product_id',$id);
            $this->db->delete('tbl_suggest_purchase_product_item');
    ...
    public function exportExcel(){
        if ($this->input->post('export_excel')) {
            ...
            $this->db->select('
                ...
                tbl_suggest_purchase_product_item.quantity as quantity,
                tbl_products.quantity_sheet_bale as quantity_sheet_bale,
                tbl_suggest_purchase_product_item.quantity_kien as quantity_kien,
                tbl_suggest_purchase_product_item.quantity_kg as quantity_kg,
                tbl_suggest_purchase_product.date_import as date_import,
                tbl_products.time_stock as time_stock,
                tbl_suggest_purchase_product.employee_id as employee_id,
                tbl_suggest_purchase_product_item.standard as standard,
                tbl_products.images as images
            ');
            $this->db->from('tbl_suggest_purchase_product');
            $this->db->join('tbl_productions_orders_details', 'tbl_productions_orders_details.id = tbl_suggest_purchase_product.pod_id', 'left');
            $this->db->join('tbl_suggest_purchase_product_item','tbl_suggest_purchase_product_item.suggest_purchase_product_id = tbl_suggest_purchase_product.id');
            $this->db->join('tbl_products','tbl_products.id = tbl_suggest_purchase_product_item.item_id');
            ...
            $this->db->join($tbOrders, 'tb_orders.object_id = tbl_suggest_purchase_product.object_id AND tbl_suggest_purchase_product.object_type = "orders"', 'left');
            $this->db->join($tbBusinessPlan, 'tb_business_plan.object_id = tbl_suggest_purchase_product.object_id AND tbl_suggest_purchase_product.object_type = "business_plan"', 'left');


            if (!$this->preViewSuggestTicketPurchaseProduct) {
                $this->db->where('tbl_suggest_purchase_product.created_by = '.get_staff_user_id().'');
            }

            if (!empty($start_date_search)) {
                $start_date_search = to_sql_date($start_date_search) . ' 00:00:00';
                $this->db->where("tbl_suggest_purchase_product.date >= '" . $start_date_search . "'");
            }

            if (!empty($end_date_search)) {
                $end_date_search = to_sql_date($end_date_search) . ' 23:59:59';
                $this->db->where("tbl_suggest_purchase_product.date <= '" . $end_date_search . "'");
            }

            $this->db->order_by('tbl_suggest_purchase_product.id desc');
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('A1',
                ('PHIẾU YÊU CẦU NHẬP KHO THÀNH PHẨM'))->getStyle("A1")->applyFromArray([
                'font' => array(
                    'bold' => true,
                    'size' => 18,
                ),
            ...
            $filename = lang('phieu_yeu_cau_nhap_kho_tp') . '.xls';
