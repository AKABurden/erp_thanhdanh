...
class Suggest_plan_recruitment extends AdminController
{
    public function __construct()
    {
        ...
        $this->preViewSuggestPlanRecruitment = true;
        $this->preViewOwnSuggestPlanRecruitment = true;
        $this->preAddSuggestPlanRecruitment = true;
        $this->preEditSuggestPlanRecruitment = true;
        $this->preApproveSuggestPlanRecruitment = true;
        $this->preDeleteSuggestPlanRecruitment= true;
    }

    public function index()
    {
        if (!$this->preViewSuggestPlanRecruitment && !$this->preViewOwnSuggestPlanRecruitment) {
            access_denied();
        }
        $data['title'] = _l('dt_suggest_plan_recruitment');
        $this->load->view('admin/suggest_plan_recruitment/index', $data);
    }

    public function getSuggestPlanRecruitment()
    {
        ...
        $aColumns = [
            ...
            'tbl_suggest_plan_recruitment.status as status',
        ];
        $sIndexColumn = 'id';
        $sTable = 'tbl_suggest_plan_recruitment';
        $where = [
        ...
        $join = [
            'INNER JOIN tbl_category_plan_time ON tbl_category_plan_time.id = tbl_suggest_plan_recruitment.category_plan',
        ];

        if (!$this->preViewSuggestPlanRecruitment) {
            array_push($where, 'AND (tbl_suggest_plan_recruitment.created_by = '.get_staff_user_id().' OR tbl_suggest_plan_recruitment.staff_id = '.get_staff_user_id().' )');
        }

        if (!empty($start_date_search)) {
            $start_date_search = to_sql_date($start_date_search) . ' 00:00:00';
            array_push($where, "AND tbl_suggest_plan_recruitment.date >= '" . $start_date_search . "'");
        }

        if (!empty($end_date_search)) {
            $end_date_search = to_sql_date($end_date_search) . ' 23:59:59';
            array_push($where, "AND tbl_suggest_plan_recruitment.date <= '" . $end_date_search . "'");
        }


        $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
            'tbl_suggest_plan_recruitment.date_status',
            'tbl_suggest_plan_recruitment.staff_status'
        ], '', []);
        ...
        foreach ($rResult as $key => $aRow) {
            ...
            $row[] = '<div class="text-left"><a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/suggest_plan_recruitment/view/' . $aRow['id']) . '" data-toggle="modal" data-target="#myModal">' . $aRow['reference_no'] . '</a></div>';
            $row[] = '<div class="text-left">' . _dt($aRow['date']) . '</div>';
            $row[] = '<div class="text-left">' . get_staff_full_name($aRow['staff_id']) . '</div>';
            $row[] = '<div class="text-left">' . ($aRow['name_category_plan']) . '</div>';
            ...
            $row[] = '<div class="text-center">' . $gender . '</div>';
            ...
            $view = '<a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/suggest_plan_recruitment/view/' . $aRow['id']) . '" data-toggle="modal" data-target="#myModal"><i class="fa fa-file-text-o"></i> ' . lang('view') . ' ' . lang('phiếu') . '</a>';

            $edit = $this->preEditSuggestPlanRecruitment ? '<a class="tnh-modal" href="' . base_url('admin/suggest_plan_recruitment/detail/' . $aRow['id']) . '"><i class="fa fa-edit"></i> ' . lang('edit') . ' ' . lang('phiếu') . '</a>' : '';

            $delete = $this->preDeleteSuggestPlanRecruitment ? '<a type="button" class="po" data-container="body" data-html="true" data-toggle="popover" data-placement="left" data-content="
                <button href=\'' . base_url('admin/suggest_plan_recruitment/delete/' . $aRow['id']) . '\' class=\'btn btn-danger po-delete-json\'>' . lang('delete') . '</button>
    ...
    public function detail($id = 0){
        $data = [];
        $this->db->select('tbl_suggest_plan_recruitment.*');
        $this->db->from('tbl_suggest_plan_recruitment');
        $this->db->where('tbl_suggest_plan_recruitment.id',$id);
        $dtData = $this->db->get()->row_array();
        if ($this->input->post()){
            if (!empty($id)){
                if ($dtData['reference_no'] != $this->input->post('reference_no')) {
                    $this->form_validation->set_rules('reference_no', lang("Mã Phiếu"), 'trim|required|is_unique[tbl_suggest_plan_recruitment.reference_no]');
                }
            } else {
                $this->form_validation->set_rules('reference_no', lang("Mã Phiếu"), 'required|is_unique[tbl_suggest_plan_recruitment.reference_no]');
            }
            ...
            if (empty($id)){
                if ($this->form_validation->run() == true) {
                    $reference_no = getReference('suggest_plan_recruitment');
                    ...
                    $this->db->insert('tbl_suggest_plan_recruitment',$fields);
                    $id = $this->db->insert_id();
                    if ($id){
                        if (getReference('suggest_plan_recruitment') == $reference_no) {
                            updateReference('suggest_plan_recruitment');
                        }
                        insertActivityLog([
                            'type_parent_obj' => 'suggest_plan_recruitment',
                            'table_obj' => 'tbl_suggest_plan_recruitment',
                            'id_obj' => $id,
                            'name_obj' => $reference_no,
                            'content' => lang('Thêm mới yêu cầu kế hoạch tuyển dụng') . ' [' . $reference_no . ']',
                            'actions' => 'add'
                        ]);
                        $data['result'] = 1;
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    $this->db->where('id',$id);
                    $success = $this->db->update('tbl_suggest_plan_recruitment',$fields);
                    if ($success){
                        insertActivityLog([
                            'type_parent_obj' => 'suggest_plan_recruitment',
                            'table_obj' => 'tbl_suggest_plan_recruitment',
                            ...
                            'content' => lang('Sửa phiếu yêu cầu kế hoạch tuyển dụng') . ' [' . $dtData['reference_no'] . ']',
                            'actions' => 'edit'
                        ]);
                        $data['result'] = 1;
            ...
            if (empty($id)){
                if (!$this->preAddSuggestPlanRecruitment){
                    accessDenied(true);
                }
                $data['title'] = lang('dt_add_suggest_plan_recruitment');
            } else {
                if (!$this->preEditSuggestPlanRecruitment){
                    accessDenied(true);
                }
                $this->db->select('tbl_suggest_plan_recruitment.*');
                $this->db->from('tbl_suggest_plan_recruitment');
                ...
                $data['dtData'] = $dtData;
                $data['title'] = lang('dt_edit_suggest_plan_recruitment');
            }
        }
        $data['employees'] = $this->manufactures_model->getAllStaff();
        $data['id'] = $id;
        $data['reference_no'] = getReference('suggest_plan_recruitment');
        $data['dtCategoryPlanTime'] = get_table_where('tbl_category_plan_time');
        $this->load->view('admin/suggest_plan_recruitment/detail',$data);
    }

    public function view($id){
        $data = [];
        $data['title'] = lang('dt_view_suggest_plan_recruitment');

        $this->db->select('tbl_suggest_plan_recruitment.*,
            tbl_category_plan_time.name as name_category_plan
        ');
        $this->db->from('tbl_suggest_plan_recruitment');
        $this->db->join('tbl_category_plan_time','tbl_category_plan_time.id = tbl_suggest_plan_recruitment.category_plan');
        $this->db->where('tbl_suggest_plan_recruitment.id',$id);
        $dtData = $this->db->get()->row_array();

        $data['dtData'] = $dtData;
        $this->load->view('admin/suggest_plan_recruitment/view',$data);
    }

    public function agree(){
        if (!$this->preApproveSuggestPlanRecruitment) {
            $data['result'] = 0;
        ...
        $this->db->select('tbl_suggest_plan_recruitment.*');
        $this->db->from('tbl_suggest_plan_recruitment');
        $this->db->where('tbl_suggest_plan_recruitment.id',$suggest_id);
        $dtData = $this->db->get()->row_array();
        if (empty($dtData)) {
            $data['result'] = 0;
            ...
            $up = $this->db->update('tbl_suggest_plan_recruitment',$options);
            if ($up) {
                ...
                insertActivityLog([
                    ...
                    'id_obj' => $suggest_id,
                    'name_obj' => $dtData['reference_no'],
                    'content' => lang('Duyệt phiếu yêu cầu kế hoạch tuyển dụng') . ' [' . $dtData['reference_no'] . ']',
                    'actions' => 'approved'
                ]);
                $data['result'] = 1;
    ...
    public function delete($id){
        if (!$this->preDeleteSuggestPlanRecruitment){
            $data['result'] = 0;
            $data['message'] = lang('access_denied');
            echo json_encode($data); die();
        }
        ...
        $this->db->where('id',$id);
        $success = $this->db->delete('tbl_suggest_plan_recruitment');
        if ($success){
            ...
            insertActivityLog([
                ...
                'id_obj' => $id,
                'name_obj' => $dtData['reference_no'],
                'content' => lang('Xóa phiếu yêu cầu kế hoạch tuyển dụng') . ' [' . $dtData['reference_no'] . ']',
                'actions' => 'delete'
            ]);
            $data['result'] = 1;
