...
class Request_calibration extends AdminController
{
    public function __construct()
    {
        ...
        $this->preViewRequestCalibration = true;
        $this->preViewOwnRequestCalibration = true;
        $this->preAddRequestCalibration = true;
        $this->preEditRequestCalibration = true;
        $this->preApproveRequestCalibration = true;
        $this->preDeleteRequestCalibration = true;
    }

    public function index()
    {
        if (!$this->preViewRequestCalibration && !$this->preViewOwnRequestCalibration) {
            access_denied();
        }
        $data['title'] = _l('ch_request_calibration');
        $this->load->view('admin/request_calibration/index', $data);
    }

    public function getRequestCalibration()
    {
        ...
        $aColumns = [
            ...
            'tbl_request_calibration.procedure as procedures',
            ...
            'tbl_request_calibration.status as status',
            'tbl_request_calibration.status_finish as status_finish',
            'tbl_result.name as name_result',
            'tblbranch.name as name_branch',
            'tbl_request_calibration.standard as standard',
            ...
            'tbl_request_calibration.created_by as created_by',
        ];
        $sIndexColumn = 'id';
        ...
        $join = [
            ...
            'LEFT JOIN tblbranch ON tblbranch.id = tbl_request_calibration.branch_id',
        ];

        if (!$this->preViewRequestCalibration) {
        ...
        $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
            'tbl_request_calibration.date_status',
            'tbl_request_calibration.staff_status',
            'tbl_request_calibration.date_status_finish',
            'tbl_request_calibration.staff_status_finish',
        ], '', []);
        ...
        foreach ($rResult as $key => $aRow) {
            ...
            $row[] = '<div class="text-left"><a target="_blank" href="' . base_url('admin/production_report/detail?object_id=' . $aRow['id'] . '&object_type=request_calibration') . '" class="btn btn-info">Tạo phiếu báo cáo không phù hợp</a></div>
                <div style="margin-top: 5px">' . $htmlReport . '</div>
            ';
            $row[] = '<div class="text-left">' . get_staff_full_name($aRow['created_by']) . '</div>';

            $view = '<a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/request_calibration/view/' . $aRow['id']) . '" data-toggle="modal" data-target="#myModal"><i class="fa fa-file-text-o"></i> ' . lang('view') . ' ' . lang('phiếu') . '</a>';

            $edit = $this->preEditRequestCalibration ? '<a class="tnh-modal" href="' . base_url('admin/request_calibration/detail/' . $aRow['id']) . '"><i class="fa fa-edit"></i> ' . lang('edit') . ' ' . lang('phiếu') . '</a>' : '';

            $delete = $this->preDeleteRequestCalibration ? '<a type="button" class="po" data-container="body" data-html="true" data-toggle="popover" data-placement="left" data-content="
    ...
    public function detail($id = 0)
    {
        ...
        if ($this->input->post()) {
            ...
            if (empty($id)) {
                if (!$this->preAddRequestCalibration) {
                    accessDenied(true);
                }
                $data['title'] = lang('ch_add_request_calibration');
            } else {
                if (!$this->preEditRequestCalibration) {
                    accessDenied(true);
                }
                $this->db->select('tbl_request_calibration.*');
                $this->db->from('tbl_request_calibration');
    ...
    {
        if (!$this->preApproveRequestCalibration) {
            $data['result'] = 0;
        ...
        if (empty($dtData)) {
            ...
            if ($up) {
                ...
                insertActivityLog([
                    ...
                    'id_obj' => $suggest_id,
                    'name_obj' => $dtData['reference_no'],
                    'content' => lang('Duyệt hoàn thành phiếu yêu cầu hiệu chuẩn') . ' [' . $dtData['reference_no'] . ']',
    ...
    {
        if (!$this->preDeleteRequestCalibration) {
            $data['result'] = 0;
            $data['message'] = lang('access_denied');
            echo json_encode($data);
            die();
    ...
    public function exportExcel()
    {
        ...
        if ($this->input->post('export_excel')) {
            ...
            $inputFileName = 'uploads/import_ch/Phieu_yeu_cau_hieu_chuan.xlsx';
            ...
            foreach ($items as $key => $value) {
                ...
                $objPHPExcel->getActiveSheet()->setCellValueExplicit($columsExcel[15] . $row, ($value['standard']), PHPExcel_Cell_DataType::TYPE_STRING);
            ...
            $objPHPExcel->getActiveSheet()->getStyle('A4:Q' . $row)->getAlignment()->setWrapText(true);
            $objPHPExcel->getActiveSheet()->getStyle('A4:Q' . $row)->applyFromArray([
                'borders' => array(
                    'allborders' => array(
                        'style' => PHPExcel_Style_Border::BORDER_THIN
                    )
            ...
            $filename = lang('Phieu_yeu_cau_hieu_chuan') . '.xls';
            ob_start();
