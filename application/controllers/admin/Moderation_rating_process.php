...
class Moderation_rating_process extends AdminController
{
    ...
    public function index()
    {
        ...
        $this->load->view('admin/moderation_rating_process/index', $data);
    }
    public function getModerationEvaluate()
    {
        ...
        $aColumns = [
            'tbl_suggest_rating_process_item.id as id',
            ...
            'tbl_suggest_rating_process.note as note',
        ...
        $join = [
            'INNER JOIN tbl_suggest_rating_process ON tbl_suggest_rating_process.id = tbl_suggest_rating_process_item.suggest_rating_process_id',
            'INNER JOIN tblbranch ON tblbranch.id = tbl_suggest_rating_process.branch_id',
            'INNER JOIN tbl_system ON tbl_system.id = tbl_suggest_rating_process_item.system_id'
        ];
        $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, ['tbl_suggest_rating_process.id as idmain'], 'GROUP BY tbl_suggest_rating_process_item.id', [], '', 'HAVING tbl_suggest_rating_process_item.id > 0');
    ...
    public function exportExcel()
    {
        if ($this->input->post('export_excel')) {
            ...
            $this->db->join('tbl_suggest_rating_process', 'tbl_suggest_rating_process.id = tbl_suggest_rating_process_item.suggest_rating_process_id');
            $this->db->join('tblbranch', 'tblbranch.id = tbl_suggest_rating_process.branch_id', 'left');
            ...
            $this->db->where('(tbl_suggest_rating_process.status = 1)');
            if (!empty($type)) {
                $this->db->where('(tbl_suggest_rating_process.object_type = "' . $type . '")');
            }
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('F' . $sttRow . '', 'Mã quy trình')->getStyle("F$sttRow")->getAlignment()->setWrapText(true);
            $objPHPExcel->getActiveSheet()->setCellValue('G' . $sttRow . '', 'Tên quy trình')->getStyle("G$sttRow")->getAlignment()->setWrapText(true);
            $objPHPExcel->getActiveSheet()->setCellValue('H' . $sttRow . '', 'Chi tiết quy trình')->getStyle("H$sttRow")->getAlignment()->setWrapText(true);
