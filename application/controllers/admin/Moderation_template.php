...
class Moderation_template extends AdminController
{
    public function __construct()
    {
        parent::__construct();
        $this->preViewModerationTemplate = true;
        $this->preViewOwnModerationTemplate = true;
        $this->preAddModerationTemplate = true;
        $this->preEditModerationTemplate = true;
        $this->preApproveModerationTemplate = true;
        $this->preDeleteModerationTemplate = true;
    }

    public function index()
    {
        if (!$this->preViewModerationTemplate && !$this->preViewOwnModerationTemplate) {
            access_denied();
        }
        $data['title'] = _l('ch_moderation_template');
        $this->load->view('admin/moderation_template/index', $data);
    }
    public function getModerationTemplate()
    {
        ...
        $aColumns = [
            'tbl_request_template_item.id as id',
            ...
            'tbl_request_template_item.expected_date as expected_date',
            'tbl_request_template_item.start_date as start_date',
            'tbl_request_template_item.end_date as end_date',
            'IF(tbl_products.images IS NOT NULL && tbl_products.images != "", CONCAT("uploads/products/", "", tbl_products.images, ""), "") as image'
        ];
        $sIndexColumn = 'id';
        $sTable = 'tbl_request_template_item';
        $where = [];
        $filter = [];

        $join = [
            'INNER JOIN tbl_request_template ON tbl_request_template.id = tbl_request_template_item.request_template_id',
            'LEFT JOIN tbl_quotes ON tbl_quotes.id = tbl_request_template.id_quotes',
            'LEFT JOIN tbl_products ON tbl_products.id = tbl_request_template_item.items_id AND tbl_request_template_item.type_item = "products"',
            ...
            'LEFT JOIN ' . $tb_customer_group . ' ON tb_customer_group.client_id = tbl_request_template.client_id',
        ];
        if (!$this->preViewModerationTemplate) {
            array_push($where, 'AND (tbl_request_template.create_by = ' . get_staff_user_id() . ')');
        }
        ...
        array_push($where, "AND tbl_request_template.id > 0");

        $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, ['tbl_request_template.id as idmain'], 'GROUP BY tbl_request_template_item.id', [], '', 'HAVING tbl_request_template_item.id > 0');
        ...
        foreach ($rResult as $key => $aRow) {
            ...
            $row[] = '<div class="text-center" style="width: 80px"><a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/request_template/view/' . $aRow['idmain']) . '" data-toggle="modal" data-target="#myModal">' . $aRow['reference_no'] . '</a></div>';
            $row[] = '<div class="text-center" style="width: 80px"><a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/quotes/view_quotes/' . $aRow['id']) . '" data-toggle="modal" data-target="#myModal">' . $aRow['reference_no_quotes'] . '</a></div>';
            ...
            foreach (getListColumTable() as $kk => $vv) {
                $_data = getDataModeration($aRow['idmain'],$vv['id'],'tbl_request_template');
    ...
    function updateDate()
    {
        ...
        if (!empty($data['id']) && !empty($data['_value']) && !empty($data['name'])) {
            ...
            $result = $this->db->update('tbl_request_template_item', $ins);
            if (!empty($result)) {
                $data['result'] = 1;
    ...
    public function exportExcel()
    {
        ...
        if ($this->input->post('export_excel')) {
            ...
            $inputFileName = 'uploads/import_ch/Phieu_dieu_do_cong_viec_phat_trien_mau.xlsx';
            ...
            $this->db->select('
                tbl_request_template.id as idMain,
                tbl_request_template_item.id as id,
                tbl_request_template.date as date,
                tbl_request_template.reference_no as reference_no,
                tbl_quotes.reference_no as reference_no_quotes,
            ...
            $this->db->join('tbl_products', 'tbl_products.id = tbl_request_template_item.items_id AND tbl_request_template_item.type_item = "products"', 'left');
            ...
            $this->db->group_by('tbl_request_template_item.id');
            if (!$this->preViewModerationTemplate) {
                $this->db->where('(tbl_request_template.create_by = ' . $staff_id . ')');
            }
            ...
            $this->db->order_by('tbl_request_template.id desc');
            ...
            foreach ($items as $key => $value) {
                ...
                $objPHPExcel->getActiveSheet()->setCellValueExplicit($columsExcel[7] . $row, ($value['client_name']), PHPExcel_Cell_DataType::TYPE_STRING);
                ...
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[11] . $row, $value['height'])->getStyle($columsExcel[10] . $row);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[12] . $row, $value['wide'])->getStyle($columsExcel[11] . $row);

                $objPHPExcel->getActiveSheet()->setCellValueExplicit($columsExcel[13] . $row, ($value['unit_measure']), PHPExcel_Cell_DataType::TYPE_STRING);
                $objPHPExcel->getActiveSheet()->setCellValueExplicit($columsExcel[14] . $row, ($value['item_code']), PHPExcel_Cell_DataType::TYPE_STRING);
                $objPHPExcel->getActiveSheet()->setCellValueExplicit($columsExcel[15] . $row, ($value['item_name']), PHPExcel_Cell_DataType::TYPE_STRING);
                $objPHPExcel->getActiveSheet()->setCellValueExplicit($columsExcel[16] . $row, ($value['packing']), PHPExcel_Cell_DataType::TYPE_STRING);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[17] . $row, $value['quantity_max'])->getStyle($columsExcel[17] . $row);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[18] . $row, $value['time_inventory'])->getStyle($columsExcel[18] . $row);
                ...
                $colStt = 21;
                foreach (getListColumTable() as $kk => $vv) {
                    $_data = getDataModeration($value['idMain'],$vv['id'],'tbl_request_template','',true);
            ...
            $filename = lang('Phieu_dieu_do_cong_viec_phat_trien_mau') . '.xls';
            ob_start();
