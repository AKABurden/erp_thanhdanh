...
class Stock_out_request_product extends AdminController
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('stock_out_request_product_model');
    ...
    public function index() {
        if ($this->input->is_ajax_request()) {
            $data = [];
            $this->app->get_table_data('stock_out_request_product', $data);
        }

        $data['title'] = _l('stock_out_request_product');
        $this->load->view('admin/stock_out_request_product/index', $data);
    }
    ...
    public function submit($id = '') {
        if ($this->input->post()) {
            ...
            $result = $this->stock_out_request_product_model->submit($formData, $id);
            ...
            $data['title'] = 'Thêm ' . lang('stock_out_request_product');
            if (!empty($id)) {
                $data['title'] = 'Sửa ' . lang('stock_out_request_product');
                $data['value'] = $this->stock_out_request_product_model->get($id);
                // var_dump($data['value']);die;
                $data['id'] = $id;
            }

            $data['breadcrumb'] = [array('link' => base_url('admin/stock_out_request_product/'), 'page' => lang('stock_out_request_product')), array('link' => '#', 'page' => $data['title'])];
            $this->load->view('admin/stock_out_request_product/submit', $data);
        }
    }

    public function view($id){
        $data = [];
        $data['title'] = lang('stock_out_request_product');

        $data['value'] = $this->stock_out_request_product_model->get($id);

        $this->load->view('admin/stock_out_request_product/view',$data);
    }

    public function delete($id) {
        $result = $this->stock_out_request_product_model->delete($id);
    ...
    public function export_excel() {
        ...
        $this->db->select('
            tblstock_out_request_product.*,
            tblstock_out_request_product_item.*,
        ...
        ');
        $this->db->from('tblstock_out_request_product_item');
        $this->db->join('tblstock_out_request_product', 'tblstock_out_request_product.id = tblstock_out_request_product_item.stock_out_request_product_id', 'left');
        $this->db->join('tbl_products', 'tbl_products.id = tblstock_out_request_product_item.item_id AND tblstock_out_request_product_item.item_type = "products"', 'left');
        $this->db->join($tb_brand, 'FIND_IN_SET(tb_brand.id, tblstock_out_request_product_item.brand_ids) > 0', 'left');
        ...
        $this->db->group_by('tblstock_out_request_product_item.id');

        if (!empty($search_date)) {
            $searchDate = explode(' - ', $search_date);
            $this->db->where('tblstock_out_request_product.date BETWEEN "' . to_sql_date($searchDate[0]) . ' 00:00:00" and "' . to_sql_date($searchDate[1]) . ' 23:59:59"');
        }
        ...
        $title = mb_strtoupper(_l('stock_out_request_product'));
        $excelRowNum = 1;
        ...
        foreach ($result as $key => $aRow) {
            ...
            foreach ($aColumns as $colIndex => $colCode) {
                ...
                if ($colCode == 'image') {
                    ...
                    $code = 'stock_out_request_product||'.$aRow['stock_out_request_product_id'];

                    $qr = vn_to_str(str_replace('||', '__', $code));
                    $folder = FCPATH . 'uploads/stock_out_request_product/';
    ...
    function selectOrder($id = null)
    {
        ...
        $result['results'] = $this->stock_out_request_product_model->getOrder($filter);
        if ($id) {
            $filter = [];
            $filter['id'] = $id;
            $result['row'] = $this->stock_out_request_product_model->getOrder($filter);
        }
    ...
    function selectItem($id = null)
    {
        ...
        $result['results'] = $this->stock_out_request_product_model->getItemData($filter);
        if ($id) {
            $filter = [];
            $filter['id'] = $id;
            $result['row'] = $this->stock_out_request_product_model->getItemData($filter);
        }
        echo json_encode($result);
    }
    ...
    public function getItemStockQuantity() {
        ...
        $result = $this->stock_out_request_product_model->getItemStockQuantity($filter);
        echo json_encode($result);
    }
}
