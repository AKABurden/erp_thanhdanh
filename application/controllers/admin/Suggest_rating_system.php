...
class Suggest_rating_system extends AdminController
{
    public function __construct()
    {
        ...
        $this->preViewSuggestRatingSystem = true;
        $this->preViewOwnSuggestRatingSystem = true;
        $this->preAddSuggestRatingSystem = true;
        $this->preEditSuggestRatingSystem = true;
        $this->preApproveSuggestRatingSystem = true;
        $this->preDeleteSuggestRatingSystem = true;
    }

    public function index(){
        if (!$this->preViewSuggestRatingSystem && !$this->preViewOwnSuggestRatingSystem){
            access_denied();
        }
        $data['title'] = _l('dt_suggest_rating_system');
        $this->load->view('admin/suggest_rating_system/index', $data);
    }

    public function getSuggestRatingSystems(){
        $end_date_search = $this->input->post('end_date_search');
        $start_date_search = $this->input->post('start_date_search');

        $this->db->simple_query('SET SESSION group_concat_max_len=15000000');
        $aColumns = [
            'tbl_suggest_rating_system.id as id',
            ...
            '(SELECT GROUP_CONCAT(CONCAT(tblproduction_report.name_report,"__",tblproduction_report.id) SEPARATOR "||")
             ...
             WHERE tblproduction_report.object_id = tbl_suggest_rating_system.id AND tblproduction_report.object_type = "suggest_rating_system"
            ) as name_report',
            'tbl_suggest_rating_system.status as status',
            'tbl_suggest_rating_system.created_by as created_by',
        ];
        $sIndexColumn = 'id';
        $sTable = 'tbl_suggest_rating_system';
        $where = [
        ...
        if (!$this->preViewSuggestRatingSystem) {
        ...
        $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
            'tbl_suggest_rating_system.date_status',
            'tbl_suggest_rating_system.staff_status'
        ], '', []);
        ...
        foreach ($rResult as $key => $aRow) {
            ...
            $row[] = '<div class="text-left"><a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/suggest_rating_system/view/' . $aRow['id']) . '" data-toggle="modal" data-target="#myModal">' . $aRow['reference_no'] . '</a></div>';
            ...
            if ($aRow['status'] == 1) {
                $row[] = '<div class="text-left"><a target="_blank" href="' . base_url('admin/production_report/detail?object_id=' . $aRow['id'] . '&object_type=suggest_rating_system') . '" class="btn btn-info">Tạo phiếu báo cáo không phù hợp</a></div>
            ...
            $view = '<a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/suggest_rating_system/view/' . $aRow['id']) . '" data-toggle="modal" data-target="#myModal"><i class="fa fa-file-text-o"></i> ' . lang('view') . ' ' . lang('phiếu') . '</a>';

            $edit = $this->preEditSuggestRatingSystem ? '<a href="' . base_url('admin/suggest_rating_system/detail/' . $aRow['id']) . '"><i class="fa fa-edit"></i> ' . lang('edit') . ' ' . lang('phiếu') . '</a>' : '';

            $delete = $this->preDeleteSuggestRatingSystem ? '<a type="button" class="po" data-container="body" data-html="true" data-toggle="popover" data-placement="left" data-content="
                <button href=\'' . base_url('admin/suggest_rating_system/delete/' . $aRow['id']) . '\' class=\'btn btn-danger po-delete-json\'>' . lang('delete') . '</button>
    ...
    public function detail($id = 0){
        ...
        if ($this->input->post()){
            if (empty($id)){
                $this->form_validation->set_rules('reference_no', lang("Mã Phiếu"), 'required|is_unique[tbl_suggest_rating_system.reference_no]');
                $this->form_validation->set_rules('date', lang("date"), 'required');
                $this->form_validation->set_rules('branch_id', lang("Chi nhánh"), 'required');
                if ($this->form_validation->run() == true) {
                    $reference_no = getReference('suggest_rating_system');
                    ...
                    $this->db->insert('tbl_suggest_rating_system',$fields);
                    $id = $this->db->insert_id();
                    if ($id){
                        if (getReference('suggest_rating_system') == $reference_no) {
                            updateReference('suggest_rating_system');
                        }

                        foreach ($items as $key => $value) {
                            $value['suggest_rating_system_id'] = $id;
                            $this->db->insert('tbl_suggest_rating_system_item',$value);
                        }
                        insertActivityLog([
                            'type_parent_obj' => 'suggest_rating_system',
                            'table_obj' => 'tbl_suggest_rating_system',
                            'id_obj' => $id,
                            'name_obj' => $reference_no,
                            'content' => lang('Thêm mới yêu cầu đánh giá hệ thống') . ' [' . $reference_no . ']',
                            'actions' => 'add'
                        ]);
                        $data['result'] = 1;
                ...
                $this->db->select('tbl_suggest_rating_system.*');
                $this->db->from('tbl_suggest_rating_system');
                $this->db->where('tbl_suggest_rating_system.id',$id);
                $dtData = $this->db->get()->row_array();
                if ($dtData['reference_no'] != $this->input->post('reference_no')) {
                    $this->form_validation->set_rules('reference_no', lang("Mã Phiếu"), 'trim|required|is_unique[tbl_suggest_rating_system.reference_no]');
                }
                $this->form_validation->set_rules('date', lang("date"), 'required');
                $this->form_validation->set_rules('branch_id', lang("Chi nhánh"), 'required');
                if ($this->form_validation->run() == true) {
                    ...
                    if (!empty($counter)){
                        foreach ($counter as $key => $value){
                            ...
                            $result = ($this->input->post('result')[$value]);
                            $detail = !empty($this->input->post('detail')[$value]) ? $this->input->post('detail')[$value] : null ;
                            $suggest_rating_system_item_id = !empty($this->input->post('suggest_rating_system_item_id')[$value]) ? $this->input->post('suggest_rating_system_item_id')[$value] : 0 ;
                            $standard = ($this->input->post('standard')[$value]);
                            $items[] = [
                                'id' => $suggest_rating_system_item_id,
                                'system_id' => $system_id,
                                'detail' => $detail,
                                'result_id' => $result,
                                'standard' => $standard,
                    ...
                    $this->db->where('id',$id);
                    $success = $this->db->update('tbl_suggest_rating_system',$fields);
                    if ($success){
                        $this->db->where('suggest_rating_system_id',$id);
                        $this->db->delete('tbl_suggest_rating_system_item');
                        foreach ($items as $key => $value) {
                            $value['suggest_rating_system_id'] = $id;
                            $this->db->insert('tbl_suggest_rating_system_item',$value);
                        }
            ...
            if (empty($id)){
                if (!$this->preAddSuggestRatingSystem){
                    accessDenied(true);
                }
                $data['title'] = lang('dt_add_suggest_rating_system');
                $data['breadcrumb'] = [array('link' => base_url('admin/suggest_rating_system'), 'page' => lang('dt_suggest_rating_system')), array('link' => '#', 'page' => lang('dt_add_suggest_rating_system'))];
            } else {
                if (!$this->preEditSuggestRatingSystem){
                    accessDenied(true);
                }
                $this->db->select('tbl_suggest_rating_system.*');
                $this->db->from('tbl_suggest_rating_system');
                ...
                $this->db->select('tbl_suggest_rating_system_item.*,tbl_system.code as code_system,tbl_system.name as name_system');
                ...
                $this->db->where('tbl_suggest_rating_system_item.suggest_rating_system_id',$id);
                $dtItems = $this->db->get()->result_array();

                $data['dtData'] = $dtData;
                $data['dtItems'] = $dtItems;
                $data['title'] = lang('dt_edit_suggest_rating_system');
                $data['breadcrumb'] = [array('link' => base_url('admin/suggest_rating_system'), 'page' => lang('dt_suggest_rating_system')), array('link' => '#', 'page' => lang('dt_edit_suggest_rating_system'))];
            }
        }
        $data['employees'] = $this->manufactures_model->getAllStaff();
        $data['id'] = $id;
        $data['reference_no'] = getReference('suggest_rating_system');
        $data['dtResult'] = get_table_where('tbl_result');
        $data['dtSystem'] = get_table_where('tbl_system',['type' => 1]);
        $this->load->view('admin/suggest_rating_system/detail',$data);
    }

    public function view($id){
        $data = [];
        $data['title'] = lang('dt_view_suggest_rating_system');
        ...
        $this->db->select('tbl_suggest_rating_system_item.*,
            tbl_result.name as name_result,
            tbl_system.code as code_system,
            tbl_system.name as name_system,
        ');
        $this->db->from('tbl_suggest_rating_system_item');
        $this->db->join('tbl_system','tbl_system.id = tbl_suggest_rating_system_item.system_id','inner');
        $this->db->join('tbl_result','tbl_result.id = tbl_suggest_rating_system_item.result_id','left');
        $this->db->where('tbl_suggest_rating_system_item.suggest_rating_system_id',$id);
        $dtDataItems = $this->db->get()->result_array();

        $data['dtData'] = $dtData;
        $data['dtDataItems'] = $dtDataItems;
        $this->load->view('admin/suggest_rating_system/view',$data);
    }

    public function agree(){
        if (!$this->preApproveSuggestRatingSystem) {
            $data['result'] = 0;
        ...
        $this->db->select('tbl_suggest_rating_system.*');
        $this->db->from('tbl_suggest_rating_system');
        $this->db->where('tbl_suggest_rating_system.id',$suggest_id);
        $dtData = $this->db->get()->row_array();
        if (empty($dtData)) {
            $data['result'] = 0;
            ...
            if ($status == 0) {
                ...
                $this->db->where('tblproduction_report.object_type', 'suggest_rating_system');
            ...
            $up = $this->db->update('tbl_suggest_rating_system',$options);
            if ($up) {
                ...
                insertActivityLog([
                    ...
                    'id_obj' => $suggest_id,
                    'name_obj' => $dtData['reference_no'],
                    'content' => lang('Duyệt phiếu yêu cầu đánh giá hệ thống') . ' [' . $dtData['reference_no'] . ']',
                    'actions' => 'approved'
                ]);
                $data['result'] = 1;
    ...
    public function delete($id){
        if (!$this->preDeleteSuggestRatingSystem){
            $data['result'] = 0;
            $data['message'] = lang('access_denied');
            echo json_encode($data); die();
        }
        ...
        $this->db->where('id',$id);
        $success = $this->db->delete('tbl_suggest_rating_system');
        if ($success){
            $this->db->where('tbl_suggest_rating_system_item.suggest_rating_system_id',$id);
            $this->db->delete('tbl_suggest_rating_system_item');
            ...
            insertActivityLog([
                ...
                'id_obj' => $id,
                'name_obj' => $dtData['reference_no'],
                'content' => lang('Xóa phiếu yêu cầu đánh giá hệ thống') . ' [' . $dtData['reference_no'] . ']',
                'actions' => 'delete'
            ]);
            $data['result'] = 1;
    ...
    public function SearchSystems($id = 0){
        ...
        $data['results'][] = ['text' => lang($type == 1 ? 'Mã hệ thống' : 'Mã quy trình'), 'children' => $pod];
        if (!empty($id)){
    ...
    public function exportExcel(){
        if ($this->input->post('export_excel')) {
            ...
            $this->db->select('
                ...
                tbl_suggest_rating_system.barcode as barcode,
            ');
            $this->db->from('tbl_suggest_rating_system');
            $this->db->join('tblproduction_report', 'tblproduction_report.id = tbl_suggest_rating_system.production_report_id', 'left');
            $this->db->join('tbl_suggest_rating_system_item','tbl_suggest_rating_system_item.suggest_rating_system_id = tbl_suggest_rating_system.id');
            $this->db->join('tbl_system','tbl_system.id = tbl_suggest_rating_system_item.system_id');
            $this->db->join('tbl_result','tbl_result.id = tbl_suggest_rating_system_item.result_id');
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('A1',
                ('PHIẾU YÊU CẦU ĐÁNH GIÁ HỆ THỐNG'))->getStyle("A1")->applyFromArray([
                'font' => array(
                    'bold' => true,
                    'size' => 18,
                ),
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('D'.$sttRow.'', 'Mã Hệ Thống');
            $objPHPExcel->getActiveSheet()->setCellValue('E'.$sttRow.'', 'Tên Hệ Thống');
            $objPHPExcel->getActiveSheet()->setCellValue('F'.$sttRow.'', 'Chi Tiết Đánh Giá')->getStyle("F$sttRow")->getAlignment()->setWrapText(true);
            ...
            if (!empty($dtData)) {
                foreach ($dtData as $key => $value) {
                    ...
                    if (!empty($value['barcode'])){
                        ...
                        $code = 'suggest_rating_system||'.$value['id'];
                        $this->db->where('id',$value['id']);
                        $this->db->update('tbl_suggest_rating_system',['barcode' => $code]);
                    }
                    $qr = vn_to_str(str_replace('||', '__', $code));
                    $folder = FCPATH . 'uploads/suggest_rating_system/';
            ...
            $filename = lang('phieu_yeu_cau_danh_gia_he_thong') . '.xls';
