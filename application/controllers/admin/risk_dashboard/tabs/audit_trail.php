        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody id="at_logs_tbody" class="divide-y divide-gray-200 text-sm">
                    <tr>
                        <td colspan="4" class="text-center text-slate-400 py-8">Đang tải dữ liệu...</td>
                    </tr>
                </tbody>
            </table>
        </div>
                ...
                $.each(d.logs, function(i, log) {
                    ...
                    var rowHtml = 
                        '<tr class="hover:bg-gray-50">' +
                            '<td class="px-6 py-4 whitespace-nowrap text-slate-500">' + log.date + '</td>' +
                            '<td class="px-6 py-4 whitespace-nowrap font-medium text-slate-900">' + log.user + '</td>' +
                            '<td class="px-6 py-4 whitespace-nowrap"><span class="px-2 py-1 rounded text-xs font-medium ' + log.action_color + '">' + log.action + '</span></td>' +
                            '<td class="px-6 py-4 text-slate-500">' + escapedDetails + '</td>' +
                        '</tr>';
                    tbody.append(rowHtml);
                });
            } else {
                tbody.html('<tr><td colspan="4" class="text-center text-slate-400 py-8">Không tìm thấy dữ liệu.</td></tr>');
            }
        })
        .fail(function() {
            var tbody = $('#at_logs_tbody');
            tbody.html('<tr><td colspan="4" class="text-center text-red-500 py-8">Lỗi tải dữ liệu. Vui lòng thử lại.</td></tr>');
        });
    }
