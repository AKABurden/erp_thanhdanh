...
class Suggest_social_welfare extends AdminController
{
    public function __construct()
    {
        ...
        $this->preViewSuggestSocialWelfare = true;
        $this->preViewOwnSuggestSocialWelfare = true;
        $this->preAddSuggestSocialWelfare = true;
        $this->preEditSuggestSocialWelfare = true;
        $this->preApproveSuggestSocialWelfare = true;
        $this->preDeleteSuggestSocialWelfare = true;

    }

    public function index()
    {
        if (!$this->preViewSuggestSocialWelfare && !$this->preViewOwnSuggestSocialWelfare) {
            access_denied();
        }
        $data['title'] = _l('suggest_social_welfare');
        $this->load->view('admin/suggest_social_welfare/index', $data);
    }

    public function getSuggestSocialWelfare()
    {
        ...
        $aColumns = [
            'tbl_suggest_social_welfare.id as id',
            'tbl_suggest_social_welfare.reference_no as reference_no',
            'tbl_suggest_social_welfare.date as date',
            ...
            'tbl_suggest_social_welfare.detail as detail',
            'tbl_suggest_social_welfare.grand_total as grand_total',
            'tbl_suggest_social_welfare.status as status',
        ];
        $sIndexColumn = 'id';
        $sTable = 'tbl_suggest_social_welfare';
        $where = [];
        $filter = [];
        $tbDepartment = "(
            SELECT
        ...
        $join = [
            'INNER JOIN tblstaff ON tblstaff.staffid = tbl_suggest_social_welfare.staff_id',
            'LEFT JOIN '.$tbDepartment.' ON tb_department.staffid = tblstaff.staffid',
            'LEFT JOIN tblroles ON tblroles.roleid = tblstaff.role',
        ];

        if (!$this->preViewSuggestSocialWelfare) {
            array_push($where, 'AND tbl_suggest_social_welfare.created_by =', get_staff_user_id());
        }

        if (!empty($start_date_search)) {
            $start_date_search = to_sql_date($start_date_search) . ' 00:00:00';
            array_push($where, "AND tbl_suggest_social_welfare.date >= '" . $start_date_search . "'");
        }

        if (!empty($end_date_search)) {
            $end_date_search = to_sql_date($end_date_search) . ' 23:59:59';
            array_push($where, "AND tbl_suggest_social_welfare.date <= '" . $end_date_search . "'");
        }


        $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
            'tbl_suggest_social_welfare.date_status',
            'tbl_suggest_social_welfare.staff_status',
        ], '', []);
        ...
        foreach ($rResult as $key => $aRow) {
            ...
            $row[] = '<div class="text-left" style="width: 120px"><a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/suggest_social_welfare/view/' . $aRow['id']) . '" data-toggle="modal" data-target="#myModal">' . $aRow['reference_no'] . '</a></div>';
            ...
            $view = '<a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/suggest_social_welfare/view/' . $aRow['id']) . '" data-toggle="modal" data-target="#myModal"><i class="fa fa-file-text-o"></i> ' . lang('view') . ' ' . lang('phiếu') . '</a>';

            $edit = $this->preEditSuggestSocialWelfare ? '<a class="tnh-modal" href="' . base_url('admin/suggest_social_welfare/detail/' . $aRow['id']) . '"><i class="fa fa-edit"></i> ' . lang('edit') . ' ' . lang('phiếu') . '</a>' : '';

            $delete = $this->preDeleteSuggestSocialWelfare ? '<a type="button" class="po" data-container="body" data-html="true" data-toggle="popover" data-placement="left" data-content="
                <button href=\'' . base_url('admin/suggest_social_welfare/delete/' . $aRow['id']) . '\' class=\'btn btn-danger po-delete-json\'>' . lang('delete') . '</button>
    ...
    public function detail($id = 0)
    {
        ...
        if ($this->input->post()) {
            if (empty($id)) {
                $this->form_validation->set_rules('reference_no', lang("Mã Phiếu"), 'required|is_unique[tbl_suggest_social_welfare.reference_no]');
                ...
                if ($this->form_validation->run() == true) {
                    $reference_no = getReference('suggest_social_welfare');
                    ...
                    $this->db->insert('tbl_suggest_social_welfare', $fields);
                    $id = $this->db->insert_id();
                    if ($id) {
                        if (getReference('suggest_social_welfare') == $reference_no) {
                            updateReference('suggest_social_welfare');
                        }
                        insertActivityLog([
                            'type_parent_obj' => 'suggest_social_welfare',
                            'table_obj' => 'tbl_suggest_social_welfare',
                            'id_obj' => $id,
                            'name_obj' => $reference_no,
                            'content' => lang('Thêm mới yêu cầu phúc lợi xã hội') . ' [' . $reference_no . ']',
                            'actions' => 'add'
                        ]);
                        $data['result'] = 1;
                ...
                $this->db->select('tbl_suggest_social_welfare.*');
                $this->db->from('tbl_suggest_social_welfare');
                $this->db->where('tbl_suggest_social_welfare.id', $id);
                $dtData = $this->db->get()->row_array();
                if ($dtData['reference_no'] != $this->input->post('reference_no')) {
                    $this->form_validation->set_rules('reference_no', lang("Mã Phiếu"), 'trim|required|is_unique[tbl_suggest_social_welfare.reference_no]');
                }
                $this->form_validation->set_rules('date', lang("date"), 'required');
                $this->form_validation->set_rules('branch_id', lang("Chi nhánh"), 'required');
                $this->form_validation->set_rules('staff_id', lang("Nhân viên"), 'required');
                if ($this->form_validation->run() == true) {
                    ...
                    $this->db->where('id', $id);
                    $success = $this->db->update('tbl_suggest_social_welfare', $fields);
                    if ($success) {
                        insertActivityLog([
                            'type_parent_obj' => 'suggest_social_welfare',
                            'table_obj' => 'tbl_suggest_social_welfare',
                            ...
                            'content' => lang('Sửa phiếu yêu cầu phúc lợi xã hội') . ' [' . $dtData['reference_no'] . ']',
                            'actions' => 'edit'
                        ]);
                        $data['result'] = 1;
            ...
            if (empty($id)) {
                if (!$this->preAddSuggestSocialWelfare) {
                    accessDenied(true);
                }
                $data['title'] = lang('dt_add_suggest_social_welfare');
            } else {
                if (!$this->preEditSuggestSocialWelfare) {
                    accessDenied(true);
                }
                ...
                ');
                $this->db->from('tbl_suggest_social_welfare');
                $this->db->join('tblstaff','tblstaff.staffid = tbl_suggest_social_welfare.staff_id');
                ...
                $data['title'] = lang('dt_edit_suggest_social_welfare');
            }
        }
        $data['dtData'] = $dtData;
        $data['staff'] = getPersonDeparmentdt(0);
        $data['id'] = $id;
        $data['reference_no'] = getReference('suggest_social_welfare');
        $this->load->view('admin/suggest_social_welfare/detail', $data);
    }

    public function view($id)
    {
        $data = [];
        $data['title'] = lang('dt_view_suggest_social_welfare');
        ...
        ');
        $this->db->from('tbl_suggest_social_welfare');
        $this->db->join('tblstaff', 'tblstaff.staffid = tbl_suggest_social_welfare.staff_id', 'inner');
        $this->db->join($tbDepartment, 'tb_department.staffid = tblstaff.staffid', 'left');
        $this->db->join('tblroles', 'tblroles.roleid = tblstaff.role', 'left');
        $this->db->where('tbl_suggest_social_welfare.id', $id);
        $dtData = $this->db->get()->row_array();
        ...
        $data['dtData'] = $dtData;
        $this->load->view('admin/suggest_social_welfare/view', $data);
    }

    public function agree()
    {
        if (!$this->preApproveSuggestSocialWelfare) {
            $data['result'] = 0;
        ...
        $this->db->select('tbl_suggest_social_welfare.*');
        $this->db->from('tbl_suggest_social_welfare');
        $this->db->where('tbl_suggest_social_welfare.id', $suggest_id);
        $dtData = $this->db->get()->row_array();
        if (empty($dtData)) {
            $data['result'] = 0;
            ...
            $up = $this->db->update('tbl_suggest_social_welfare', $options);
            if ($up) {
                ...
                insertActivityLog([
                    ...
                    'id_obj' => $suggest_id,
                    'name_obj' => $dtData['reference_no'],
                    'content' => lang('Duyệt phiếu yêu cầu phúc lợi xã hội') . ' [' . $dtData['reference_no'] . ']',
                    'actions' => 'approved'
                ]);
                $data['result'] = 1;
    ...
    {
        if (!$this->preDeleteSuggestSocialWelfare) {
            $data['result'] = 0;
            $data['message'] = lang('access_denied');
            echo json_encode($data);
            die();
        ...
        $this->db->where('id', $id);
        $success = $this->db->delete('tbl_suggest_social_welfare');
        if ($success) {
            ...
            insertActivityLog([
                ...
                'id_obj' => $id,
                'name_obj' => $dtData['reference_no'],
                'content' => lang('Xóa phiếu yêu cầu phúc lợi xã hội') . ' [' . $dtData['reference_no'] . ']',
                'actions' => 'delete'
            ]);
            $data['result'] = 1;
    ...
    public function exportExcel()
    {
        ...
        if ($this->input->post('export_excel')) {
            ...
            $inputFileName = 'uploads/import_ch/phieu_yeu_cau_phuc_loi_xa_hoi.xlsx';
            ...
            $this->db->join('tblroles', 'tblroles.roleid = tblstaff.role', 'left');
            if (!$this->preViewSuggestSocialWelfare) {
                $this->db->where('(tbl_suggest_social_welfare.created_by = ' . $staff_id . ')');
            }

            if (!empty($start_date_search)) {
                $start_date_search = to_sql_date($start_date_search) . ' 00:00:00';
                $this->db->where("tbl_suggest_social_welfare.date >= '" . $start_date_search . "'");
            }

            if (!empty($end_date_search)) {
                $end_date_search = to_sql_date($end_date_search) . ' 23:59:59';
                $this->db->where("tbl_suggest_social_welfare.date <= '" . $end_date_search . "'");
            }
            $this->db->order_by('tbl_suggest_social_welfare.id asc');
            ...
            $filename = lang('phieu_yeu_cau_phuc_loi_xa_hoi') . '.xls';
            ob_start();
