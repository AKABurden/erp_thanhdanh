...
class Request_printed_page_layout extends AdminController
{
    public function __construct()
    {
        ...
        $this->preViewRequestPrintedPageLayout = true;
        $this->preViewOwnRequestPrintedPageLayout = true;
        $this->preAddRequestPrintedPageLayout = true;
        $this->preEditRequestPrintedPageLayout = true;
        $this->preApproveRequestPrintedPageLayout = true;
        $this->preDeleteRequestPrintedPageLayout = true;
    }

    public function index()
    {
        if (!$this->preViewRequestPrintedPageLayout && !$this->preViewOwnRequestPrintedPageLayout) {
            access_denied();
        }
        $data['title'] = _l('ch_request_printed_page_layout');
        $this->load->view('admin/request_printed_page_layout/index', $data);
    }
    public function view($id)
    {
        $data = [];
        $data['title'] = lang('ch_view_request_printed_page_layout');

        $this->db->select('tbl_request_printed_page_layout.*');
        $this->db->from('tbl_request_printed_page_layout');
        $this->db->where('tbl_request_printed_page_layout.id', $id);
        $dtData = $this->db->get()->row_array();

        $this->db->select('tbl_request_printed_page_layout_item.*
        ');
        $this->db->from('tbl_request_printed_page_layout_item');
        $this->db->where('tbl_request_printed_page_layout_item.request_printed_page_layout_id', $id);
        $dtDataItems = $this->db->get()->result_array();

        $data['dtData'] = $dtData;
        $data['dtDataItems'] = $dtDataItems;
        $this->load->view('admin/request_printed_page_layout/view', $data);
    }
    public function detail($id = 0)
    {
        ...
        if ($this->input->post()) {
            if (!empty($dtData) && $dtData['reference_no'] != $this->input->post('reference_no')) {
                $this->form_validation->set_rules('reference_no', lang("Mã Phiếu"), 'trim|required|is_unique[tbl_request_printed_page_layout.reference_no]');
            }
            ...
            if (empty($id)) {
                if ($this->form_validation->run() == true) {
                    $reference_no = getReference('request_printed_page_layout');
                    ...
                    $this->db->insert('tbl_request_printed_page_layout', $fields);
                    $id = $this->db->insert_id();
                    if ($id) {
                        if (getReference('request_printed_page_layout') == $reference_no) {
                            updateReference('request_printed_page_layout');
                        }

                        foreach ($items as $key => $value) {
                            $value['request_printed_page_layout_id'] = $id;
                            $this->db->insert('tbl_request_printed_page_layout_item', $value);
                        }
                        insertActivityLog([
                            'type_parent_obj' => 'request_printed_page_layout',
                            'table_obj' => 'tbl_request_printed_page_layout',
                            'id_obj' => $id,
                            'name_obj' => $reference_no,
                            'content' => lang('Thêm mới phiếu yêu cầu dàn trang in') . ' [' . $reference_no . ']',
                            'actions' => 'add'
                        ]);
                        $data['result'] = 1;
                ...
                if ($this->form_validation->run() == true) {
                    ...
                    $this->db->where('id', $id);
                    $success = $this->db->update('tbl_request_printed_page_layout', $fields);
                    if ($success) {
                        $this->db->where('request_printed_page_layout_id', $id);
                        $this->db->delete('tbl_request_printed_page_layout_item');
                        foreach ($items as $key => $value) {
                            $value['request_printed_page_layout_id'] = $id;
                            $this->db->insert('tbl_request_printed_page_layout_item', $value);
                        }
                        insertActivityLog([
                            ...
                            'content' => lang('Sửa phiếu yêu cầu dàn trang in') . ' [' . $dtData['reference_no'] . ']',
                            'actions' => 'edit'
                        ]);
                        $data['result'] = 1;
            ...
            if (empty($id)) {
                if (!$this->preAddRequestPrintedPageLayout) {
                    accessDenied(true);
                }
                $data['title'] = lang('ch_add_request_printed_page_layout');
                $data['breadcrumb'] = [array('link' => base_url('admin/request_printed_page_layout'), 'page' => lang('ch_request_printed_page_layout')), array('link' => '#', 'page' => lang('ch_add_request_printed_page_layout'))];
            } else {
                if (!$this->preEditRequestPrintedPageLayout) {
                    accessDenied(true);
                }
                ...
                $data['title'] = lang('dt_edit_request_printed_page_layout');
                $data['breadcrumb'] = [array('link' => base_url('admin/request_printed_page_layout'), 'page' => lang('ch_request_printed_page_layout')), array('link' => '#', 'page' => lang('dt_edit_request_printed_page_layout'))];
            }
        }
        $data['employees'] = $this->manufactures_model->getAllStaff();
        $data['id'] = $id;
        $data['reference_no'] = getReference('request_printed_page_layout');
        ...
        $this->load->view('admin/request_printed_page_layout/detail', $data);
    }
    public function searchMachines($id = 0)
    {
        ...
        $this->db->where('tbl_type_productionlist.id', 24);
    ...
    //     echo json_encode($data);
    // }
    public function getRequestPrintedPageLayout()
    {
        ...
        $aColumns = [
            'tbl_request_printed_page_layout.id as id',
        ...
        $sIndexColumn = 'id';
        $sTable = 'tbl_request_printed_page_layout';
        $where = [];
        $filter = [];

        $join = [
            'INNER JOIN tblstaff ON tblstaff.staffid = tbl_request_printed_page_layout.staff_create',
            'INNER JOIN tbl_productions_orders ON tbl_productions_orders.id = tbl_request_printed_page_layout.po_id',
        ];

        if (!$this->preViewRequestPrintedPageLayout) {
        ...
        $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, ['tbl_request_printed_page_layout.po_id'], '', []);
        ...
        foreach ($rResult as $key => $aRow) {
            ...
            $row[] = '<div class="text-left" style="width: 120px"><a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/request_printed_page_layout/view/' . $aRow['id']) . '" data-toggle="modal" data-target="#myModal">' . $aRow['reference_no'] . '</a></div>';
            ...
            $view = '<a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/request_printed_page_layout/view/' . $aRow['id']) . '" data-toggle="modal" data-target="#myModal"><i class="fa fa-file-text-o"></i> ' . lang('view') . ' ' . lang('phiếu') . '</a>';

            $edit = $this->preEditRequestPrintedPageLayout ? '<a href="' . base_url('admin/request_printed_page_layout/detail/' . $aRow['id']) . '"><i class="fa fa-edit"></i> ' . lang('edit') . ' ' . lang('phiếu') . '</a>' : '';

            $delete = $this->preDeleteRequestPrintedPageLayout ? '<a type="button" class="po" data-container="body" data-html="true" data-toggle="popover" data-placement="left" data-content="
                <button href=\'' . base_url('admin/request_printed_page_layout/delete/' . $aRow['id']) . '\' class=\'btn btn-danger po-delete-json\'>' . lang('delete') . '</button>
    ...
    public function exportExcel()
    {
        ...
        if ($this->input->post('export_excel')) {
            ...
            $inputFileName = 'uploads/import_ch/Phieu_yeu_cau_dan_trang_in.xlsx';
            ...
            $this->db->select('
                tbl_request_printed_page_layout.reference_no,
                tbl_productions_orders.reference_no as reference_no_productions_orders,
                tbl_request_printed_page_layout.date,
                tbl_request_printed_page_layout_item.*,
                tbl_machines.id as id, 
                tbl_machines.name as text,
                tbl_machines.code as code,
                tbl_machines.name as name,
            ...
            $this->db->order_by('tbl_request_printed_page_layout.id asc');
            ...
            foreach ($items as $key => $value) {
                ...
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[7] . $row, $value['childsheet'])->getStyle($columsExcel[9] . $row);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[8] . $row, $value['columnssheets1'])->getStyle($columsExcel[9] . $row);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[9] . $row, $value['rowssheets1'])->getStyle($columsExcel[9] . $row);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[10] . $row, $value['quantity_print_color1'])->getStyle($columsExcel[9] . $row);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[11] . $row, $value['quantity_zinc1'])->getStyle($columsExcel[9] . $row);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[12] . $row, $value['number_operations1'])->getStyle($columsExcel[9] . $row);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[13] . $row, $value['columnssheets2'])->getStyle($columsExcel[9] . $row);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[14] . $row, $value['rowssheets2'])->getStyle($columsExcel[9] . $row);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[15] . $row, $value['quantity_print_color2'])->getStyle($columsExcel[9] . $row);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[16] . $row, $value['quantity_zinc2'])->getStyle($columsExcel[9] . $row);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[17] . $row, $value['number_operations2'])->getStyle($columsExcel[9] . $row);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[18] . $row, $value['quantity_total_zinc'])->getStyle($columsExcel[9] . $row);
                $objPHPExcel->getActiveSheet()->setCellValue($columsExcel[19] . $row, $value['timequota'])->getStyle($columsExcel[9] . $row);
            ...
            $filename = lang('Phieu_yeu_cau_dan_trang_in') . '.xls';
            ob_start();
    ...
    {
        if (!$this->preDeleteRequestPrintedPageLayout) {
            $data['result'] = 0;
        ...
        $this->db->where('id', $id);
        $success = $this->db->delete('tbl_request_printed_page_layout');
        if ($success) {
            $this->db->where('tbl_request_printed_page_layout_item.request_printed_page_layout_id', $id);
            $this->db->delete('tbl_request_printed_page_layout_item');
