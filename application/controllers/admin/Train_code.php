...
class Train_code extends AdminController
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('train_code_model');
    }
    function index()
    {
        $data = [];
        $data['title'] = lang('Danh Sách Mã Đào Tạo');
        $data['type'] = 1;
        $this->load->view('admin/train_code/manage', $data);
    }
    function getTrainCode()
    {
        $aColumns = [
            'tbl_train_code.id as id',
            'tbl_train_code.code as code',
            'tbl_train_code.name as name',
            '"" as actions'
        ];
        $type_search = $this->input->post('type_search');

        $sIndexColumn = 'id';
        $sTable       = 'tbl_train_code';
        $where        = [];
        $filter = [];
        ...
        foreach ($rResult as $key => $aRow) {
            ...
            $row = [];
            $id = $aRow['id'];
            $edit = '<a class="tnh-modal" href="' . base_url('admin/train_code/handling/' . $id) . '"><i class="fa fa-edit"></i> ' . lang('tnh_edit') . '</a>';
            $delete = '<a type="button" class="po" data-container="body" data-html="true" data-toggle="popover" data-placement="left" data-content="
                <button href=\'' . base_url('admin/train_code/delete/' . $id) . '\' class=\'btn btn-danger po-delete-json\'>' . lang('delete') . '</button>
    ...
    function handling($id = '', $type = 1)
    {
        ...
        $data = [];
        $train_code = $id ? $this->train_code_model->getTrainCodeById($id) : [];
        if ($this->input->post()) {
            if (!empty($id)) {
                if ($train_code['code'] != $this->input->post('code')) {
                    $this->form_validation->set_rules('code', lang("Mã quy định"), 'trim|required|is_unique[tbl_train_code.code]');
                }
            } else {
                $this->form_validation->set_rules('code', lang("Mã quy định"), 'trim|required|is_unique[tbl_train_code.code]');
            }
            ...
            if ($this->form_validation->run() == true) {
                ...
                if ($id) {
                    ...
                    $ins = $this->train_code_model->updateTrainCode($id, $option);
                    $standard_id = $id;
                } else {
                    $option['staff_create'] = get_staff_user_id();
                    $option['date_create'] = date('Y-m-d H:i:s');
                    $ins = $this->train_code_model->insertTrainCode($option);
                    $standard_id = $ins;
                }
        ...
        $data['id'] = $id;
        $data['type'] = $type;
        $data['dtData'] = $train_code;
        $title = '';
        $title = $id ? lang('Sửa Danh Sách Mã Đào Tạo') : lang('Thêm Danh Sách Mã Đào Tạo');
        $data['title'] = $title;
        $this->load->view('admin/train_code/handling', $data);
    }
    public function delete($id)
    {
        $data = [];
        if ($this->train_code_model->deleteTrainCode($id)) {
            $data['result'] = 1;
            $data['message'] = lang('success');
        } else {
            $data['result'] = 0;
    ...
    {
        $title = '';
        $title = lang('Import Danh Sách Mã Đào Tạo');
        $data['title'] = $title;
        $data['type'] = $type;
        $this->load->view('admin/train_code/import_excel', $data);
    }
        ...
        if (!empty($data)) {
            foreach ($data as $key => $value) {
                ...
                if ($checkerrors == 0) {
                    ...
                    $check_id = $this->db->get('tbl_train_code')->row_array();
                    $id = '';
                    if (!empty($check_id)) {
                        $id = $check_id['id'];
                    }
