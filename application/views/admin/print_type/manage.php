    ...
    <div class="panel_s mbot10 H_scroll" id="H_scroll">
        <div class="panel-body _buttons">
            <div class="_buttons">
                ...
                <a href="<?= base_url('admin/print_type/modal_excel_import') ?>" class="btn btn-info pull-right mright10 H_action_button c_modal">
                        ...
                        <table id="table-costs" class="table dt-tnh table-hover table-cost-new" style="width: 100%;">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center"><?= lang('print_type_code') ?></th>
                                    <th class="text-center"><?= lang('print_type_name') ?></th>
...
<script>
    $(function() {
        ...
        oTable = tnhInitDataTable('#table-costs', '<?= site_url('admin/print_type/table') ?>', {
            'order': [
                [0, 'asc']
            ],
            'fixedHeader': {
            ...
            "ajax": {
                "url": '<?= site_url('admin/print_type/table') ?>',
                "type": "POST",
                "data": function(d) {
    ...
    function add(id) {
        $('#modal').html('');
        $.get(admin_url + 'print_type/add_modal/' + id).done(function(response) {
            $('#modal').html(response);
            $('#add_modal').modal({
                show: true,
                backdrop: 'static'
    ...
    function deleting (id = "") {
        ...
        if (r == false) {
            ...
            $.get(admin_url + 'print_type/delete/' + id, function(response) {
                if (response.success) {
                    alert_float('success', response.message);
                    oTable.draw();
                } else {
