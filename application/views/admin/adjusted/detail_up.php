				 	...
				 	<?php 
						...
						<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4 mbot50">
							<div class="panel panel-warning">
								<div class="panel-heading"><?=_l('ch_items_up')?></div>
								<div class="panel-body">
									<table class="tnh-tb table-bordered table-hover dont-responsive-table m-group0" style="table-layout: fixed;width: 100%;">
										<tbody>
											<tr>
												...
												<td style="width: 70%;">
								                    <input data-placeholder="<?=_l('ch_list_items')?>" id="custom_item_select" class="custom_item_select" style="width: 100%" >
												</td>
											</tr>
											<tr>
												<td>
												...
												<td>
					                  				<select style="width: 100%;" id="localtion_id" class="localtion_id"  data-live-search="true" data-width="100%" data-placeholder="<?=_l('ch_list_localtion')?>" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
										            </select>
												</td>
											</tr>
											<tr>
									...
									<!-- <table class="table items item-adjusted no-mtop dont-responsive-table"> -->
										<thead>
											<tr>
												...
												<th style="width: 100px" class="text-center quantity_time hide"><?php echo _l('invoice_table_quantity_heading'); ?></th>
												<th style="width: 100px" class="text-center"><?php echo _l('tnh_quantity_warehouses'); ?></th>
                                                <th style="width: 120px" class="text-center"><?php echo _l('ch_quantity_up'); ?></th>
						...
						<table class="table tnh-tb noMargin table-color_sum dont-responsive-table">
							<tbody>
								<tr>
									...
									<td>
										<span class="bold"><?php echo _l('ch_quantity_up_total'); ?> :</span>
									</td>
									<td class="total_net">
...
<script>
		   	...
		   	function createTrItem(data,type)
			    {
			        ...
			        var td7 = $('<td class="text-center"><input class="height_auto handling" type="hidden" name="items[' + uniqueArray + '][handling]" value=""></td>');
			    ...
			    $(document).on('keyup', '.mainQuantityNet', (e)=>{
			        ...
			        handlingInput=$(e.currentTarget).parents('tr').find('input.handling');
			        handlingTd=handlingInput.parent();
			        var handling=tnhFormatNumber(Number(mainQuantityNet)+Number(quantity.val()));
			        handlingTd.text(handling);
			        handlingInput.val(handling);
			        handlingTd.append(handlingInput);
			        var total = unformat_number($(e.currentTarget).parents('tr').find('input.price').val());
					$(e.currentTarget).parents('tr').find('td.amount').text(formatNumber(Math.abs(total*handling)));
	...
	$('.form-submiters_all').on('click', (e)=>{
        ...
        if(items.length == 0)
        {
            alert_float('danger', "ch_not_itemss");
            return;
    	}
        $.each(items, (index,value)=>{
