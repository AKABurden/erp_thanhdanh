<div class="modal" id="modal_handling_accreditation">
    <?php echo form_open_multipart('admin/categories_other/handlingaccreditation/' . (!empty($id) ? $id : ''), array('id' => 'handling-accreditation')); ?>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input type="text" name="code_group" id="code_group" placeholder="<?= lang('Mã khu vực') ?>" class="form-control code_group" value="<?= !empty($accreditation) ? $accreditation['code_group'] : '' ?>" required="required">
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <?= lang('Tên khu vực', 'name') ?>
                            <input type="text" name="name" id="name" placeholder="<?= lang('Tên khu vực') ?>" class="form-control name" value="<?= !empty($accreditation) ? $accreditation['name'] : '' ?>" required="required">
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <?= lang('Ghi chú', 'detail') ?>
                            <textarea name="note"  id="note" placeholder="<?= lang('Chi tiết') ?>" class="form-control" rows="3"><?= !empty($accreditation) ? $accreditation['note'] : '' ?></textarea>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <table id="tb-handling_accreditation_detail" class="table table-hover table-cs dataTable">
                            <thead>
                            <tr>
                                ...
                                <th>Tiêu Chuẩn Test/Kiểm Định</th>
                                <th class="text-center" style="width: 50px;"><i class="fa fa-trash-o"></i></th>
                            </tr>
                            </thead>
                            <tbody>
                            <?php if(!empty($accreditation['detail'])) {
                                foreach($accreditation['detail'] as $key => $value) {?>
									<?php
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <textarea type="text" name="detail[<?=$key?>][note]" class="form-control note mbot10" placeholder="Tiêu chuẩn Test/Kiểm định" aria-invalid="false"><?=$value['note']?></textarea>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <textarea type="text" name="detail[0][note]" class="form-control note mbot10" placeholder="Tiêu chuẩn Test/Kiểm định" aria-invalid="false"></textarea>
...
<script>
    $(function() {
        $('#modal_handling_accreditation').modal('show');
        appValidateForm($('#handling-accreditation'), {
            name: 'required',
            code_group: 'required',
        }, handlingData);
    ...
    var countItemsMain = <?=!empty($accreditation['detail']) ? count($accreditation['detail']) : 1?>;
    function addTd() {
        ...
        var tdFile = `<td>
                     <textarea type="text" name="detail[${countItemsMain}][note]" class="form-control note mbot10" placeholder="Tiêu Chuẩn Test/Kiểm định" aria-invalid="false"></textarea>
        ...
        $('#tb-handling_accreditation_detail tbody').append(trProcess);
        countItemsMain++;
        stt_handling_accreditation();
    }

    function stt_handling_accreditation() {
        var tr = $('#tb-handling_accreditation_detail tbody tr');
    ...
    function removeFile(id, _this) {
        if(confirm('Bạn có muốn xóa file này?')) {
            $.get(admin_url + 'categories_other/remove_file_accreditation/' + id, function (result) {
