...
<style type="text/css">
    ...
    .table-suggestion tr td:nth-child(13) {
        width: 100px;
        white-space: unset;
        text-align: center;
    }
    .table-suggestion tr td:nth-child(14) {
        width: 100px;
        white-space: unset;
        text-align: center;
    }

    .table-suggestion tr td:nth-child(15) {
        width: 120px;
        white-space: unset;
        text-align: center;
    }
        ...
        <div class="row">
            ...
            <div class="col-md-2">
                ...
                <div class="form-group">
                    ...
                    <select data-placeholder="Trạng thái" name="status" style="width: 100%" class="status" id="status" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                        <option value=""></option>
            ...
            <div class="col-md-2">
                ...
                <div class="form-group">
                    ...
                    <select data-placeholder="Duyệt người đề nghị" name="status_dn" style="width: 100%" class="status_dn" id="status_dn" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                        <option value=""></option>
            ...
            <div class="col-md-2">
                ...
                <div class="form-group">
                    ...
                    <select data-placeholder="Duyệt trưởng phòng" autocomplete="off" name="status_tp" style="width: 100%" class="status_tp" id="status_tp" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                        <option value=""></option>
            ...
            <div class="col-md-2">
                ...
                ?>
                <div class="form-group">
                    <label for="status_tq" class="control-label"><?= _l('Duyệt thủ quỹ') ?></label>
                    <select data-placeholder="Duyệt thủ quỹ" autocomplete="off" name="status_tq" style="width: 100%" class="status_tq" id="status_tq" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                        <option value=""></option>
                        <?php foreach ($status_tq as $t) { ?>
                            <option value="<?php echo $t['id']; ?>"><?= $t['name'] ?> </option>
                        <?php } ?>
                    </select>
                </div>
                ...
                <div class="panel_s">
                    <div class="panel-body">
                        ...
                        <div class="col-md-5 mbot10">
                            <div class="col-md-6"><div class="border10 bg-warning pointer checkTotal" data-id="cash">
                                    <div style="font-size: 12px;">Tổng Phiếu Cần Chi Quỹ - Tiền Mặt </div>
                                    <b class="total_cash" style="font-size: 18px;"></b>
                                </div>
                            </div>
                            <div class="col-md-6"><div class="border10 bg-success pointer checkTotal" data-id="bank">
                                    <div style="font-size: 12px;">Tổng Phiếu Cần Chi Quỹ - Ngân Hàng </div>
                                    <b class="total_bank" style="font-size: 18px;"></b>
                                </div>
                            </div>
                        </div>
                        <div class="clearfix"></div>
                        <input type="hidden" id="filterStatus" name="filterStatus" value="" />
                        <input type="hidden" id="filterCheckTotal" name="filterCheckTotal" value="" />
                        ...
                        <?php render_datatable_tfoot_ch(array(
                            ...
                            _l('Loại'),
							_l('Trạng thái'),
                            _l('Hình thức thanh toán'),
                            _l('Người duyệt chi'),
                            _l('Người đề xuất'),
                            _l('Số tiền đề xuất'),
                            _l('Tổng chi'),
                            _l('Nội dung mô tả danh mục chi'),
                            _l('Người đề xuất duyệt'),
                            _l('Trưởng phòng duyệt'),
                            _l('Thủ quỹ hoàn thành'),
                            _l('Tác vụ'),
                        ), 'suggestion'); ?>
                    </div>
                </div>
...
<script>
    $('.table-suggestion').on('draw.dt', function() {
        ...
        $('.dataTables_scrollFoot').find('tfoot td').eq(8).html('<div class="text-right">' + sums.total + '</div>');
        $('.dataTables_scrollFoot').find('tfoot td').eq(9).html('<div class="text-right">' + sums.pay + '</div>');


        var total = itemsTable.ajax.json().total;
    ...
    $(function() {
        var CustomersServerParams = {
            ...
            'filterCheckTotal': '[name="filterCheckTotal"]',
        };
    ...
    function approved_treasurer(id) {
        dataString = {
            id: id,
            [csrfData['token_name']]: csrfData['hash']
        };
        jQuery.ajax({
            type: "post",
            url: "<?= admin_url() ?>suggestion/ApprovedTreasurer",
            data: dataString,
            cache: false,
            success: function(response) {
                response = JSON.parse(response);
    ...
    $('.checkTotal').click(function() {
        ...
        $('.checkTotal').removeClass('active');
        if(active == 0) {
            $(this).addClass('active');
            $('#filterCheckTotal').val($(this).data('id')).trigger('change');
        }
        else {
            $('#filterCheckTotal').val('').trigger('change');
        }
