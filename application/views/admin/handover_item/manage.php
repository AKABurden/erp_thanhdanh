    ...
    <div class="panel_s mbot10 H_scroll" id="H_scroll">
        <div class="panel-body _buttons">
            <div class="_buttons">
                ...
                <a href="<?= base_url('admin/handover_item/modal_excel_import') ?>" class="btn btn-info pull-right mright10 H_action_button c_modal">
            ...
            <div class="col-md-12">
                <div class="panel_s">
                    <div class="panel-body">
                        <table id="data-table" class="table dt-tnh table-hover " style="width: 100%;">
                            <thead>
                                <tr>
                                    <th class="text-center"><?= lang('STT') ?></th>
                                    <th class="text-center"><?= lang('handover_item_name') ?></th>
...
<script>
    $(function() {
        ...
        oTable = tnhInitDataTable('#data-table', '<?= site_url('admin/handover_item/table') ?>', {
            'order': [
                [0, 'asc']
            ],
            'fixedHeader': {
            ...
            "ajax": {
                "url": '<?= site_url('admin/handover_item/table') ?>',
                "type": "POST",
                "data": function(d) {
    ...
    function add(id) {
        $('#modal').html('');
        $.get(admin_url + 'handover_item/add_modal/' + id).done(function(response) {
            $('#modal').html(response);
            $('#add_modal').modal({
                show: true,
                backdrop: 'static'
    ...
    function deleting (id = "") {
        ...
        if (r == false) {
            ...
            $.get(admin_url + 'handover_item/delete/' + id, function(response) {
                if (response.success) {
                    alert_float('success', response.message);
                    oTable.draw();
                } else {
