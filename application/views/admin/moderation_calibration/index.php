...
<style type="text/css">
    ...
    #table-moderation_calibration tr th:nth-child(2) {
        width: 100px;
    }

    #table-moderation_calibration tr th:nth-child(7) {
        width: 120px;
    }
</style>
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="">
                                <div class="table-responsive">
                                    <table id="table-moderation_calibration" class="table dt-tnh table-moderation_calibration-new" style="width: 100%;">
                                        <thead>
                                            <tr>
                                                <th class="text-center"><?= lang('STT') ?></th>
                                                <th class="text-center"><?= lang('Mã Số Phiếu') ?></th>
                                                <th class="text-center"><?= lang('Ngày Lập Phiếu') ?></th>
                                                <th class="text-center"><?= lang('Nhóm hiệu chuẩn') ?></th>
                                                <th class="text-center"><?= lang('Bộ Phận Hiệu Chuẩn') ?></th>
                                                <th class="text-center"><?= lang('Chi Tiết Hiệu Chuẩn') ?></th>
                                                <th class="text-center"><?= lang('Số lượng') ?></th>
                                                <th class="text-center"><?= lang('Đơn giá') ?></th>
                                                <th class="text-center"><?= lang('Thành tiên') ?></th>
...
<script>
    ...
    oTable = tnhInitDataTable('#table-moderation_calibration',
        '<?= site_url('admin/moderation_calibration/getModerationCalibration') ?>', {
            'order': [
                [0, 'desc']
            ],
            'fixedHeader': {
            ...
            "ajax": {
                "url": '<?= site_url('admin/moderation_calibration/getModerationCalibration') ?>',
                "type": "POST",
                "data": function(d) {
    ...
    $('#table-moderation_calibration').on('draw.dt', function(event) {
        init_datepicker();
    });
    $(document).on('change',
    ...
    function updateDate(_this, id, name) {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/moderation_calibration/updateDate',
            data: dataPOST,
            dataType: "json",
    ...
    function agree(_this, suggest_id, status) {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/moderation_calibration/agree',
            data: dataPOST,
            dataType: "json",
            success: function(response) {
                if (response.result) {
    ...
    function exportExcel() {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/moderation_calibration/exportExcel',
            data: {
