...
<style type="text/css">
    ...
    .dt-buttons.btn-group .buttons-collection.btn-default-dt-options {
    ...
    <div class="panel_s mbot10 H_scroll" id="H_scroll">
        <div class="panel-body ">
            <div class="_buttons">
                ...
                <a  class="btn btn-info H_action_button pull-right mleft5" target="_blank" href="<?=admin_url('pay_slip')?>">Xem danh sách phiếu chi cũ</a>
                ...
                <?php } ?>
                <?php if ($this->type == 'service'){ ?>
                <?php if (has_permission('other_payslips', '', 'create')) { ?>
                    <div class="line-sp"></div>
                    <a href="" onclick="new_other_payslips(true); return false;" class="btn btn-info mright5 test pull-right H_action_button">
                        <i class="lnr lnr-plus-circle" aria-hidden="true"></i>
                        <?php echo _l('create_add_new').' '._l('Loại cũ'); ?></a>
                <?php } ?>
                <?php } ?>
            </div>
                                ...
                                <ul class="nav nav-tabs nav-tabs-horizontal" role="tablist">
                                    ...
                                    <li>
                                        <a class="H_filter" data-id="7">
                                            <?= _l('ch_pay_tscd') ?>(<span class="pay_tscd">0</span>)
                                        </a>
                                    </li>
                                </ul>
                            </div>
...
<script>
    ...
    function new_other_payslips(old = false) {
        $('#view_other_payslips').html('');
        $(document).off('change', '#id_costs');
        $(document).off('change', '#objects_id');
        $(document).off('change', '#type_vouchers');
        $.get(admin_url + `other_payslips/other_payslips/0/${old}?type=<?= $type ?>`).done(function(response) {
    ...
    $(function() {
        ...
        tAPI = initDataTableCustom('.table-other_payslips', admin_url + 'other_payslips/table', [0], [0], CustomersServerParams, <?php echo hooks()->apply_filters('customers_table_default_order', json_encode(array(0, 'desc'))); ?>, fixedColumns = {
            leftColumns: 2,
            rightColumns: 1
        });
        ...
        $('.table-other_payslips').on('draw.dt', function() {
            ...
            $('.dataTables_scrollFoot').find('tfoot td').eq(9).html('<div class="text-right">' + sums.payment + '</div>');
            get_total_limit();
        });
    });
    ...
    function exportExcel() {
        ...
        type_check = $('[name="type"]').val();

        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/other_payslips/exportExcel',
            data: {
