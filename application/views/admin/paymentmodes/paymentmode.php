            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <?php echo render_input('opening_balance','opening_balance',0,'text',array('onkeyup'=>"formatNumBerKeyUp(this)")); ?>
                        ...
                        <div class="hide checkbox checkbox-primary">
                        ...
                        <div class="checkbox checkbox-primary pm-available-to">
                            ...
                            <label for="cash"><?php echo _l('ch_cash'); ?></label>
                        </div>
                        <div class="checkbox checkbox-primary pm-available-to">
                            <input type="checkbox" name="bank" id="bank">
                            <label for="bank"><?php echo _l('ch_bank'); ?></label>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    window.addEventListener('load',function(){
        ...
        $('.pm-available-to input').on('change',function(){
            ...
            if(checked == 1 && name == 'bank'){
                $('input[name="cash"]').prop('disabled',true);
            } else if(checked == 0 && name == 'bank'){
                $('input[name="cash"]').prop('disabled',false);
            } else if(checked == 1 && name=='cash'){
                $('input[name="bank"]').prop('disabled',true);
            } else if(checked == 0 && name == 'cash'){
                $('input[name="bank"]').prop('disabled',false);
            }
        });
        ...
        $('#payment_mode_modal').on('show.bs.modal', function(event) {
            ...
            $('#payment_mode_modal input[name="cash"]').prop('checked', true).prop('disabled',false);
            $('#payment_mode_modal input[name="bank"]').prop('checked', false).prop('disabled',false);
            ...
            if (typeof(id) !== 'undefined') {
                ...
                var description = $(button).parents('tr').find('td').eq(3).html();
                var active = $(button).parents('tr').find('td').eq(4).find('input').prop('checked');
                $('#payment_mode_modal input[name="active"]').prop('checked', active);
                $('#payment_mode_modal input[name="cash"]').prop('checked', cash).change();
                $('#payment_mode_modal input[name="bank"]').prop('checked', bank).change();
