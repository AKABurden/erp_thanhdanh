                    ...
                    <div class="panel-body">
                        <div class="col-md-12">
                            ...
                            <div class="table-responsive">
                                <table id="table-moderation_evaluate" class="table dt-tnh table-moderation_evaluate-new" style="width: 100%;">
                                    <thead>
                                        <tr>
                                            <th class="text-center"><?= lang('STT') ?></th>
                                            <th class="text-center"><?= lang('Ngày Lập Phiếu') ?></th>
                                            <th class="text-center"><?= lang('Mã phiếu đánh giá') ?></th>
                                            ...
                                            <th class="text-center"><?= lang('Mã đánh giá') ?></th>
                                            <th class="text-center"><?= lang('Chi tiết đánh giá') ?></th>
                                            <th class="text-center"><?= lang('Hiện trạng thực tế') ?></th>
...
<script>
    ...
    oTable = tnhInitDataTable('#table-moderation_evaluate',
        '<?= site_url('admin/moderation_evaluate/getModerationEvaluate') ?>', {
            'order': [
                [0, 'desc']
            ],
            'fixedHeader': {
            ...
            "ajax": {
                "url": '<?= site_url('admin/moderation_evaluate/getModerationEvaluate') ?>',
                "type": "POST",
                "data": function(d) {
    ...
    $('#table-moderation_evaluate').on('draw.dt', function(event) {
        init_datepicker();
    });
    ...
    function updateDate(_this, id, name) {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/moderation_evaluate/updateDate',
            data: dataPOST,
            dataType: "json",
    ...
    function exportExcel() {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/moderation_evaluate/exportExcel',
            data: {
