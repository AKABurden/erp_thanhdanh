    ...
    <div class="panel_s mbot10 H_scroll" id="" style="margin-bottom: unset">
        <div class="panel-body ">
            <div class="_buttons">
                ...
                <?php if ($this->preAddSuggestProbationaryEvaluate): ?>
                    <div class="pull-right mright5 H_border">
                        <a href="<?= base_url('admin/suggest_probationary_evaluate/detail?type='.$this->type.'') ?>" class=" tnh-modal btn btn-info H_action_button">
                            <?php echo _l('add'); ?>
                        </a>
                    </div>
                <?php endif ?>
                        ...
                        <div class="col-md-12">
                            <div class="row" style="margin-bottom:5px">
                                <div id="search-tnh" class="collapse in" aria-expanded="true">
                                    <input type="hidden" name="type" class="type" id="type" value="<?= $this->type ?>">
                                    <div class="col-md-3">
                            ...
                            <div class="clearfix"></div>
                            <div class="">
                                <table id="table-suggest-probationary-evaluate" class="table dt-tnh table-suggest-probationary-evaluate-new" style="width: 100%;">
                                    <thead>
                                    <tr>
                                        ...
                                        <th class="text-center"><?= lang('Ngày Bắt Đầu Thử Việc') ?></th>
                                        <th class="text-center"><?= lang('Ngày Kết Thúc Thử Việc') ?></th>
                                        <th class="text-center"><?= lang('Quản lý trực tiếp') ?></th>
                                        <th class="text-center"><?= lang('Trưởng phòng nhân sự') ?></th>
...
<script>
    ...
    oTable = tnhInitDataTable('#table-suggest-probationary-evaluate',
        '<?= site_url('admin/suggest_probationary_evaluate/getSuggestProbationaryEvaluate') ?>', {
            'order': [
                [0, 'desc']
            ],
            'fixedHeader': {
            ...
            "ajax": {
                "url": '<?= site_url('admin/suggest_probationary_evaluate/getSuggestProbationaryEvaluate') ?>',
                "type": "POST",
                "data": function(d) {
    ...
    <?php if ($this->type == 2){ ?>
    oTable.columns(8).visible(false, false);
    oTable.columns(9).visible(false, false);
    <?php } ?>
    $(document).on('change',
    ...
    function agree(_this, suggest_id, status) {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url+'admin/suggest_probationary_evaluate/agree',
            data: dataPOST,
            dataType: "json",
    ...
    function exportExcel() {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/suggest_probationary_evaluate/exportExcel',
            data: {
