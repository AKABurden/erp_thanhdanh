<div class="modal-dialog modal-lg" style="min-width: 80%;">
    <?php echo form_open(admin_url('suggest_accreditation/detail/' . $id),
        ['id' => 'suggest_accreditation']); ?>
                ...
                <div class="col-md-12">
                    <table id="tb-maintenance-machines" class="table dataTable">
                        ...
                        <tr>
                            ...
                            <th class="text-center"><?= lang('Quy định Test/Kiểm định') ?></th>
                                    ...
                                    <tr class="rowID_<?=$rowID?> is_<?=$rowID?>_<?= $value['machines_maintenance_id'] ?>">
                                        ...
                                        <td>
                                            <div>
                                                ...
                                                <input type="hidden" class="suggest_accreditation_item_id"
                                                       name="suggest_accreditation_item_id[<?= $counter ?>]"
                                                       value="<?= $value['id'] ?>">
...
<script type="text/javascript">
    ...
    ajaxSelectParams('#machines_id', 'admin/suggest_accreditation/search_suggest_repalce', '', true, true);
    $("#branch_id").select2();
    ...
    $('#suggest_accreditation').on('change', '.check_items_all', function () {
    ...
    $('#suggest_accreditation').on('click', '.event-plus', function() {
        var TrClass = $(this).attr('data-class');
    ...
    $("#machines_id").change(function () {
        ...
        if (string_id) {
            $.ajax({
                url: site.base_url + 'admin/suggest_accreditation/getMaintenaceMachinesDetail',
                type: 'POST',
                dataType: 'json',
                data: {
                    ...
                    if (data.dtMaintenaceMachines.length > 0) {
                        ...
                        alert_float('danger', 'Thiết bị/Khu vực không có danh mục kiểm tra Test/Kiểm định');
                    }
                    $("#machines_id").val('');
                    ajaxSelectParams('#machines_id', 'admin/suggest_accreditation/search_suggest_repalce', '', true, true);
                })
    ...
    appValidateForm($('#suggest_accreditation'), {
        date: 'required',
