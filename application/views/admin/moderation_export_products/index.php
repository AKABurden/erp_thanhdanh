                    ...
                    <div class="panel-body">
                        <div class="col-md-12">
                            ...
                            <div class="table-responsive">
                                <table id="table-moderation_export_products" class="table dt-tnh table-moderation_export_products-new" style="width: 100%;">
                                    <thead>
                                        <tr>
                                            <th class="text-center" rowspan="2"><?= lang('STT') ?></th>
                                            <th class="text-center" rowspan="2"><?= lang('Ngày Lập Phiếu') ?></th>
                                            <th class="text-center" rowspan="2"><?= lang('Mã Phiếu Yêu Cầu xuất TP tồn') ?></th>
                                            ...
                                            <th class="text-center" rowspan="2"><?= lang('Số Con') ?></th>
                                            <th class="text-center" rowspan="2"><?= lang('Số Kiện') ?></th>
                                            <th class="text-center" rowspan="2"><?= lang('Số Kg') ?></th>
                                            <th class="text-center" rowspan="2"><?= lang('Tổng Số Kiện') ?></th>
                                            <th class="text-center" rowspan="2"><?= lang('Thời Gian Quy Định') ?></th>
...
<script>
    ...
    oTable = tnhInitDataTable('#table-moderation_export_products',
        '<?= site_url('admin/moderation_export_products/getModerationExportProducts') ?>', {
            'order': [
                [0, 'desc']
            ],
            scrollX:true,
            "ajax": {
                "url": '<?= site_url('admin/moderation_export_products/getModerationExportProducts') ?>',
                "type": "POST",
                "data": function(d) {
    ...
    $('#table-moderation_export_products').on('draw.dt', function(event) {
        init_datepicker();
    });
    ...
    function updateDate(_this, id, name) {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/moderation_export_products/updateDate',
            data: dataPOST,
            dataType: "json",
    ...
    function exportExcel() {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/moderation_export_products/exportExcel',
            data: {
