...
<body>
    <div class="app">
        ...
        <div class="container">
            ...
            <div class="sidebar">
                <div class="card">
                    <div class="row"><span>Tổng số chuyến giao trong ngày</span><span></span></div>
...
<script>
    ...
    function rowTpl(r) {
        return `<div class="table-row" data-id="${r.id}">
            ...
            <div><span>${r.code_orders}</span></div>
            <div><span>${r.reference_no}</span></div>
            ...
            <div class="${(r.warehouseman_id > 0 ? 'text_delivery' : 'text_no')}"><span>${(r.warehouseman_id > 0 ? 'Đã duyệt kho' : 'Chưa duyệt kho')}</span></div>
            ...
            rows.forEach(r => {
                ...
                if (r.date) {
                    ...
                    if (datePart && timePart) {
                        ...
                        if (deliveryDate < new Date() && Number(r.warehouseman_id) === 0) {
                            isOverdue = true;
                        }
                    }
                }
    ...
    //     //     setCellText($order, row.id);
    //     //     setCellText($sku, row.sku);
    //     //     setCellText($stage, row.stage);

    //     //     $row.find(".yellow").text(Number(row.qty_plan).toLocaleString());
    //     //     $row.find(".green").text(Number(row.qty_done).toLocaleString());
    //     //     $row.find(".red").text(Number(row.qty_todo).toLocaleString());

    //     //     $row.find(".bar-fill").css({
    //     //         width: row.percent + "%",
    //     //         background: row.bar_color
    //     //     });
    //     //     $row.find(".progress-bar .percent").text(row.percent + "%");
    ...
    function updateStats(rows) {
        ...
        // sidebar
        $(".card .row:contains('Tổng số chuyến giao trong ngày') span:last").text(total);
