			...
			<div class="col-md-12">
				<div class="panel_s">
					<div class="panel-body" style="overflow-x: auto;">
						...
						<div id="calendar_mainternance"></div>
					</div>
				</div>
			</div>
		</div>
...
<script type="text/javascript">
	// $(document).ready(function() {
		calendar_selector_pod = $("#calendar_mainternance");
		var nCalendar = {
            ...
            loading: function(e, t) {
                e && $("#calendar_mainternance .fc-header-toolbar .btn-default").addClass("btn-info").removeClass("btn-default").css("display", "block"), e ? $(".dt-loader").removeClass("hide") : $(".dt-loader").addClass("hide")
                setTimeout(function() {
                    $('.bg-danger-fc').parents('.fc-day-grid-event').addClass('bg-danger-fc-full');
                }, 1500)

            },
            eventSources: [{
                url: admin_url + "maintenance/getCalendar",
                data: function() {
                    var e = {[csrfData['token_name']] : csrfData['hash']};
                    e['machines_search'] = $('#machines_search').val();
                    e['maintenance_search'] = $('#maintenance_search').val();
            ...
            eventLimitClick: function(e, t) {
                $("#calendar_mainternance").fullCalendar("gotoDate", e.date), $("#calendar_mainternance").fullCalendar("changeView", "basicDay");
            },
	...
	// });
    $('body').on('change', '#machines_search, #maintenance_search', function() {
        calendar_selector_pod.fullCalendar('refetchEvents');
        // $('button[onclick="data_filler()"]').attr('disabled', true);
    })
</script>