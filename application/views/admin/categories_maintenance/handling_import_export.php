<div class="modal-dialog modal-lg" style="min-width: 60%;">
    <?php echo form_open(
        admin_url('categories_maintenance/handling_import_export/' . $id . '/' . $type),
        ...
        <div class="modal-body">
            <table class="tnh-tb table-bordered table-hover">
                <tbody>
                    <tr>
                        <td>
                            <?php if ($type == 'imported_documents') { ?>
                                <?= lang('Mã nhập khẩu', 'code') ?>
                            <?php } else { ?>
                                <?= lang('Mã xuất khẩu', 'code') ?>
                            <?php } ?>
                        </td>
                        <td colspan="3">
                        ...
                        <td>
                            <?php if ($type == 'imported_documents') { ?>
                                <?= lang('Tên nhập khẩu', 'code') ?>
                            <?php } else { ?>
                                <?= lang('Tên xuất khẩu', 'code') ?>
                            <?php } ?>
                        </td>
                        <td colspan="3">
                            <div class="form-group">
                    ...
                    <tr>
                        <td><?= lang('Danh mục', 'categories') ?></td>
                        <td colspan="3">
                            <div class="form-group">
                                <input type="text" name="categories" class="form-control categories" id="categories" value="<?= !empty($dtData) ? ($dtData['categories']) : '' ?>">
                            </div>
                        </td>
                    </tr>
                </tbody>
