<div class="modal-dialog modal-lg" style="min-width: 60%;">
    <?php echo form_open(
        admin_url('categories_maintenance/handling/' . $id.'/'.$type),
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <table class="tnh-tb table-bordered table-hover">
                <tbody>
                    <tr>
                        <td><?= lang('Mã Phiếu', 'code') ?></td>
                        <td>
                            <input type="text" name="code" id="code" class="form-control" data-placeholder="<?= lang('Mã Phiếu') ?>" value="<?= !empty($dtData) ? $dtData['code'] : '' ?>" title="">
                        </td>
                    ...
                    <tr>
                        <td><?= lang('Bộ phận bảo dưỡng', 'maintenance_department') ?></td>
                        <td colspan="1">
                            <select name="maintenance_department" id="maintenance_department" class="maintenance_department" data-placeholder="<?= lang('Bộ phận bảo dưỡng') ?>" style="width: 100%;">
                                <option value=""></option>
                                <?php if (!empty($dtMaintenanceDepartment)) { ?>
                                    <?php foreach ($dtMaintenanceDepartment as $key => $value) { ?>
                                        <option <?= !empty($dtData) ? ($dtData['id_maintenance_department'] == $value['id'] ? 'selected' : '') : '' ?> value="<?= $value['id'] ?>"><?= $value['name'] ?></option>
                                    <?php } ?>
                                <?php } ?>
                            </select>
                        </td>
                            ...
                            <select name="category_maintenance" id="category_maintenance" class="category_maintenance" data-placeholder="<?= lang('Nhóm bảo dưỡng') ?>" style="width: 100%;">
                                ...
                                <?php if (!empty($dtCategoryMaintenance)) { ?>
                                    <?php foreach ($dtCategoryMaintenance as $key => $value) { ?>
                                        <option <?= !empty($dtData) ? ($dtData['id_category_maintenance'] == $value['id'] ? 'selected' : '') : '' ?> value="<?= $value['id'] ?>"><?= $value['name'] ?></option>
                                    <?php } ?>
                                <?php } ?>
                            </select>
                        </td>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="form-group">
                                <input type="text" name="quantity" class="form-control quantity number-format" id="quantity" value="<?= !empty($dtData) ? formatNumber($dtData['quantity']) : 0 ?>">
                            </div>
                        </td>
                        <td><?= lang('Thời hạn quy định', 'date_regulations') ?></td>
                        <td>
                            <div class="form-group">
                                <input type="text" name="date_regulations" class="form-control date_regulations number-format" id="date_regulations" value="<?= !empty($dtData) ? formatNumber($dtData['date_regulations']) : 0 ?>">
                            </div>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td style="width: 35%;">
                            <?= form_input(
                                'date_start',
                                !empty($dtData) ? _dt($dtData['date_start']) : '',
                                'id="date" class="form-control datetimepicker" placeholder="' . lang('date') . '" required '
                            ) ?>
                        </td>
                        <td style="width: 15%;">
                            <?= lang('Thời gian tái tục', 'date_renewals') ?>
                        </td>
                        <td style="width: 35%;">
                            <?= form_input(
                                'date_renewals',
                                !empty($dtData) ? _dt($dtData['date_renewals']) : '',
                                'id="date" class="form-control datetimepicker" placeholder="' . lang('date') . '" required '
                            ) ?>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td colspan="3">
                            <textarea name="detail" id="detail" class="form-control detail" cols="3" rows="4"><?= !empty($dtData) ? $dtData['detail'] : '' ?></textarea>
                        </td>
                    </tr>
                </tbody>
            </table>
