...
<style type="text/css">
    ...
    #table-business-fee-boiler-overtime th {
...
<div id="wrapper">
    <div class="panel_s mbot10 H_scroll" id="H_scroll" style="margin-bottom: unset">
        <div class="panel-body ">
            ...
            <a onclick="add()" class="btn btn-info test pull-right hide H_action_button">
            ...
            <div class="col-md-12">
                <div class="panel_s">
                    <div class="panel-body">
                        ...
                        <div class="clearfix"></div>
                        <table id="table-business-fee-boiler-overtime" class="table dt-tnh table-business-fee-boiler-overtime-new" style="">
                            <thead>
                            <tr>
                                <th></th>
                                ...
                                <th class="text-center"><?= lang('Tổng tăng ca') ?></th>
...
<script>
    ...
    function loadTableNew() {
        oTable = tnhInitDataTable('#table-business-fee-boiler-overtime',
            '<?= site_url('admin/business_fee_other/getBusinessFeeBoilerOvertimes') ?>', {
                'order': [
                    [0, 'desc'],
                ],
                "ajax": {
                    "url": '<?= site_url('admin/business_fee_other/getBusinessFeeBoilerOvertimes') ?>',
                    "type": "POST",
                    "data": function (d) {
    ...
    $('#table-business-fee-boiler-overtime tbody').on('click', 'td .rows-child', function() {
    ...
    $('body').on('click', '#agree_child', function () {
        ...
        if (status == 2) {
            ...
            $(".po-save").click(function() {
                ...
                if (note != '') {
                    $.ajax({
                        url: site.base_url + 'admin/business_fee_other/update_status_overtime_child',
                        type: 'POST',
                        dataType: 'JSON',
                        data: {
            ...
            $.post(admin_url + 'business_fee_other/update_status_overtime_child', data, function (result) {
                result = JSON.parse(result);
                if (result.result) {
                    oTable.draw('page');
                    alert_float('success', result.message);
    ...
    function clickAgreeAll(id){
        ...
        if (r == false) {
            ...
            if (id) {
                ...
                jQuery.ajax({
                    type: "post",
                    url: "<?= admin_url() ?>business_fee_other/clickAgreeAll",
                    data: dataString,
                    cache: false,
                    success: function(response) {
                        response = JSON.parse(response);
