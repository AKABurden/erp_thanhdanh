                    ...
                    <div class="panel-body">
                        <div class="col-md-12">
                            ...
                            <div class="table-responsive">
                                <table id="table-moderation-plan-stage-phan-don" class="table dt-tnh table-moderation-plan-stage-phan-don-new" style="width:2200px;">
                                    <thead>
                                    <tr>
                                        ...
                                        <th class="text-center"><?= lang('Tên Mã Chủng Loại SP') ?></th>
                                        <th class="text-center"><?= lang('Tên Brand') ?></th>
                                        <th class="text-center"><?= lang('Phân Loại Khách Hàng') ?></th>
                                        <th class="text-center"><?= lang('Đơn Vị Tính SP') ?></th>
                                        <th class="text-center"><?= lang('Số Lượng Size') ?></th>
                                        <th class="text-center"><?= lang('Số Lần Soạn') ?></th>
...
<script>
    ...
    oTable = tnhInitDataTable('#table-moderation-plan-stage-phan-don',
        '<?= site_url('admin/moderation_plan_stage/getModerationPlanStagesPhanDon') ?>', {
            'order': [
                [0, 'desc']
            ],
            scrollX:true,
            "ajax": {
                "url": '<?= site_url('admin/moderation_plan_stage/getModerationPlanStagesPhanDon') ?>',
                "type": "POST",
                "data": function(d) {
    ...
    $('#table-moderation-plan-stage-phan-don').on('draw.dt', function(event) {
        init_datepicker();
    });
    ...
    function updateModerationPlanStagePhanDon(_this,po_id,item_id,type_productionlist_id,stage_id,name){
        var dataPOST = {};
        ...
        $.ajax({
            type: "POST",
            url: site.base_url+'admin/moderation_plan_stage/updateModerationPlanStagePhanDon',
            data: dataPOST,
            dataType: "json",
    ...
    function exportExcel() {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/moderation_plan_stage/exportExcelStagePhanDon',
            data: {
