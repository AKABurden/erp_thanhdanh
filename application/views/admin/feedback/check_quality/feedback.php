<?php $folder = 'feedback/check_quality';?>
<?php $class = 'check_quality';?>
<?php $mC = '-check_quality';?>
<?php $urlForm = 'feedback/add_feed_back_check_quality';?>
<?php $urlRemove = 'feedback/remove_feed_back_check_quality';?>
...
<div class="examples">
    <textarea name="comment_feedback" id="feedback_<?=time();?>" placeholder="<?php echo _l('feedback'); ?>"
        id="feedback_comment" rows="3" class="form-control ays-ignore mention-textarea"></textarea>
</div>
...
<button type="button" class="btn btn-info pull-right mbot20" autocomplete="off"
    onclick="add_feedback(<?=!empty($qualityControl) ? $qualityControl['id'] : ''?>);">
    Thêm bình luận
</button>
<?php echo form_close(); ?>
...
<div class="clearfix"></div>
<div
    class="feedback-<?=$class?> data-feed-back-check_quality-<?=!empty($qualityControl) ? $qualityControl['id'] : ''?>">
...
function add_feedback(id) {
    ...
    $.ajax({
            ...
            if (data.success) {
                if (data.html) {
                    if ($('div[data-feed-back-check_quality="' + data.id + '"]').length == 0) {
...
function removeFeedBack(id = "") {
    if (confirm('Xóa feedback sẽ không thể khôi phục bạn có chắc muốn xóa?')) {
        $.get(admin_url + '<?=$urlRemove?>/' + id, function(data) {
            ...
            if (data.success) {
                $('div[data-feed-back-check_quality="' + id + '"]').remove();
            }
            alert_float(data.alert_type, data.message);
        })
    }
