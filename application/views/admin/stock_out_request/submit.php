...
<?php echo form_open('admin/stock_out_request/submit/' . ($id ?? '') . '', array('id' => 'submit_form')); ?>
<div>
                        ...
                        <tbody>
                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td class="item_name"><?= $itemValue['item_name'] ?? '' ?></td>
                                        ...
                                        <td>
                                            <div class="stock_quantity text-center"><?= formatNumber($itemValue['stock_quantity']) ?></div>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="purchase_require_quantity text-center hide"><?= formatNumber($itemValue['purchase_require_quantity']) ?></div>
                                            <input type="text" name="purchase_require_quantity[<?= $counter ?>]" id="purchase_require_quantity_<?= $counter ?>" class="input_purchase_require_quantity number-format form-control" value="<?= !empty($itemValue['purchase_require_quantity']) ? formatNumber($itemValue['purchase_require_quantity']) : 0 ?>">
                                        </td>
...
<script type="text/javascript">
    ...
    if (edit == 1){
        for (i = 0; i < counter; i++) {
            ajaxSelectParamsCallback(`#order_id_${i}`, 'admin/stock_out_request/selectOrder', $(`#order_id_${i}`).val(), {
                'production_order_id': $('#production_order_id').val()
            }, true);
            ajaxSelectParamsCallback(`#item_id_${i}`, 'admin/stock_out_request/selectItem', $(`#item_id_${i}`).val(), {
                'production_order_id': $('#production_order_id').val(),
    ...
    $(document).on('change', '.order_id', function() {
        ...
        ajaxSelectParamsCallback(`#${item.attr('id')}`, 'admin/stock_out_request/selectItem', 0, {
            'production_order_id': $('#production_order_id').val(),
            'order_id': $($(this)).val()
        }, true);
        item.change();
    ...
    $(document).on('change', 'input.item_id', function() {
        ...
        if (item) {
            ...
            $.ajax({
                    ...
                    url: site.base_url + 'admin/stock_out_request/getItemStockQuantity',
                    type: 'POST',
                    dataType: 'json',
                    data: paramsData
                })
    ...
    function addRow() {
        ...
        ajaxSelectParamsCallback(`#order_id_${counter}`, 'admin/stock_out_request/selectOrder', 0, {
            'production_order_id': $('#production_order_id').val()
        }, true);
        ajaxSelectParamsCallback(`#item_id_${counter}`, 'admin/stock_out_request/selectItem', 0, {
            'production_order_id': $('#production_order_id').val(),
        ...
        $.ajax({
            ...
            .done(function(data) {
                ...
                if (data.result) {
                    ...
                    // window.location.href = site.base_url+'admin/stock_out_request';
