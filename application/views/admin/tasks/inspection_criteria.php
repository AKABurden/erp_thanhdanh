<style>
    /* ============================================================ */
    /* MOBILE: Modal Kiểm Quy Trình - Responsive Full-Screen        */
    /* ============================================================ */
    @media (max-width: 768px) {
        /* Modal chiếm gần hết màn hình */
        ...
        /* Header: chữ nhỏ lại, nút X lớn hơn */
        #view_modal .modal-header {
            padding: 12px 15px !important;
        }
        #view_modal .modal-header h4.modal-title {
        ...
        /* Checkbox duyệt / không duyệt to hơn, dễ chạm */
        #view_modal .checkbox input[type="checkbox"] {
...
<div class="modal fade" id="view_modal" role="dialog">
    <div class="modal-dialog" style="min-width: 10%;width: 50%;">
        <div class="modal-content">
            ...
            <?php echo form_open('admin/tasks/add_task_process/', array('id' => 'task')); ?>
...
<script>
    ...
    $.post(admin_url + 'tasks/get_table_delivery_records_internal_proposal', data, function (result) {
    ...
    $(function () {
        ...
        function saveHandlingProducts(form) {
            ...
            var disabledFields = form.find(':input:disabled').prop('disabled', false);
            $.each(formParams, function (i, val) {
                if($(val).attr('disabled') === 'disabled' || $(val).attr('readonly') === 'readonly'){
                    val.value = 0;
                }
    ...
    function checkResult(id, _this, type) {
        ...
        if (type == 1) {
            // Duyệt
            message = 'Bạn có chắc chắn muốn duyệt tiêu chí này không?';
            ...
            if (r == false) {
                ...
                // Gọi ajax xử lý duyệt và gửi mail
                var form = $('#task');
                var formData = new FormData(form[0]);
                formData.append('inspection_criteria_current_id', id);
                formData.append('type', type);
                // Lấy role_processing từ hidden input
                var roleProcessing = $('input[name="role_processing[' + id + ']"]').val();
                formData.append('role_processing', roleProcessing || '');

                $.ajax({
                    url: admin_url + 'tasks_approval/approve_criteria',
                    type: 'POST',
                    dataType: 'JSON',
                    cache: false,
                    contentType: false,
        ...
        if ($(_this).prop('checked')) {
            ...
            if (type == 2) {
                ...
                if (r == false) {
                    ...
                    var formData = new FormData(form[0]);
                    $.ajax({
                        url: admin_url + 'tasks/add_task_process_reject',
                        type: 'POST',
                        dataType: 'JSON',
                        cache: false,
                        contentType: false,
                        ...
                        success: function (data) {
                            if (data.success) {
                                var url = '<?= base_url('admin/production_report/detail') . '?id_tasks=' . $id . '&id_tasks_process=' . $detail_id . '&id_tasks_process_child=' ?>' + id
