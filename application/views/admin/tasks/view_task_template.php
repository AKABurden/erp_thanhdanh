...
<div class="modal-body">
    <div class="row">
        <div class="col-md-8 task-single-col-left">
            ...
            <?php } ?>
            <?php if (!empty($task->id_list_object) || !empty($task->rel_id)) {
                echo '<div class="task-single-related-wrapper">';

                $string_departments = [];
                if (!empty($departments)) {
                    foreach ($departments as $k => $v) {
                        $string_departments[] = $v['name'];
                    }
                }
                $rowSQL = '';
                $rowSQL = implode(', ', $string_departments);
                //                $task_rel_value = get_table_where('tbldepartments_tasks', ['id' => $task->id_list_object], '', 'row');
                //                if(!empty($task_rel_value)) {
                //					$rowSQL .= $task_rel_value->name;
                //                }

                if (!empty($task->rel_type)) {
                    ...
                    if (!empty($task_rel_value['id'])) {
                        ...
                        if ($task->rel_type == "orders") {
                            $rowSQL .= '<a data-tnh="modal" class="tnh-modal2" data-toggle="modal" data-target="#myModal2" href="' . $task_rel_value['link'] . '">' . $task_rel_value['name'] . '</a>';
                        } else {
                            $rowSQL .= '<a href="' . $task_rel_value['link'] . '" target="_blank">' . $task_rel_value['name'] . '</a>';
                        }
                    }
                }
                echo !empty($rowSQL) ? '<h4 class="bold font-medium mbot15">' . _l('LIÊN QUAN ĐẾN') . ': ' . $rowSQL . '</h4>' : '';

                if ($task->rel_type == 'order_production_details') {
                    $shiftWork = $this->site_model->getShiftWorkById($task->shift_work);
                    echo ' ' . $shiftWork['name'] . ' - SL: ' . $task->quantity_shift_work;
                }
            ...
            ?>
                ...
                <!--                                            <li>-->
                <!--                                                <a href="#" onclick="-->
                <!--                                                        //task_change_milestone(--><?php //echo $milestone['id']; 
                                                                                                        ?><!--//,--><?php ////echo $task->id; 
                                                                                                                    ?>
                <!--                                                        //);">-->
                ...
                <!--            --><?php //} else {
                                    ...
                                    echo '</h4>';
                                    echo '</div>';
                                } else if (!empty($task->rel_type)) {
                                    //				echo '<div class="task-single-related-wrapper"><div class="bold font-medium mbot15">' . _l('task_single_related') . ': ' . _l('c_tasks_' . $task->rel_type) . '</div></div>';
                                } ?>
            ...
            <!--công-->
            <?php if (!empty($task->rel_type) && $task->rel_type == "maintenance_ticket") {
                ...
                $this->db->where('tblmaintenance_ticket.id', $task->rel_id);
                $items_machines = $this->db->get('tblmaintenance_ticket')->row_array();

                $machines = $this->db->get_where('tbl_machines', ['id' => $items_machines['id_machines']])->row();
                ...
                $this->db->where('tblmaintenance_stick_category.maintenance_stick', $task->rel_id);
                $category_maintenance = $this->db->get_where('tblmaintenance_stick_category')->result_array();

                $_print_maintenance = '';
                if (has_permission('maintenance', '', 'print')) {
                    $_print_maintenance = '<a class="btn btn-default btn-icon" target="_blank" href="' . admin_url('maintenance/pdf/' . $task->rel_id) . '"><i class="fa fa-print" aria-hidden="true"></i></a>';
                }

                echo '<h4 class="bold font-medium mbot15">THÔNG TIN CHI TIẾT PHIẾU BẢO TRÌ MÁY MÓC ' . $_print_maintenance . '</h4>';
                $htmlItems = '';
                $htmlItemsMM = '';
                if (!empty($items_machines)) {
                    $this->db->select([
                    ...
                    if (!empty($htmlItemsMM)) {
                        $htmlItemsMM = '<table class="table">
                                        <tr>
                ...
                echo '<h5 class="no-mtop">
                        <span class="text-dark">* Thiết bị: <span class="text-dark">' . ($machines->name) . '</span></span>
                	  </h5>';
                echo '<h5 class="no-mtop">
                        <span class="text-dark">* Ngày bảo trì theo lịch trình: <span class="text-dark">' . _d($items_machines['date_maintenance']) . '</span></span>
                	  </h5>';

                echo '<h5 class="no-mtop">
                        <span class="text-dark">* Tổng Số lượng : <span class="text-dark">' . number_format_data($items_machines['quantity_pcs']) . '</span></span>
                	  </h5>';
                ...
                $htmlCategory = '<table class="table" style="margin-top: 5px;">
                                    <thead>
                                        <tr>
                                            <th style="text-align: center;">Mã hạng mục bảo trì</th>
                                            <th style="text-align: center;">Tên hạng mục bảo trì</th>
                                            <th style="text-align: center;">Đạt</th>
                                            <th style="text-align: center;">Không Đạt</th>
                                            <th style="text-align: center;">Nhân viên đánh giá</th>
                                        </tr>
                                    </thead>
                                    <tbody>' . $htmlBody . '</tbody>
                                </table>';

                echo $htmlCategory . '<hr/>';
            } ?>
            <!-- end công-->
        ...
        <div class="col-md-4 task-single-col-right">
            ...
            <h5 class="no-mtop task-info-created">
                ...
                if (!empty($task->tasks_group)) {
                    $dtTaskGroup = get_table_where('tbl_task_group', ['id' => $task->tasks_group], '', 'row_array');
                    if (!empty($dtTaskGroup)) {
                        $task_group = $dtTaskGroup['name'];
                    }
                }
                ?>
                <div class="mtop10"><b style="text-transform: capitalize"><u>Nhóm Công Việc Phòng Ban:</u></b> <span><?= $task_group ?></span></div>
                <?php
                $rowDepartments = '';
                if (!empty($departments)) {
                    $rowDepartments .= '<div style="margin-left: -5px;">';
                    foreach ($departments as $k => $v) {
                        $rowDepartments .= '<span class="inline-block label mleft5 mtop5" style="font-size: 9px;color:' . (!empty($color_department[$k]) ? $color_department[$k] : '') . ';border:1px solid ' . (!empty($color_department[$k]) ? $color_department[$k] : '') . '">' . $v['name'] . '</span>';
                    }
                    $rowDepartments .= '</div>';
                }
                ?>
                <?php if (!empty($rowDepartments)) { ?>
                    <div class="mtop10"><b style="text-transform: capitalize"><u>Phòng ban</u></b> <span><?= $rowDepartments ?></span></div>
                <?php } ?>
            </h5>
            ...
            <div class="task-info task-info-user-logged-time">
                <h5>
                    <i class="fa fa-bandcamp task-info-icon fa-fw fa-lg" aria-hidden="true"></i>Mã công việc: <?= !empty($category_tasks) ? ($category_tasks->code) : '' ?>
                </h5>
                <br /><?= !empty($category_tasks) ? ('<i>' . $category_tasks->content . '</i>') : '' ?>
            </div>
            <div class="task-info task-info-user-logged-time">
                <h5>
                    <?php
                    $dtOrder = $task->order_id ? get_table_where('tbl_orders', ['id' => $task->order_id], '', 'row_array', '', 'reference_no') : null;
                    ?>
                    <i class="fa fa-bandcamp task-info-icon fa-fw fa-lg" aria-hidden="true"></i>Đơn đặt hàng: <?= ($dtOrder['reference_no'] ?? '') ?>
                </h5>
            </div>
            <div class="task-info task-info-user-logged-time">
                <h5>
                    <?php
                    $dtOrderpurchase_order_id = $task->purchase_order_id ? get_table_where('tblpurchase_order', ['id' => $task->purchase_order_id], '', 'row_array', '', 'prefix,code') : null;
                    ?>
                    <i class="fa fa-bandcamp task-info-icon fa-fw fa-lg" aria-hidden="true"></i>Đơn đặt hàng mua: <?= ($dtOrderpurchase_order_id['prefix'] ?? '') ?>-<?= ($dtOrderpurchase_order_id['code'] ?? '') ?>
                </h5>
            </div>
            <div class="task-info task-info-user-logged-time">
                <h5>
                    <?php
                    $dtOrderimport_id = $task->import_id ? get_table_where('tblimport', ['id' => $task->import_id], '', 'row_array', '', 'prefix,code') : null;
                    ?>
                    <i class="fa fa-bandcamp task-info-icon fa-fw fa-lg" aria-hidden="true"></i>Nhập hàng: <?= ($dtOrderimport_id['prefix'] ?? '') ?>-<?= ($dtOrderimport_id['code'] ?? '') ?>
                </h5>
            </div>
            ...
            <div class="task-info task-single-inline-wrap task-info-start-date">
                <h5><i class="fa task-info-icon fa-fw fa-lg fa-calendar-plus-o pull-left fa-margin"></i>
                    ...
                    <?php if (has_permission('tasks', '', 'edit') && $task->status != 5) { ?>
                        <input name="startdate" tabindex="-1" value="<?php echo _d($task->startdate); ?>" style="width: 120px;" id="task-single-startdate" class="task-info-inline-input-edit datetimepicker pointer task-single-inline-field">
                    <?php } else { ?>
                        <?php echo _d($task->startdate); ?>
                    <?php } ?>
                </h5>
            </div>
            <div class="task-info task-info-due-date task-single-inline-wrap<?php if (!$task->duedate && !has_permission('tasks', '', 'edit')) {
                                                                                echo ' hide';
                                                                            } ?>" <?php if (!$task->duedate) {
                ...
                <h5>
                    ...
                    <?php if (has_permission('tasks', '', 'edit') && $task->status != 5) { ?>
                        <input name="duedate" tabindex="-1" value="<?php echo _dt($task->duedate); ?>" id="task-single-duedate" class="task-info-inline-input-edit datetimepicker pointer task-single-inline-field" autocomplete="off" <?php if ($project_deadline) {
            ...
            <?php if (isset($task) && $task->total_cycles > 0) { ?>
                <div class="task-info task-info-finished">
                    <h5>
                        <i class="fa task-info-icon fa-fw fa-lg pull-left fa-check"></i>
                        <?php echo _l('cycles_passed', $task->total_cycles); ?></span>
                    </h5>
                </div>
            <?php }
            ?>
            ...
            <?php if ((has_permission('tasks', '', 'create') || has_permission('tasks', '', 'edit'))) { ?>
                <div class="task-info task-info-hourly-rate hide">
                ...
                <div class="task-info task-info-billable hide">
                ...
                <?php if (
                    ...
                    <div class="task-info task-billable-amount hide">
            ...
            <?php
            $checkZalo = $this->site_model->rowNotifiZalo($task->id, get_staff_user_id())['notif_zalo'];
            ?>
            <span class="hide">
                ...
                <div class="pull-left mtop10 mright10"><?= lang('notification_zalo') ?></div>
                <div class="view-chonse pull-left mtop10 mbot10">
                    <div class="onoffswitch">
                        <input type="checkbox" data-switch-url="<?= base_url('admin/tasks/chonseNotifiZalo') ?>" name="onoffswitch" class="onoffswitch-checkbox" id="chonse-zalo" data-id="<?= $task->id ?>" <?= $checkZalo == 1 ? 'checked' : '' ?>>
                        <label class="onoffswitch-label" for="chonse-zalo"></label>
                    </div>
                </div>
            </span>
            ...
            <!--            <hr class="task-info-separator" />-->
            <div class="clearfix"></div>
            <h4 class="task-info-heading font-normal font-medium-xs"><i class="fa fa-user-o" aria-hidden="true"></i> <?php echo _l('Người giao việc'); ?></h4>
            <div class="simple-bootstrap-select">
                <div class="task-user" data-toggle="tooltip" data-title="<?= get_staff_full_name($task->addedfrom) ?>">
                    <a href="<?= admin_url('profile/' . $task->addedfrom) ?>" target="_blank">
                        <?= staff_profile_image($task->addedfrom, array('staff-profile-image-small')) ?></a></span>
                </div>
            </div>
            <div class="clearfix"></div>
