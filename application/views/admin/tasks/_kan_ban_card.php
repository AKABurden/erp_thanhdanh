...
//    $departments_tasks_name = $CI->db->get_where('tbltask_department', ['task_id' => $task['id']])->row('list_name');
	...
	$CI->db->select('GROUP_CONCAT(tbl_room.name) as list_name');
	$CI->db->join('tbl_room', 'tbl_room.id = tbltask_department.department_id');
	$room_tasks_name = $CI->db->get_where('tbltask_department', ['task_id' => $task['id']])->row('list_name');
?>
    ...
    <div class="panel-body">
        <div class="row">
            <div class="col-md-12 task-name">
                ...
                <div><i><b style="text-transform: capitalize">Ngày bắt đầu:</b></i> <?=_dC($task['startdate'])?></div>
				<?php
//				$task_rel_value_list = get_table_where('tbldepartments_tasks', ['id' => $task['id_list_object']], '', 'row');
				...
				$row_QL = '';
                if(!empty($room_tasks_name)) {
					$row_QL .= $room_tasks_name;
				}
...
//                    $departments = $CI->db->get_where('tbltask_department', ['task_id' => $task['id']])->result_array();
					...
					$CI->db->select('tbl_room.*');
					$CI->db->join('tbl_room', 'tbl_room.id = tbltask_department.department_id');
					$rooms = $CI->db->get_where('tbltask_department', ['task_id' => $task['id']])->result_array();
