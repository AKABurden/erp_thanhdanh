...
<?php init_head(); ?>
<style>
	.div-detailed_overview .btn-default-dt-options {
		display: none;
	}
</style>
<div id="wrapper">
			...
			<div class="col-md-12">
				<div class="panel_s">
					<div class="panel-body">
						...
						<div class="row">
							<div class="col-md-2">
								<?php echo render_select('room_task[]', !empty($room) ? $room : [], ['id', 'name', 'code'], '', $room_task, ['data-none-selected-text' => _l('Phòng'), 'multiple' => true, 'onchange' => 'tasks_kanban()', 'data-actions-box' => true], [], '', '', false) ?>
							</div>	
							...
							<div class="col-md-2">
								<button type="submit" class="btn btn-info" style="margin-top:3px;"><?php echo _l('filter'); ?></button>
								<button type="button" class="btn btn-success" onclick="exportExcelDetail()" style="margin-top:3px;"><?php echo _l('excel'); ?></button>
							</div>
						</div>
						<?php echo form_close(); ?>
					</div>
				</div>
				<div class="panel_s div-detailed_overview">
					<div class="panel-body">
...
<script>
	function exportExcelDetail() {
		...
		dataPOST['member'] = $('#member').val();
		dataPOST['month'] = $('#month').val();
		dataPOST['status'] = $('#status').val();
		dataPOST['year'] = $('#year').val();
		dataPOST['room_task'] = $('select[name="room_task[]"]').val();
		dataPOST['export_excel'] = 1;

		$.ajax({
			type: "POST",
			url: site.base_url + 'admin/tasks/export_excel_detailed_overview',
			data: dataPOST,
			dataType: "json",
