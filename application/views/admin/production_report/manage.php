    ...
    <div class="panel_s mbot10 H_scroll" id="H_scroll">
        <div class="panel-body _buttons">
            ...
            <div class="pull-right mright5 H_border">
                ...
                <?php } ?>
                <a href="javascript:void(0)" onclick="exportExcel()" class="btn btn-info H_action_button"><?= lang('Xuất excel') ?></a>
                <a href="<?=admin_url('production_report/modal_excel')?>" class="c_modal btn btn-info H_action_button"> <i class="fa fa-upload" style="display: initial;" aria-hidden="true"></i> <?= lang('Import excel') ?></a>
            </div>
        </div>
    </div>
    <div class="content">
...
<script>
    ...
    $(function() {
        ...
        TableData.columns(24).visible(false, false);
    ...
    function exportExcel() {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/production_report/export_excel',
            data: {
    ...
    function updatedStatusAll(id, process_id, status) {

        var r = confirm("<?php echo _l('Bạn có muốn duyệt'); ?>");
        if (r == false) {
            TableData.draw('page');
            return false;
        } else {
            ...
            jQuery.ajax({
                type: "post",
                url: "<?= admin_url() ?>production_report/updatedStatusAll",
                data: dataString,
                cache: false,
                success: function(response) {
    ...
    function updatedStatus(id, process_id, status) {
        ...
        if (r == false) {
            ...
            jQuery.ajax({
                type: "post",
                url: "<?= admin_url() ?>production_report/updatedStatus",
                data: dataString,
                cache: false,
                success: function(response) {
                    response = JSON.parse(response);
