                    ...
                    <div class="panel-body">
                        <div class="col-md-12">
                            ...
                            <div class="table-responsive">
                                <table id="table-moderation_production_order" class="table dt-tnh table-moderation_production_order-new" style="width: 100%;">
                                    <thead>
                                        <tr>
                                            <th class="text-center"><?= lang('STT') ?></th>
                                            <th class="text-center"><?= lang('Ngày Lập Phiếu') ?></th>
                                            <th class="text-center"><?= lang('Mã Phiếu Yêu Cầu Mở Lệnh') ?></th>
...
<script>
    ...
    oTable = tnhInitDataTable('#table-moderation_production_order',
        '<?= site_url('admin/moderation_production_order/getModerationProductionOrder') ?>', {
            'order': [
                [0, 'desc']
            ],
            scrollX:true,
            "ajax": {
                "url": '<?= site_url('admin/moderation_production_order/getModerationProductionOrder') ?>',
                "type": "POST",
                "data": function(d) {
    ...
    $('#table-moderation_production_order').on('draw.dt', function(event) {
        init_datepicker();
    });
    ...
    function updateDate(_this, id,name) {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/moderation_production_order/updateDate',
            data: dataPOST,
            dataType: "json",
    ...
    function exportExcel() {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/moderation_production_order/exportExcel',
            data: {
