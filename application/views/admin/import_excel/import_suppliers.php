                ...
                <div class="col-md-12">
                    <div class="text-danger mbot10">
                        <div>
                            ...
                            <a href="<?= base_url('file/Mẫu import NCC.xlsx?vs=1.1') ?>" title=""><?= lang('tnh_file_sample') ?></a>
                        </div>
                    </div>
                    <?php
                    ...
                    ?>
                    <?php 
                        $colum_info_suppliers = get_fields_import_excel_hau();
                    ?>
                    <?php
                    if (!empty($colum_info_suppliers)) {
                        $colum_info_suppliers['colum_client'] = array_merge($colum_info_suppliers['colum_client'], ['fullname_contact', 'phone_contact', 'email_contact']);
                    }

                    if (!empty($colum_info_suppliers['colum_client'])) {
                    ...
                    <div class="col-md-4">
                        <div class="radio radio-primary">
                            <input type="radio" name="action_check" class="checkActive" id="check_add" value="1" checked>
                            <label for="check_add"><?= _l('cong_add') ?></label>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="radio radio-primary">
                            <input type="radio" name="action_check" class="checkActive" id="check_update" value="2">
                    ...
                    <div class="col-md-6">
                        <div class="form-plus-footer mtop20">
                            <button type="button" class="btn btn-info btn-addRow" style="display: none; pointer-events: none;">
                                <i class="fa fa-plus-circle" aria-hidden="true"></i> <?= _l('cong_add_colum') ?>
                            </button>

                            <button type="button" class="btn btn-success btn-addRow-all" style="display: none; pointer-events: none;" data-loading-text="<i class='fa fa-spinner fa-spin '></i> <?= _l('cong_createing_colum') ?>">
...
<script>
    ...
    $('body').on('change', 'select.fieldsColums', function(e) {
        ...
        var ListArray = ['city', 'district', 'ward', 'groups_in', 'addedfrom', 'default_currency', 'cost_id'];
        ...
        $.each(ListArray, function(i, v) {
            if (ValFields == v) {
                ...
                if (ValFields == 'city' || ValFields == 'district' || ValFields == 'ward' || ValFields == 'addedfrom' || ValFields == 'default_currency') {
    ...
    $('body').on('click', '.btn-addRow-all', function(e) {
        ...
        setTimeout(function() {
            ...
            $.each(fieldsColumsItem, function(iKey, vVal) {
                ...
                var ListArray = ['city', 'district', 'ward', 'groups_in', 'addedfrom', 'default_currency', 'discount_id', 'cost_id'];
                $.each(TypeFieldForm, function(i, v) {
                    ListArray.push(i);
                })
                $.each(ListArray, function(i, v) {
                    if (vVal.id == v) {
                        ...
                        row_items.find('.checkbox').removeClass('hide');
                        if (vVal.id == 'city' || vVal.id == 'district' || vVal.id == 'ward' || vVal.id == 'addedfrom' || vVal.id == 'default_currency' || vVal.id == 'discount_id') {
                            row_items.find('.checkbox').not('.cbobox').addClass('hide');
                        }
                        return false;
                    }
