...
<div class="modal-dialog modal-lg" style="width: 70%;">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title"><?= lang('Xem giữ kho TP (trên truyền)') ?></h4>
        </div>
...
<script>
    ...
    function rowChildKeepStock(_this, c_transfer_id) {
        ...
        if ( row.child.isShown() ) {
            ...
            if (c_transfer_id) {
                $.ajax({
                    type: "GET",
                    url: site.base_url+'admin/manufactures_temp/loadKeepStockProducts',
                    data: {
    ...
    function deleteTransferToPlan(c_transfer_id) {
        bootbox.confirm("Bạn có muốn xóa giữ kho TP (Trên truyền) ?", function(result){
            if (result == true) {
                $.ajax({
                    type: "POST",
                    url: site.base_url+'admin/manufactures_temp/deleteTransferToPlanProducts',
                    data: {
                        transfer_id: c_transfer_id,
                        '<?php echo $this->security->get_csrf_token_name(); ?>':'<?php echo $this->security->get_csrf_hash(); ?>'
                    },
    ...
    $(document).ready(function() {
        oTableKeepStockMaterials = tnhInitDataTable('#table-view-keep-stock-material', '<?= site_url('admin/manufactures_temp/getKeepStockProducts') ?>', {
            'order': [
                [1, 'desc']
            ],
            // 'fixedHeader': {
            ...
            'responsive': true,
            "ajax": {
                "url": '<?= site_url('admin/manufactures_temp/getKeepStockProducts') ?>',
                "type": "POST",
                "data": function(d) {
