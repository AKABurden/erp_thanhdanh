...
<?php echo form_open('admin/manufactures/handlingProductsStep/', array('id' => 'handling_products_step')); ?>
<div class="modal-dialog modal-lg modal-semi" style="width: 65%;">
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <div class="row">
                <div class="col-md-12">
                    <table id="tb-handling-products" class="table dataTable" style="min-width: 100%;">
                        <thead>
                            <tr>
                                <th class="text-center" style="width: 350px;" ><?= lang('products') ?></th>
...
<script>
        ...
        for (iSemiProducts = 0; iSemiProducts < nSemiProducts; iSemiProducts++) {
            ...
            if (subMaterials.length > 0) {
                ...
                $.each(subMaterials, function(iSub, vSub) {
                    ...
                    is_single_use = $(vSub).find('.is_single_use').val();
                ...
                $.each(subMaterials, function(iSub, vSub) {
                    ...
                    if (subW.length > 0) {
                        $.each(subW, function (iSW, vSW) { 
                            ...
                            json_w_default = $(vSW).find('.data-json-w-default').val();
    ...
    function rowSubWarehouses(tempCounterWarehouses, tempCounterSemiProduct, quantity_sub_warehouses = 0, tempCounterSub, dataW = '') {
        return `
            <div class="flex-center">
                <div style="width: 60%;">
                    <input type="hidden" class="form-control data-json-w-default" value='${dataW}'>
                ...
                <tr class="not-tr not-border" tr-sub-materials="${tempCounterSemiProduct}" tr-counterSemiProduct="${tempCounterSemiProduct}">
                    <td class="" style="width: 200px; border-left: 1px solid #cedae6 !important; border-bottom: 0px !important;">
                        ...
                        <div style="padding-left: 40px;">
                            <input type="text" name="items[${tempCounterSemiProduct}][materials][${tempCounterSubAuto}][quantity_single]" onchange="totalProducts(true)" class="form-control number-format quantity_single" value="${(value.quantity_single)}">
                        </div>
    ...
    function rowDataProducts(dtData) {
        ...
        if (dtData) {
            ...
            tdName = `<div class="td-name bold text-primary" style="font-size: 14px;">
                ${semi_product_name}(${semi_product_code})
            </div>`;
            ...
            if (dtData.subItems && dtData.subItems.length > 0) {
                ...
                $.each(subItems, function(index, value) {
                    ...
                    tdName = `
                        <div class="td-name text-left italic">
                            ${value.item_name}(${value.item_code})
                            ${typeof value['stage_name'] !== 'undefined' ? `<div class="mtop5"><span class="label label-success">${value['stage_name']}</span></div>` : ''}
                        </div>
                        <input type="hidden" class="form-control item_cs_id" value="${value.item_cs_id}">
                    ...
                    if (arrW != null && arrW.length > 0) {
                        $.each(arrW, function (iW, vW) { 
                            if (typeof vW.warehousePlan !== 'undefined' && vW.warehousePlan != null) {
                                jsonString = JSON.stringify(vW.warehousePlan);
                                tempQuantity = cQuantityNeedHold;
                                vW.warehousePlan.product_quantity = formatDecimal(vW.warehousePlan.product_quantity * value.quantity_exchange);

                                if (cQuantityNeedHold > 0) {
                                    cQuantityNeedHold = cQuantityNeedHold - vW.warehousePlan.product_quantity;
                                    if (cQuantityNeedHold > 0) {
                                        quantityW = vW.warehousePlan.product_quantity;
                                    } else {
                                        quantityW = tempQuantity;
                                    }

                                    if (quantityW >= 0) {
                                        trWarehouses+= rowSubWarehouses(counterWarehouses, counterSemiProduct, quantityW, counterSub, jsonString);
                                        counterWarehouses++;
                                    }
                                }
                            }
                            ...
                            // jsonString = JSON.stringify(vW.warehousePlan);
                            // trWarehouses+= rowSubWarehouses(counterWarehouses, counterSemiProduct, value.quantity, counterSub, jsonString);
                            // counterWarehouses++;
                        });
                    }
                    ...
                    trSub += `
                        <tr class="not-tr not-border" tr-sub-materials="${counterSemiProduct}" tr-counterSemiProduct="${counterSemiProduct}">
                            <td class="" style="width: 350px; border-left: 1px solid #cedae6 !important; border-bottom: 0px !important;">
                                <div class="flex-center" style="padding-left: 40px;">
                                ${tdImages} ${tdName}
                                </div>
                            ...
                            <td style="border-right: 1px solid #cedae6 !important; border-bottom: 0px !important;width: 200px;">
                                <div class="">
                ...
                trItems+= `<tr data-semiproduct="${dtData.product_id}" tr-counterSemiProduct="${counterSemiProduct}">
                    <td colspan="4">
                        <div class="text-danger text-left italic tag-not-stage"><?= lang('Chưa tôn tại thành phẩm trước đó') ?></div>
                    </td>
                </tr>`;
                isBom = false;
                counterSubAuto = 0;
    ...
    function loadALLProducts() {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/manufactures/loadALLProductsStep',
            data: data,
            dataType: "json",
    ...
    $(function() {
        ...
        appValidateForm($('#handling_products_step'), {
            warehouses_products: 'required'
        }, saveHandlingProductsStep);

        function saveHandlingProductsStep(form) {
