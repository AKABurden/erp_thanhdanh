            ...
            <div class="col-md-12">
                <div id="search-tnh" class="collapse in" aria-expanded="true">
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="text" name="start_date" placeholder="<?= lang('start_date') ?>"
                            id="start_date" class="start_date datepicker form-control"
                            style="width: 100%;" value="">
                    </div>
                    <div class="col-md-2">
                        <?= lang('end_date', 'end_date') ?>
                        <input type="text" name="end_date" placeholder="<?= lang('end_date') ?>"
                            id="end_date" class="end_date datepicker form-control" style="width: 100%;"
                            value="">
                    </div>
                </div>
            </div>
...
$(document).ready(function() {
            ...
            'fnServerData': function(sSource, aoData, fnCallback) {
                ...
                $.ajax({
                    ...
                    'success': function(response) {
                        ...
                        // $('.count_not_keep').html('(' + tnhFormatNumber(response.count_not_keep) +
                        //     ')');
                        fnCallback(response);
                    }
                });
            ...
            "columnDefs": [{
                ...
                },
                {
                    "render": function(data, type, row) {
                        ...
                        if (row[7]) {
                            db = row[7].split('custom');
                            titletoolTip = row[7].replace(/custom/g, "</br>");
                            countData = db.length;
                ...
                },
                {
                    "render": function(data, type, row) {
                        ...
                        if (data[3] > 0) {
                            imagesPurchases =
                                `<a class="tnh-modal" style="position: relative;" data-tnh="modal" href="${site.base_url}admin/manufactures_temp/view_sugg/${row[0]}" data-toggle="modal" data-target="#myModal"><img class="images-status" src="${site.base_url+'assets/images/shopping-bag_1.png'}"><span class="label icon-total-cs bg-warning">${data[3]}</span><a>`;
                        } else {
                            imagesPurchases =
                                `<a href="javascript:void(0)"><img class="images-status" src="${site.base_url+'assets/images/shopping-bag_4.png'}"></a>`;
                        ...
                        if (data[2] > 0) {
                            imagesTranferBusiness =
                                `<a class="tnh-modal" style="position: relative;" data-tnh="modal" href="${site.base_url}admin/manufactures_temp/view_tranfer_business/${row[0]}" data-toggle="modal" data-target="#myModal"><img class="images-status" src="${site.base_url+'assets/images/pallet_1.png'}"><span class="label icon-total-cs bg-warning">${data[2]}</span><a>`;
                        } else {
                            imagesTranferBusiness =
                                `<a href="javascript:void(0)"><img class="images-status" src="${site.base_url+'assets/images/pallet_4.png'}"></a>`;
                        ...
                        // <div data-toggle="tooltip" class="pull-left pointer status-icon-text" title="<?= lang('Giữ kho TP') ?>">${imagesTranferBusiness}<span><?= lang('Giữ kho') .'<br> TP' ?></span></div>
                        htmlStatus = `<div data-toggle="tooltip" class="pull-left pointer status-icon-text" style="margin-right: 5px;" title="<?= lang('tnh_keep_stock') ?>">${imagesKeepWarehouses}<span><?= lang('tnh_keep_stock') ?></span></div>
                        <div data-toggle="tooltip" class="pull-left pointer status-icon-text" title="<?= lang('tnh_export_other') ?>">${imagesPurchases}<span><?= lang('tnh_export_other') ?></span></div>`;
                        // row[11]+
                        return htmlStatus;
                    },
                    "targets": 9,
                ...
                {"targets": 12, "name": 'status_manufactures', 'visible': false, 'searchable': false, 'sortable': false},
                {
                    "targets": 13,
                    "name": 'actions',
                    'searchable': false,
    ...
    $(document).on('change', '#orders_search, #business_plan_search, #start_date, #end_date, #start_date_search, #end_date_search, #products_search, #productions_orders_search', function(event) {
        oTable.draw();
    });
