...
<div id="wrapper">
    ...
    <div class="content">
        <div class="row">
                                        ...
                                        <div class="container-fluid">
                                            <div class="row">
                                                <div style="height: 118px;" class="col-md-4">
                                                    <div class="card text-white bg-warning o-hidden h-100">
                                                        <div class="card-body">
                                                            ...
                                                            <h3 class="mr-5"> Hoá Đơn Bán Hàng <br><span style="font-size: 16px" id="count_coupon_invoice" id2="count_purchase_invoice">0</span></h3>
                                                        </div>
                                                    </div>
                                                </div> 
                                                <div style="height: 118px;" class="col-md-4">
                                                <div class="card text-white bg-success o-hidden h-100">
                                                    <div class="card-body">
                                                    <img src="<?= base_url('assets/images/circle.svg')?>" class="card-img-absolute" alt="circle-image" />
                                                    <h3 class="mr-5">Phiếu Thu Bán Hàng<br><span style="font-size: 16px" id="vouchers_coupons">0</span></h3>
                                                    </div>
                                                </div>
                                                </div>
                                                <div style="height: 118px;" class="col-md-4">
                                                <div class="card text-white bg-primary o-hidden h-100" style="background-color: #ff4015;">
                                                    <div class="card-body">
                                                    <img src="<?= base_url('assets/images/circle.svg')?>" class="card-img-absolute" alt="circle-image" />
                                                    <h3 class="mr-5">Công Nợ <?=_l('tnh_customer')?><br><span style="font-size: 16px" id="debt_clients">0</span></h3>
                                                    </div>
                                                </div>
                                                </div>
                                            </div>
            ...
            <div class="bg-light-gray border-radius-4 hidden">
                <div class="p8">
                    <div class="form-group ">
                        <?= lang('customers') ?>
                                        ...
                                        <!-- //Top 5 Khách Hàng Mua Hàng Nhiều Nhất -->
                                            <div class="col-md-4">
                                                <div class="border-radius-4 wrap_box">
                                                    <strong><h4 class="text-center text-capitalize"><?=_l('sum_top')?> 5 <?=_l('tnh_customer')?> Có Công Nợ Nhiều Nhất</h4></strong>
                                                    <hr/>
                                                    <div>
                                                    <div style="float: left; padding: 0 0 0 10px;">
                                                        <span style="font-size: 10px;">Tên <?=_l('tnh_customer')?></span>
                                                    </div>
                                                    <div style="float: right; padding: 0 10px 0 0;">
                                                        <span style="font-size: 10px;"><?=_l('debt')?></span>
                                                    </div>
                                                    <div class="clearfix"></div>
                                                    </div>
                                                    <br/>

                                                    <div class="pay_slips" id="top_pay_slips"></div>
                                                </div>
                                            </div>
<div class="col-md-4">
    <div class="border-radius-4 wrap_box">
        <strong><h4 class="ttext-center text-capitalize">Top 5 khách hàng mua nhiều</h4></strong>
        <hr/>
    <div>
    ...
    <div class="top_client"></div>
                                        ...
                                        <!-- Top 5 Khách Hàng Mua Hàng Nhiều Nhất -->
                                            <div class="col-md-6 hidden">
                                                <div class="border-radius-4 wrap_box">
                                                    <strong><h4 class="text-center text-capitalize"><?=_l('sum_top')?> 5 <?=_l('tnh_customer')?> mua hàng nhiều nhất</h4></strong>
                                                    <hr/>
                                                <div>
                                                    <div style="float: left; padding: 0 0 0 10px;">
                                                        <span style="font-size: 10px;">Tên <?=_l('tnh_customer')?></span>
                                                    ...
                                                    <div style="float: right; padding: 0 10px 0 0;">
                                                        <span style="font-size: 10px;"><?=_l('total_price')?> ( <?=_l('sum_vnd')?> ) </span>
                                                    </div>
                                                    <div style="float: right; padding: 0 10px 0 0;">
                                                        <span style="font-size: 10px;"><?=_l('total_price')?> ( <?=_l('currency_lang')?> ) </span>
                                                    </div>
                                                    <div class="clearfix"></div>
                                                    <!-- </div> -->
                                                    <br/>
                                                    <div class="top_supplier_imports" id="top_supplier_imports"></div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="clearfix"></div>
...
function get_total_limit() {
        ...
        jQuery.ajax({
            type: "post",
            url: "<?=admin_url()?>dashboard_debt_selling/count_all/",
            data: dataString,
            cache: false,
            success: function (data) {
                data = JSON.parse(data);
                $('#count_coupon_invoice').html(data.orders);
                $('#vouchers_coupons').html(data.vouchers_coupons);
                $('#debt_clients').html(data.debt_clients);
            console.log(data);
            },
    ...
    function top_count_purchase_invoice() {
        var months_report = $('select[name="months-report"]').val();
        var report_to = $('input[name="report-to"]').val();
        var report_from = $('input[name="report-from"]').val();
        var customers_ch = $('[name="customers_ch"]').val();
        ...
        jQuery.ajax({
            type: "post",
            url: "<?=admin_url()?>dashboard_debt_purchase/top_count_purchase_invoice/",
            data: dataString,
            cache: false,
            success: function (data) {
              data = JSON.parse(data);
              $('.top_count_purchase_invoice').html(data);
              }
        });
    }
    function top_pay_slips() {
        var months_report = $('select[name="months-report"]').val();
        var report_to = $('input[name="report-to"]').val();
        var report_from = $('input[name="report-from"]').val();
        var customers_ch = $('[name="customers_ch"]').val();
        ...
        jQuery.ajax({
            type: "post",
            url: "<?=admin_url()?>dashboard_debt_selling/top_pay_slips/",
            data: dataString,
            cache: false,
            success: function (data) {
              data = JSON.parse(data);
              $('#top_pay_slips').html(data);
              }
        });
    }
    function top_supplier_imports() {
        var months_report = $('select[name="months-report"]').val();
        var report_to = $('input[name="report-to"]').val();
        var report_from = $('input[name="report-from"]').val();
        var customers_ch = $('[name="customers_ch"]').val();
        ...
        jQuery.ajax({
            type: "post",
            url: "<?=admin_url()?>dashboard_debt_purchase/top_supplier_imports/",
            data: dataString,
            cache: false,
            success: function (data) {
              data = JSON.parse(data);
              $('#top_supplier_imports').html(data);
            }
        });
    }
    // top_count_purchase_invoice();
    // top_items();
    top_pay_slips();
    top_supplier_imports();
    top_client();
    // top_items_bestsell();
    // top_product_bestsell();
    get_total_limit();
    var myLineChart;
    var myCotChart;
    ...
    //     var customers_ch = $('[name="customers_ch"]').val();
    //     dataString = {[csrfData['token_name']] : csrfData['hash'],months_report:months_report,report_from:report_from,report_to:report_to,search_id_staff:search_id_staff,customers_ch:customers_ch};
    //     $.post(admin_url + 'dashboard_debt_purchase/dashboard_report_cot/',dataString, function(response) {
    ...
    //     var customers_ch = $('[name="customers_ch"]').val();
    //     dataString = {[csrfData['token_name']] : csrfData['hash'],months_report:months_report,report_from:report_from,report_to:report_to,search_id_staff:search_id_staff,customers_ch:customers_ch};
    //     $.post(admin_url + 'dashboard_debt_purchase/dashboard_report/',dataString, function(response) {
    //     var response = JSON.parse(response);
