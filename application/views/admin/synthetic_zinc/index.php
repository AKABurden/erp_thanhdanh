            ...
            <div class="col-md-12">
                <div class="panel_s">
                    <div class="panel-body p-top-0">
                        ...
                        <table id="table-synthetic-zinc" class="table table-hover table-condensed table-synthetic-zinc dont-responsive-table dataTable" style="width: 100%;">
                            <thead>
                            <tr>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('change', '#productions_orders_search, #items_search, #start_date_search, #end_date_search, #orders_search, #business_plan_search, #period_time, #orders_and_business_plan, #type_print_search, #customer_search', function() {
            oTableNew.draw();
        });
    ...
    function loadTable() {
        oTableNew = tnhInitDataTable('#table-synthetic-zinc', '<?= site_url('admin/synthetic_zinc/getSyntheticZinc') ?>', {
            'order': [
                [1, 'desc']
            ],
            // 'fixedHeader': {
            ...
            // 'responsive': true,
            "ajax": {
                "url": '<?= site_url('admin/synthetic_zinc/getSyntheticZinc') ?>',
                "type": "POST",
                "data": function(d) {
                ...
                "dataSrc": function(json) {
                    $('#table-productions-orders-all tfoot tr td:nth-child(7)').html('<div class="text-center">' + tnhFormatNumber(json.total_quantity) + '</div>');
                    $('#table-productions-orders-all tfoot tr td:nth-child(8)').html('<div class="text-center">' + tnhFormatNumber(json.total_quantity_sx) + '</div>');
                    $('#table-productions-orders-all tfoot tr td:nth-child(9)').html('<div class="text-center">' + tnhFormatNumber(json.total_quantity_hold) + '</div>');
                    $('#table-productions-orders-all tfoot tr td:nth-child(10').html('<div class="text-center">' + (json.total_quantity_new) + '</div>');
                    $('#table-productions-orders-all tfoot tr td:nth-child(11)').html('<div class="text-center">' + tnhFormatNumber(json.total_quantity_finished) + '</div>');
                    $('#table-productions-orders-all tfoot tr td:nth-child(12)').html('<div class="text-center">' + tnhFormatNumber(json.total_quantity_errors) + '</div>');
                    return json.aaData;
                }
            },
            "columnDefs": [{
