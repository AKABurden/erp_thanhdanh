<div class="modal-dialog modal-lg" style="min-width: 80%;">
    <?php echo form_open(admin_url('suggest_pccc/detail/' . $id),
        ['id' => 'suggest_pccc']); ?>
                ...
                <div class="col-md-12">
                    <table id="tb-maintenance-machines" class="table dataTable">
                        ...
                        <tr>
                            ...
                            <th class="text-center"><?= lang('Quy định PCCC') ?></th>
                                    ...
                                    <tr class="rowID_<?=$rowID?> is_<?=$rowID?>_<?= $value['machines_maintenance_id'] ?>">
                                        ...
                                        <td>
                                            <div>
                                                ...
                                                <input type="hidden" class="suggest_pccc_item_id"
                                                       name="suggest_pccc_item_id[<?= $counter ?>]"
                                                       value="<?= $value['id'] ?>">
...
<script type="text/javascript">
    ...
    ajaxSelectParams('#machines_id', 'admin/suggest_pccc/search_suggest_repalce', '', true, true);
    $("#branch_id").select2();
    ...
    $('#suggest_pccc').on('change', '.check_items_all', function () {
    ...
    $('#suggest_pccc').on('click', '.event-plus', function() {
        var TrClass = $(this).attr('data-class');
    ...
    $("#machines_id").change(function () {
        ...
        if (string_id) {
            $.ajax({
                url: site.base_url + 'admin/suggest_pccc/getMaintenaceMachinesDetail',
                type: 'POST',
                dataType: 'json',
                data: {
                    ...
                    if (data.dtMaintenaceMachines.length > 0) {
                        ...
                        alert_float('danger', 'Thiết bị/Khu vực không có danh mục kiểm tra PCCC');
                    }
                    $("#machines_id").val('');
                    ajaxSelectParams('#machines_id', 'admin/suggest_pccc/search_suggest_repalce', '', true, true);
                })
