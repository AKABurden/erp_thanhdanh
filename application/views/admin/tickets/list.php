...
<div id="wrapper">
    <div class="panel_s mbot10 H_scroll" id="H_scroll">
        <div class="panel-body _buttons">
            ...
            <a href="#" class="btn btn-default btn-with-tooltip pull-right mleft4" data-toggle="tooltip"
            ...
            <div class="pull-right mright5 H_border">
                <a href="<?php echo admin_url('tickets/add'); ?>" class="btn btn-info H_action_button">
...
<script>
    ...
    var inner_popover_template_status = '<div class="popover"><div class="arrow"></div><div class="popover-inner"><h3 class="popover-title"></h3><div class="popover-content"></div></div></div>'; 
    $(document).on('click','.task-menu-status',function(e){
          $(this).popover({
           html: true,
           placement: "bottom",
           trigger: 'click',
    ...
    function mark_as_status(status, id) {
        var data = {};
        if (typeof(csrfData) !== 'undefined') {
          data[csrfData['token_name']] = csrfData['hash'];
        }
        $.post(admin_url+'tickets/update_status/'+status+'/'+id, data).done(function(response){
            response = JSON.parse(response);
            alert_float(response.alert_type,response.message);
            $('.table-tickets').DataTable().ajax.reload();
            $('.task-menu-status').popover('hide');
        });
    }
</script>
</body>
