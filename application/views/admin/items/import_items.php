                ...
                <div class="panel_s">
                    <div class="panel-body">
                        <div class="text-danger">
                            ...
                            <!-- <div>Cập nhật dựa vào cột mã để cập nhật, các trường có thể cập nhật được(Quy cách NPL)</div> -->
                            <div>Cập nhật dựa vào cột mã để cập nhật</div>
                            <div>
                                - <?= lang('tnh_please_download_template_sample') ?>:
                                <a href="<?= base_url('file/Mẫu NPL.xlsx?vs=1.4') ?>" title=""><?= lang('tnh_file_sample') ?></a>
                            </div>
                        </div>
                        <hr class="hr-panel-heading" />
                        <?php
                        $template_import = get_table_where('tbltemplate_import', ['type' => 'nvl']);
                        ?>
...
<script type="text/javascript">
    ...
    function checkWarning(field, row, checked = '',checked1 = '')
    {
        ...
        if (field == "code") {
        ...
        } else if (field == "exchange_unit") {
            // row.find('.td-note').html(textErrors('<?= lang('tnh_enter_code_required') ?>'));
            // row.find('.td-options').html(radioChosse('exchange_unit'));
        } else if (field == "standard_unit") {
            row.find('.td-note').html(textErrors('<?= lang('tnh_enter_code_required') ?>'));
            row.find('.td-options').html(radioChosse('standard_unit',checked, checked1));
        } else if (field == "unit_payment") {
            row.find('.td-note').html(textErrors('<?= lang('tnh_enter_code_required') ?>'));
            row.find('.td-options').html(radioChosse('unit_payment',checked, checked1));
        } else if (field == "recipe") {
            row.find('.td-note').html(textErrors('<?= lang('1: mặc định, 2: khổ, 3: dài rộng') ?>'));
            ...
            row.find('.td-note').html(textErrors('<?= lang('Mã NCC hoặc tên và phải có trong phần mềm') ?>'));
        ...
        } else if (field == "unit_manufactures") {
            row.find('.td-note').html(textErrors('<?= lang('tnh_enter_code') ?>'));
            row.find('.td-options').html(radioChosse('unit_manufactures',checked, checked1));
        } else if (field == "mode_id") {
            row.find('.td-note').html(textErrors('<?= lang('tnh_enter_code') ?>'));
        } else if (field == "id_branch") {
            row.find('.td-note').html(textErrors('<?= lang('Nhập tên chi nhánh') ?>'));
        }  else if (field == "unit_of_measure") {
            row.find('.td-note').html(textErrors('<?= lang('tnh_enter_code_required') ?>'));
            row.find('.td-options').html(radioChosse('unit_of_measure',checked, checked1));
        } else {
            row.find('.td-note').html('');
        }
    }

    function autoFieldMaterials() {
