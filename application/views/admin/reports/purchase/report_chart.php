  ...
  <div class="content-wrapper">
      <div class="container-fluid">
          ...
          <div class="col-md-4">
              ...
              <div id="date-range_ch" class="hide mbot15">
                  <div class="row">
                      <div class="col-md-6">
                      ...
                      <div class="col-md-6">
                          <label for="report-to_ch" class="control-label"><?php echo _l('report_sales_to_date'); ?></label>
                          <div class="input-group date">
                              <input type="text" class="form-control datepicker" disabled="disabled" id="report-to_ch" name="report-to_ch">
                              <div class="input-group-addon">
                                  <i class="fa fa-calendar calendar-icon"></i>
                              </div>
                          </div>
          ...
          <div class="col-md-8">
              <div class="border-radius-4 wrap_box">
                  <strong><a onclick="view_detail_ncc(); return;">
                          <h4 class="text-center">Top 5 nhà cung cấp mua nhiều</h4>
                      </a></strong>
                  <hr />
                  <div>
                      <div style="float: left; padding: 0 0 0 10px;">
                          <span style="font-size: 10px;"><?= _l('supplier') ?></span>
                      </div>
  ...
  <script type="text/javascript">
      $('select[name="months-report_ch"]').on('change', function() {
          var val = $(this).val();
          $('input[name="report-to_ch"]').val('');
          $('input[name="report-from_ch"]').val('');
          if (val == 'custom') {
              $('#date-range_ch').addClass('fadeIn').removeClass('hide');
              return;
          } else {
              if (!$('#date-range_ch').hasClass('hide')) {
                  $('#date-range_ch').removeClass('fadeIn').addClass('hide');
              }
          }
      ...
      function get_total_limit() {
          ...
          var report_to = $('input[name="report-to_ch"]').val();
      ...
      function top_suppplier() {
          ...
          jQuery.ajax({
              type: "post",
              url: "<?= admin_url() ?>reports/top_suppplier/",
              data: dataString,
              cache: false,
              success: function(data) {
                  data = JSON.parse(data);
                  $('.top_suppplier').html(data);
              }
          });
      }
      init_dashboard_report();
