...
$aColumns = [
    ...
    '(tblwarehouse_product.price * tblwarehouse_product.product_quantity_payment_left) as total_price',
];
$sIndexColumn = 'id';
...
foreach ($rResult as $r => $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'tblwarehouse_product.price') {
            $_data = '<div class="text-right">' . formatNumber($aRow['tblwarehouse_product.price']) . '</div>';
        }
        if ($aColumns[$i] == '(tblwarehouse_product.price * tblwarehouse_product.product_quantity_payment_left) as total_price') {
...
$output['title_excel'] = ['SỐ NGÀY CẢNH BÁO HẾT HẠN: ' . get_option('limit_date')];
$type_limit_date = $this->ci->input->post('type_limit_date');
$output['title_excel'][1] = 'TÌNH TRẠNG HẠNG SỬ DỤNG : TẤT CẢ';
if($type_limit_date == 1) {
    $output['title_excel'][1] = 'TÌNH TRẠNG HẠNG SỬ DỤNG : SẮP HẾT HẠN';
}
else if($type_limit_date == 2) {
    $output['title_excel'][1] = 'TÌNH TRẠNG HẠNG SỬ DỤNG : QUÁ HẠN';
}
