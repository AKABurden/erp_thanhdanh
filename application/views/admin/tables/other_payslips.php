    ...
    if ($this->ci->input->post('type') == 'import'){
        array_push($where, 'AND tblother_payslips.type_vouchers = 1');
    } else {
        array_push($where, 'AND tblother_payslips.type_vouchers != 1');
    }
    ...
    foreach ($rResult as $aRow) {
        ...
        for ($i = 0; $i < count($aColumns); $i++) {
            ...
            if ($aColumns[$i] == 'tblcosts.name') {
                if ($aRow['type_new'] == 1){
                    $this->ci->db->dbprefix = '';
                    $this->ci->db->select("tblcosts.name,
                    tblcosts.id,tblcosts.type_cost,tblcosts.object_id
                    ");
                    $this->ci->db->from('tblother_payslip_cost');
                    $this->ci->db->join('tblcosts','tblcosts.id = tblother_payslip_cost.cost_id');
                    $this->ci->db->join('tbl_machines','tbl_machines.id = tblcosts.object_id AND tblcosts.type_cost = 2','left');
                    $this->ci->db->join('tbl_category_payslip','tbl_category_payslip.id = tblcosts.object_id AND tblcosts.type_cost = 3','left');
                    $this->ci->db->join('tbl_category_items','tbl_category_items.id = tblcosts.object_id AND tblcosts.type_cost = 1','left');
                    $this->ci->db->where('tblother_payslip_cost.other_payslip_id',$aRow['id']);
                    $dtCost = $this->ci->db->get()->result_array();
                    $htmlCost = "";
                    if (!empty($dtCost)){
                        foreach ($dtCost as $k => $v){
                            $type_cost = $v['type_cost'];
                            $object_id = $v['object_id'];
                            $name_object = '';
                            if ($type_cost == 1){
                                $this->ci->db->from('tbl_category_items');
                                $this->ci->db->where('tbl_category_items.id',$object_id);
                                $dtObject = $this->ci->db->get()->row_array();
                                if(!empty($dtObject)){
                                    $name_object = $dtObject['name'];
                                }
                            } elseif ($type_cost == 2){
                                $this->ci->db->from('tbl_machines');
                                $this->ci->db->where('tbl_machines.id',$object_id);
                                $dtObject = $this->ci->db->get()->row_array();
                                if(!empty($dtObject)){
                                    $name_object = $dtObject['name'];
                                }
                            } elseif ($type_cost == 3){
                                $this->ci->db->from('tbl_category_payslip');
                                $this->ci->db->where('tbl_category_payslip.id',$object_id);
                                $dtObject = $this->ci->db->get()->row_array();
                                if (!empty($dtObject)){
                                    $name_object = $dtObject['name'];
                                }
                            }
                            $htmlCost .= '<div>'.'-'.$v['name'].'('.$name_object.')'.'</div>';
                        }
                    }
                    $_data = $htmlCost;
                } else {
                            ...
                            if (!empty($import)) {
                                if (!empty($import['internal_proposal'])) {
                                    ...
                                    if (!in_array($internal_proposal[0], $internal_proposal_array)) {
                                        ...
                                        $_data .= '<a class="c_modal" href="' . (admin_url('internal_proposal/view/' . $internal_proposal[0])) . '">' . $internal_proposal[1] . '</a>,<br>';
                                    }
                                }
                                if (!empty($import['internal_proposalv2'])) {
                                    ...
                                    if (!in_array($internal_proposalv2[0], $internal_proposal_array)) {
                                        ...
                                        $_data .= '<a class="c_modal" href="' . (admin_url('internal_proposal/view/' . $internal_proposalv2[0])) . '">' . $internal_proposalv2[1] . '</a>,<br>';
                                    }
                                }
                            }
                        }
                        if ($value->id_import) {
                            $import = get_table_where('tblimport', array('id' => $value->id_import),'','row_array');
                            if(!empty($import)){
                            ...
                            // $import = get_table_where('tblpurchase_order', array('id' => $value->id_service), '', 'row_array', '', 'tblpurchase_order.*,(SELECT CONCAT(tblinternal_proposal.id, "__", tblinternal_proposal.code) FROM tblinternal_proposal WHERE tblinternal_proposal.id_suggestion = tblsuggestion.id LIMIT 1) as internal_proposal,(SELECT CONCAT(tblinternal_proposal.id, "__", tblinternal_proposal.code) FROM tblinternal_proposal WHERE tblinternal_proposal.id = tblsuggestion.id_internal_proposal LIMIT 1) as internal_proposalv2');
                        }
                    }
                }
            }
