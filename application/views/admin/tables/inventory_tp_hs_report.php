...
$aColumns = [
    ...
    'tblunits.unit',
    '(IF(object_item_type = "orders",price_order,prices) * tblwarehouse_product.quantity_left) as price',
...
array_push($where, 'AND tbl_products.type_products = "products"');
...
if ($this->ci->input->post('products_id_hs')) {
    array_push($where, 'AND tblwarehouse_product.product_id = ' . $this->ci->input->post('products_id_hs'));
}
...
if (!empty($search)) {
    array_push($where, 'AND (tbl_products.name like "%' . $search . '%" OR tbl_products.code like "%' . $search . '%")');
}
...
$result  = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, ['tblwarehouse_product.product_id', 'tblwarehouse_product.id', 'type_export', 'tblwarehouse_product.date_warehouse'], $group_by, [], [], $having);
...
foreach ($rResult as $r => $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'tbl_products.code') {
            $_data = '<div style="font-size:16px">' . $aRow['tbl_products.code'] . '</div>';
        }
        if ($aColumns[$i] == 'tbl_products.name') {
            $_data = '<div style="font-size:16px">' . $aRow['tbl_products.name'] . '</div>';
        }
        ...
        if ($aColumns[$i] == '(IF(object_item_type = "orders",price_order,prices) * tblwarehouse_product.quantity_left) as price') {
            $_data = '<div class="text-right">' . number_format($aRow['price']) . '</div>';
        ...
        if ($aColumns[$i] == 'tblwarehouse_product.date_warehouse') {
            ...
            // $check_export = get_table_where('tblwarehouse_export', array('product_id' => $aRow['product_id'], 'type_items' => 'product', 'lot_code' => $aRow['lot_code'], 'date_sd' => $aRow['date_sd']), '', 'row');
            $this->ci->db->where(['product_id' => $aRow['product_id'], 'type_items' => 'product', 'lot_code' => $aRow['lot_code'], 'date_sd' => $aRow['date_sd']]);
            $this->ci->db->limit(1);
            $this->ci->db->order_by('tblwarehouse_export.id desc');
            $check_export = $this->ci->db->get('tblwarehouse_export')->row();
            // echo '<pre>';print_arrays($check_export);die;
                ...
                if ($aRow['type_export'] == 1 || $aRow['type_export'] == 3 || $aRow['type_export'] == 20) {
                    ...
                    if ($aRow['type_export'] == 2) {
                        if (!empty($aRow['import_id'])) {
                            $transfer =  get_table_where('tbltransfer_warehouse_detail', array('id_transfer' => $aRow['import_id'], 'id_items' => $aRow['product_id'], 'type' => 'product', 'lot_code' => $aRow['lot_code'], 'date_sd' => $aRow['date_sd']), '', 'row');
                            if (!empty($transfer)) {
            ...
            $_data = '<div class="text-center">' . number_format($days) . '</div>';
