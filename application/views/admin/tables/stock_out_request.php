...
$aColumns = [
    'tblstock_out_request.id as id',
    'tblstock_out_request.date as date',
    'tblstock_out_request.code as code',
    'tbl_productions_orders.reference_no as production_order_code',
    'tblstock_out_request.note as note',
    '1',
    '"" as action',
];
$sIndexColumn = 'id';
$sTable       = 'tblstock_out_request';
$join = [];
$join[] = 'LEFT JOIN tbl_productions_orders ON tbl_productions_orders.id = tblstock_out_request.production_order_id';
$join[] = 'LEFT JOIN tbl_category_recommended ON tbl_category_recommended.name_table = "tblstock_out_request"';

$where = [];
if (!empty($search_date)) {
    $searchDate = explode(' - ', $search_date);
    array_push($where, 'AND tblstock_out_request.date BETWEEN "' . to_sql_date($searchDate[0]) . ' 00:00:00" and "' . to_sql_date($searchDate[1]) . ' 23:59:59"');
}

$result  = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
    '(SELECT count(tbltasks.id) FROM tbltasks  LEFT JOIN tbl_category_recommended ON tbl_category_recommended.id = tbltasks.category_recommended_id  WHERE suggest_id = tblstock_out_request.id AND tbl_category_recommended.name_table="tblstock_out_request") as countTask',
...
foreach ($rResult as $rowIndex => $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($field == 'id') {
            ...
            $_data = '<div class="text-left" style="width: 120px"><a data-tnh="modal" class="tnh-modal" href="' . base_url('admin/stock_out_request/view/' . $aRow['id']) . '" data-toggle="modal" data-target="#myModal">' . $aRow['code'] . '</a></div>';

        } else if ($field == 'date' || $field == 'deadline') {
            $_data = '<div class="text-center">'._d($_data).'</div>';
        } else if ($field == 'action') {
            $edit = '<a href="'.admin_url('stock_out_request/submit/'.$aRow['id']).'"><i class="fa fa-edit"></i> ' . lang('edit') . ' ' . lang('stock_out_request') . '</a>';
            $delete = '<a type="button" class="po" data-container="body" data-html="true" data-toggle="popover" data-placement="left" data-content="
                    <button href=\'' . base_url('admin/stock_out_request/delete/'.$aRow['id']) . '\' class=\'btn btn-danger po-delete-json\'>' . lang('delete') . '</button>
                    <button class=\'btn btn-default po-close\'>' . lang('close') . '</button>
                    "><i class="fa fa-remove width-icon-actions"></i> ' . lang('delete') . ' ' . lang('stock_out_request') . '</a>';
