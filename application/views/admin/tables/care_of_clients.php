...
if(!empty($this->ci->input->post('procedure')))
{
    ...
    $this->ci->db->select('group_concat(id_care_of) as list_id');
    if(!empty($datestart)) {
        $this->ci->db->where('DATE_FORMAT(date_create, "%Y-%m-%d") >=', $datestart);
    }
    if(!empty($dateend))
    {
        $this->ci->db->where('DATE_FORMAT(date_create, "%Y-%m-%d") <=', $dateend);
    }
    $this->ci->db->group_by('id_care_of');
    $this->ci->db->where('status_procedure', $procedure);
    $list_id = $this->ci->db->get(db_prefix().'procedure_care_of')->row();
    if(!empty($list_id->list_id))
    {
        $where[] = 'AND '.db_prefix().'care_of_clients.id IN ('.$list_id->list_id.')';
    }
    else
    {
        $where[] = 'AND '.db_prefix().'care_of_clients.id IN (NULL)';
    }
}
if($this->ci->input->post())
{
    ...
    {
        $where[] = 'AND c.company like "%'.$this->ci->input->post('name_client').'%"';
    }

    if(!empty($this->ci->input->post('code_care_of')))
    {
        $where[] = 'AND concat('.db_prefix().'care_of_clients.prefix,'.db_prefix().'care_of_clients.code) like "%'.$this->ci->input->post('code_care_of').'%"';
    }
    if(!empty($this->ci->input->post('code_client')))
    {
        $where[] = 'AND concat(prefix_client,prefix_client) like "%'.$this->ci->input->post('code_client').'%"';
    }
...
foreach ($rResult as $aRow) {
    ...
    $row[] = '<a class="pointer" href="'.admin_url('clients/client/'.$aRow['client']).'" target="_blank">'.$aRow['company'].(!empty($aRow['fullcode_client']) ? '('.$aRow['fullcode_client'].')' : '' ).'</a>';
    $row[] = _d($aRow['date']);
...
//    $row[] = $aRow['priority']; // mức độ ưu tiên
    ...
    $row[] = !empty($data_vip_rating[$aRow['rating']]) ? $data_vip_rating[$aRow['rating']] : ''; // Xếp loại khashc hàng
