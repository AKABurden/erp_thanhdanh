...
$aColumns = [
    'concat('.db_prefix().'advisory_lead.prefix,'.db_prefix().'advisory_lead.code) as fullcode',
    'lead',
    'date',
    'product_other_buy',
    'address_other_buy',
...
if(!empty($this->ci->input->post('procedure')))
{
    ...
    $this->ci->db->select('group_concat(id_advisory) as list_id');
    if(!empty($datestart)) {
    ...
    $this->ci->db->group_by('id_advisory');
    $this->ci->db->where('status_procedure', $procedure);
    $list_id = $this->ci->db->get(db_prefix().'procedure_advisory_lead')->row();
    if(!empty($list_id->list_id))
    {
        $where[] = 'AND '.db_prefix().'advisory_lead.id IN ('.$list_id->list_id.')';
    }
    else
    {
        $where[] = 'AND '.db_prefix().'advisory_lead.id IN (NULL)';
    }
}
...
foreach ($rResult as $aRow) {
    ...
    $row[] = '<a class="pointer" onclick="init_lead('.$aRow['lead'].');return false;">'.$aRow['name_lead'].' '.(!empty($aRow['fullcode_lead']) ? '('.$aRow['fullcode_lead'].')' : '' ).'</a>';
    $row[] = _d($aRow['date']);
...
//    $_row = [];
    ...
    foreach($procedure_detail as $key => $value)
    {
        ...
        if($status_first == 1)
        {
            ...
            $string_Row .= '    <a class="pointer '.(!empty($color_text) ? 'update_status_lead ' : '').$color_text.'"  title="'.$title_data_finish.'"  status-procedure="'.$value['id'].'" id-data="'.$aRow['id'].'">';
