...
// --- START PREFETCHING FOR OPTIMIZATION ---
$task_ids = [];
$suggest_queries = [];
foreach ($rResult as $aRow) {
    $task_ids[] = $aRow['id'];
    if (!empty($aRow['name_table']) && !empty($aRow['suggest_id'])) {
        $suggest_queries[$aRow['name_table']][] = $aRow['suggest_id'];
    }
    if (!empty($aRow['name_table_task']) && !empty($aRow['suggest_id_task'])) {
        $suggest_queries[$aRow['name_table_task']][] = $aRow['suggest_id_task'];
    }
}

$rooms_by_task = [];
$data_checklist_items_by_task = [];
$delivery_records_by_task = [];
$checks_by_task_process = [];
$prod_reports_by_task_process_child = [];
$suggest_data = [];

if (!empty($task_ids)) {
    $this->ci->db->select('tbl_room.*, tbltask_department.task_id');
    $this->ci->db->join('tbl_room', 'tbl_room.id = tbltask_department.department_id');
    $this->ci->db->where_in('tbltask_department.task_id', $task_ids);
    $all_rooms = $this->ci->db->get('tbltask_department')->result_array();
    foreach ($all_rooms as $r) {
        $rooms_by_task[$r['task_id']][] = $r;
    }
    ...
    $this->ci->db->join(
        ...
        'p.process_id = tbltask_checklist_items.process_id 
            AND p.tasks = tbltask_checklist_items.taskid',
        'left'
    );
    $this->ci->db->where_in('tbltask_checklist_items.taskid', $task_ids);
    $this->ci->db->group_by('tbltask_checklist_items.id');
    $all_checklists = $this->ci->db->get()->result_array();
    foreach ($all_checklists as $c) {
        $data_checklist_items_by_task[$c['taskid']][] = $c;
    }

    $this->ci->db->where_in('id_create', $task_ids);
    $this->ci->db->where('type_create', 'tasks');
    $all_delivery_records = $this->ci->db->get('tbl_delivery_records')->result_array();
    foreach ($all_delivery_records as $dr) {
        $delivery_records_by_task[$dr['id_create']] = $dr;
    }

    $this->ci->db->where_in('tasks', $task_ids);
    $this->ci->db->where('isCheckNot', 1);
    $all_checks = $this->ci->db->get('tbl_tasks_inspection_criteria_process')->result_array();
    foreach ($all_checks as $c) {
        $checks_by_task_process[$c['tasks']][$c['process_id']][$c['id_tasks_process']][] = $c;
    }

    $this->ci->db->where_in('id_tasks', $task_ids);
    $all_prod_reports = $this->ci->db->get('tblproduction_report')->result_array();
    foreach ($all_prod_reports as $pr) {
        $prod_reports_by_task_process_child[$pr['id_tasks']][$pr['id_tasks_process']][$pr['id_tasks_process_child']] = $pr;
    }
}

foreach ($suggest_queries as $table => $ids) {
    if (!empty($ids)) {
        $this->ci->db->where_in('id', array_unique($ids));
        $res = $this->ci->db->get($table)->result_array();
        foreach ($res as $r) {
            $suggest_data[$table][$r['id']] = $r;
        }
    }
}
// --- END PREFETCHING ---
...
foreach ($rResult as $kr => $aRow) {
	...
	$rooms = isset($rooms_by_task[$aRow['id']]) ? $rooms_by_task[$aRow['id']] : [];
	if (!empty($rooms)) {
	...
	if (!empty($aRow['name_table'])) {
		$dtSuggest = isset($suggest_data[$aRow['name_table']][$aRow['suggest_id']]) ? $suggest_data[$aRow['name_table']][$aRow['suggest_id']] : [];
		if (!empty($dtSuggest)) {
	...
	if (!empty($aRow['name_table_task'])) {
		$dtSuggest = isset($suggest_data[$aRow['name_table_task']][$aRow['suggest_id_task']]) ? $suggest_data[$aRow['name_table_task']][$aRow['suggest_id_task']] : [];
		if (!empty($dtSuggest)) {
			if (!empty($dtSuggest['reference_no'])) {
				$code_Suggest = $dtSuggest['reference_no'];
			}
...
//	$data_checklist_items = $this->ci->db->get('tbltask_checklist_items')->result_array();
    ...
    $data_checklist_items = isset($data_checklist_items_by_task[$aRow['id']]) ? $data_checklist_items_by_task[$aRow['id']] : [];
	...
	if (!empty($data_checklist_items)) {
		...
		foreach ($data_checklist_items as $k => $v) {
			...
			if ($v['finished']) {
				...
				if (!empty($v['stages'])) {
					$delivery_records_object = isset($delivery_records_by_task[$aRow['id']]) ? $delivery_records_by_task[$aRow['id']] : [];
				...
				$check = isset($checks_by_task_process[$aRow['id']][$v['process_id']][$v['id']]) ? $checks_by_task_process[$aRow['id']][$v['process_id']][$v['id']] : [];
				if (!empty($check)) {
					foreach ($check as $kvchild => $vvchild) {
						$production_report = isset($prod_reports_by_task_process_child[$aRow['id']][$v['id']][$vvchild['inspection_criteria']]) ? $prod_reports_by_task_process_child[$aRow['id']][$v['id']][$vvchild['inspection_criteria']] : [];
...
if (!empty($task_statuses)) {
	$this->ci->db->select('tbltasks.status, count(tbltasks.id) as total');
	if (!empty($this->is_branch)) {
		if (!is_admin()) {
			if (!empty($list_branch)) {
	...
	$this->ci->db->group_by('tbltasks.status');
	$status_counts = $this->ci->db->get('tbltasks')->result_array();
...
$data['all'] = $this->ci->db->count_all_results('tbltasks');
$data['2_3'] = 0;
if (!empty($data[2])) {
	$data['2_3'] += $data[2];
}
