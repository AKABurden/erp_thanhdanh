                    ...
                    <div class="panel-body">
                        <div class="col-md-12">
                            ...
                            <div class="table-responsive">
                                <table id="table-moderation_graft_size" class="table dt-tnh table-moderation_graft_size-new" style="width: 100%;">
                                    <thead>
                                        <tr>
                                            <th class="text-center" rowspan="2"><?= lang('STT') ?></th>
                                            <th class="text-center" rowspan="2"><?= lang('Ngày Lập Phiếu') ?></th>
                                            <th class="text-center" rowspan="2"><?= lang('Mã Phiếu Yêu Cầu Ghép Size') ?></th>
...
<script>
    ...
    oTable = tnhInitDataTable('#table-moderation_graft_size',
        '<?= site_url('admin/moderation_graft_size/getModerationGraftSize') ?>', {
            'order': [
                [0, 'desc']
            ],
            scrollX:true,
            "ajax": {
                "url": '<?= site_url('admin/moderation_graft_size/getModerationGraftSize') ?>',
                "type": "POST",
                "data": function(d) {
    ...
    $('#table-moderation_graft_size').on('draw.dt', function(event) {
        init_datepicker();
    });
    ...
    function updateDate(_this, id, name) {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/moderation_graft_size/updateDate',
            data: dataPOST,
            dataType: "json",
    ...
    function exportExcel() {
        ...
        $.ajax({
            type: "POST",
            url: site.base_url + 'admin/moderation_graft_size/exportExcel',
            data: {
