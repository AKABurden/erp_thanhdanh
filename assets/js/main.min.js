...
$(document).ready(function () {
    ...
    $.post(admin_url + 'misc/checkShowModal', data).done(function (response) {
        response = JSON.parse(response);
        if (response === true) {
            $('#notification_software_modal').modal({backdrop: 'static', keyboard: false});
        } else if (response === false) {
            $('.check-close').addClass('hide');
            $('#notification_software_modal').modal({backdrop: 'static', keyboard: false});
        }
    });
});
$(document).on('click', '.check-close', function (e) {
    ...
    $.post(admin_url + 'misc/closeModal', data).done(function (response) {
        $('#notification_software_modal').modal('hide');
    });
});
$(document).ready(function () {
    var dashboard = window.location.href;
    if (dashboard.indexOf('admin/dashboard') > 0) {
        $('.wrap-container-premium').removeClass('hide');
        setTimeout(function () {
            $('.wrap-container-premium').addClass('hide');
        }, 6000);
    }
});
...
//     setTimeout(function(){ responsive_table(); }, 200);
// });
// window.onresize = function() {responsive_table()};
$('a[role="tab"], a[data-toggle="tab"]').click(function () {
    if ($(this).parents('li').attr('active')) {
...
$(document).on('click', '.js-menu', function (e) {
    if (!$('aside[id=menu]').hasClass('active')) {
        $('aside[id=menu]').addClass('active');
        $('aside[id=menu]').css({"margin-left": "0px"});
    } else {
        $('aside[id=menu]').removeClass('active');
        $('aside[id=menu]').css({"margin-left": "-210px"});
    }
});
$(document).on('click', '.action-menu', function (e) {
    ...
    if (!target.hasClass('active')) {
        ...
        target.removeClass('fa-angle-double-left');
        target.addClass('fa-angle-double-right');
        $('#wrapper').css({"margin-left": "50px"});
        $('.dashboard_user').css({"left": "-170px"});
        var second_lever = $('.nav-second-level');
        $.each(second_lever, function (i, v) {
            if ($(v).hasClass('in')) {
                $(v).parents('li').find('a:eq(0)').trigger('click');
            }
        });
    } else {
        target.removeClass('active');
        target.addClass('fa-angle-double-left');
        target.removeClass('fa-angle-double-right');
        $('#wrapper').css({"margin-left": "210px"});
        $('.dashboard_user').css({"left": "0px"});
        var second_lever = $('.H_li');
        $.each(second_lever, function (i, v) {
            if ($(v).hasClass('active') && !$(v).parents('ul').hasClass('in')) {
                $(v).parents('li').find('a:eq(0)').trigger('click');
            }
        });
    }
...
function changeRowNew_ch(table, id) {
    var data = {};
    if (typeof (csrfData) !== 'undefined') {
        data[csrfData['token_name']] = csrfData['hash'];
    }
    $.post(admin_url + 'misc/changeRowNew_ch/' + table + '/' + id, data).done(function (response) {
    });
}
    ...
    if (document.body.scrollTop > 60 || document.documentElement.scrollTop > 60) {
        if ($('#H_scroll').length > 0) {
            if (!$('.action-menu').hasClass('active')) {
                document.getElementById("H_scroll").style.position = "fixed";
                document.getElementById("H_scroll").style.top = "0";
                document.getElementById("H_scroll").style.width = "calc(100% - 210px)";
                document.getElementById("H_scroll").style.zIndex = "99";
                document.getElementsByClassName("content")[0].style.marginTop = "50px";
            } else {
                document.getElementById("H_scroll").style.position = "fixed";
                document.getElementById("H_scroll").style.top = "0";
                document.getElementById("H_scroll").style.width = "calc(100% - 50px)";
                document.getElementById("H_scroll").style.zIndex = "99";
                document.getElementsByClassName("content")[0].style.marginTop = "50px";
            }
        }
        ...
        if ($('#H_scroll').length > 0) {
            document.getElementById("H_scroll").style.position = "unset";
            document.getElementById("H_scroll").style.top = "unset";
            document.getElementById("H_scroll").style.width = "unset";
            document.getElementById("H_scroll").style.zIndex = "unset";
...
$(document).ready(function () {
    ...
    if (width_document < 1100) {
        width_document = width_document - 10;
        if ($('#H_menu_v2').length) {
            document.getElementById("H_menu_v2").style.width = width_document + 'px';
        }
    }
});

function changeWidth_menu() {
    var width_document = $(document).width();
    if (width_document < 1100) {
        width_document = width_document - 10;
        if ($('#H_menu_v2').length) {
        ...
        if ($('#H_menu_v2').length) {
            document.getElementById("H_menu_v2").style.width = '1100px';
        }
    }
}

function checkNoDrop() {
    var dropdown_menu = $('.H_drop');
    $.each(dropdown_menu, function (i, v) {
        var check_li = $($(this).find('li'));
        if (check_li.length > 0) {
            $($(this).removeClass('no-drop'));
            $($(this).find('.dropdown-toggle').removeClass('none-event'));
        }
    });
}

function changeRowNew(table, id) {
    var data = {};
    if (typeof (csrfData) !== 'undefined') {
        data[csrfData['token_name']] = csrfData['hash'];
    }
    $.post(admin_url + 'misc/changeRowNew/' + table + '/' + id, data).done(function (response) {
    });
}

function responsive_table() {
    var width_document = $(document).width();
    var loopTable = $('.table-responsive');
    $.each(loopTable, function (index, value) {
        var width_div = $(this).width();
        var tableAPI = $(this).find('.table');
        if (Number(width_document) > 780 && !tableAPI.hasClass('dont-responsive-table') && tableAPI.hasClass('dataTable')) {
            //set Height table
            var table = document.getElementsByClassName("table");
            var localtion_inPage = table[index].getBoundingClientRect();
            var height = document.body.offsetHeight;
            var heightTable = Number(height) - Number(localtion_inPage.top) - 75;
            var heightTable_scroll = Number(heightTable) - 75;
            document.getElementsByClassName("table-responsive")[index].style.height = heightTable + 'px';
            //set width header table
            var loopTd = tableAPI.find('tbody').find('tr:eq(0)').find('td');
            var loopTh = tableAPI.find('thead').find('tr:eq(0)').find('th');
            var empty = tableAPI.find('tbody').find('tr:eq(0)').find('td:eq(0)').hasClass('dataTables_empty');
            var widthTD = 0;
            var widthTH = 0;
            var totalWidthBody = 0;
            var totalWidthHead = 0;
            if (loopTd.length > 0 && loopTh.length > 0 && !empty) {
                for (var i = 0; i < loopTd.length; i++) {
                    if (i == 0) {
                        if (tableAPI.find('thead').find('tr:eq(0)').find('th:eq(' + i + ')').hasClass('sorting_disabled')) {
                            widthTD = Number(tableAPI.find('tbody').find('tr:eq(0)').find('td:eq(' + i + ')').width()); //số padding
                            widthTH_new = Number(tableAPI.find('thead').find('tr:eq(0)').find('th:eq(' + i + ')').width()); //số padding
                            if (widthTH_new > widthTD) {
                                widthTD = widthTH_new;
                                if (widthTD < 70) {
                                    widthTD = 70;
                                }
                            ...
                            totalWidthBody += Number(widthTD) + 2;
                            totalWidthHead += Number(widthTD) + 2;
                            widthTH = Number(widthTD) - 2;
                            tableAPI.find('thead').find('tr:eq(0)').find('th:eq(' + i + ')').css({"width": widthTH + "px"});
                            var child = i + 1;
                            $("head").append('<style>\
                            table:not(.dont-responsive-table) tbody tr td:nth-child(' + child + ') {width: ' + widthTD + 'px;}\
                          </style>');
                        } else {
                            widthTD = Number(tableAPI.find('tbody').find('tr:eq(0)').find('td:eq(' + i + ')').width()); //số padding
                            widthTH_new = Number(tableAPI.find('thead').find('tr:eq(0)').find('th:eq(' + i + ')').width()) + 22; //số padding
                            if (widthTH_new > widthTD) {
                                widthTD = widthTH_new;
                                if (widthTD < 70) {
                                    widthTD = 70;
                            ...
                            totalWidthBody += Number(widthTD) + 22;
                            totalWidthHead += Number(widthTD) + 22;
                            widthTH = Number(widthTD) - 22;
                            tableAPI.find('thead').find('tr:eq(0)').find('th:eq(' + i + ')').css({"width": widthTH + "px"});
                            var child = i + 1;
                            $("head").append('<style>\
                            table:not(.dont-responsive-table) tbody tr td:nth-child(' + child + ') {width: ' + widthTD + 'px;}\
                        ...
                        if (tableAPI.find('thead').find('tr:eq(0)').find('th:eq(' + i + ')').hasClass('sorting_disabled')) {
                            ...
                            totalWidthBody += Number(widthTD) + 1;
                            totalWidthHead += Number(widthTD) + 1;
                            widthTH = Number(widthTD) - 1;
                            tableAPI.find('thead').find('tr:eq(0)').find('th:eq(' + i + ')').css({"width": widthTH + "px"});
                            var child = i + 1;
                            $("head").append('<style>\
                            table:not(.dont-responsive-table) tbody tr td:nth-child(' + child + ') {width: ' + widthTD + 'px;}\
                            ...
                            widthTD = Number(tableAPI.find('tbody').find('tr:eq(0)').find('td:eq(' + i + ')').width()); //số padding
                            widthTH_new = Number(tableAPI.find('thead').find('tr:eq(0)').find('th:eq(' + i + ')').width()) + 21; //số padding
                            if (widthTH_new > widthTD) {
                                widthTD = widthTH_new;
                                if (widthTD < 70) {
                                    widthTD = 70;
                            ...
                            totalWidthBody += Number(widthTD) + 21;
                            totalWidthHead += Number(widthTD) + 21;
                            widthTH = Number(widthTD) - 21;
                            tableAPI.find('thead').find('tr:eq(0)').find('th:eq(' + i + ')').css({"width": widthTH + "px"});
                            var child = i + 1;
                            $("head").append('<style>\
                            table:not(.dont-responsive-table) tbody tr td:nth-child(' + child + ') {width: ' + widthTD + 'px;}\
                ...
                totalWidthBody = totalWidthBody + 10;
                tableAPI.find('tbody').css({
                    "overflow-y": "auto",
                    "display": "block",
                    "height": heightTable_scroll + "px"
                });
                if (Number(width_div) < Number(totalWidthBody)) {
                    tableAPI.find('tbody').css({"width": totalWidthBody + "px"});
                    tableAPI.find('thead').find('tr:eq(0)').css({"width": totalWidthHead + "px", "display": "table"});
                    $("head").append('<style>\
                              table:not(.dont-responsive-table) tbody tr {width: calc(100%);}\
                            </style>');
                } else {
                    tableAPI.find('thead').find('tr:eq(0)').css({"width": "calc(100% - 10px)", "display": "table"});
                    tableAPI.find('tbody').css({"width": "calc(100% + 10px)"});
                    $("head").append('<style>\
                              table:not(.dont-responsive-table) tbody tr {width: calc(100% - 10px);}\
                            </style>');
                }
                $("head").append('<style>\
                              .table-responsive {overflow-y: hidden;}\
                              .dataTables_empty {width: ' + totalWidthBody + 'px !important;}\
                            </style>');
            }
        } else {
            document.getElementsByClassName("table-responsive")[index].style.height = 'unset !important';
            tableAPI.find('tbody').css({
            ...
            document.getElementsByClassName("table-responsive")[index].style.overflowY = 'auto';
            var loopTd = tableAPI.find('tbody').find('tr:eq(0)').find('td');
            if (loopTd.length > 0) {
                for (var i = 0; i < loopTd.length; i++) {
                    tableAPI.find('thead').find('tr:eq(0)').find('th:eq(' + i + ')').css({"width": "unset !important"});
                }
            }
        }
    });
...
function mainWrapperHeightFix() {
    ...
    // ẩn tự động tạo min-height
    // setup_menu.css("min-height", $(document).outerHeight(!0) - 126 + "px"), content_wrapper.css("min-height",
    // $(document).outerHeight(!0) - 63 + "px"), t < e && content_wrapper.css("min-height", e + "px"), t < e && e <
    // $(window).height() && content_wrapper.css("min-height", $(window).height() - 63 + "px"), t > e && t <
    // $(window).height() && content_wrapper.css("min-height", $(window).height() - 63 + "px"), is_mobile() && "true"
    // == isRTL && side_bar.css("min-height", $(document).outerHeight(!0) - 63 + "px")
}
...
function update_todo_items() {
    ...
    }), data = {[csrfData['token_name']]: csrfData['hash']}, data.data = i, $.post(admin_url + "todo/update_todo_items_order", data)
}
...
function edit_note(e) {
    ...
    data['description'] = t;
    "" !== t && ($.post(admin_url + "misc/edit_note/" + e, data).done(function (a) {
...
function init_kanban(e, t, a, i, n, s) {
    if (0 !== $("#kan-ban").length) {
        ...
        var de = $('select[name="departments_task"]').val();
        if (de.length > 0) {
            l.departments_task = de;
        }
    
        var deRom = $('select[name="room_task"]').val();
        if (deRom.length > 0) {
            l.departments_task = deRom;
        }
        var st = $('select[name="staff_task"]').val();
        if (st.length > 0) {
            l.staff_task = st;
        }
        0 != r.length && "" !== r.val() && (l.sort_by = r.val(), l.sort = c), l.kanban = !0, e = admin_url + e, console.log(l), e = buildUrl(e, l), delay(function () {
...
function lead_add_inline_select_submit(e) {
    ...
    if ("" !== t) {
        ...
        var i = {};
        if (typeof (csrfData) !== 'undefined') {
            i[csrfData['token_name']] = csrfData['hash'];
        }
...
function validate_lead_form() {
    var e = {
            company: "required",
            name_facebook: "required",
            source: "required",
            zcode: {
                remote: {
                    url: admin_url + "misc/lead_code_exists",
                    type: 'post',
                    data: {
                        code: function () {
                            return $('#lead_form input[name="zcode"]').val();
                        },
                        id: function () {
                            return $('body').find('input[id="id_leads_ch"]').val();
                        },
                        [csrfData['token_name']]: csrfData['hash']
                    },
                },
    ...
    if (typeof (is_required_contact_lead) !== 'undefined') {
        $.each(is_required_contact_lead, function (iLead, vLead) {
            e[iLead] = vLead;
        })
    }
    // console.log(e)
            ...
            if ($.isNumeric(e.id)) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.success) {
                            $('#imgLead').attr('src', data.url.url);
                            $('#imgLead').parent().find('removeImg').attr('name_img', data.url.name_img);
                            $('.info_image').removeClass('hide');
                        }
                    }
                });
            }
            e.proposal_warning && 0 != e.proposal_warning ? ($("body").find("#lead_proposal_warning").removeClass("hide"), $("body").find("#lead-modal").animate({
                scrollTop: 0
...
function leads_kanban_update(e, t) {
    if (t === e.item.parent()[0]) {
        var a = {};
        if (typeof (csrfData) !== 'undefined') {
            a[csrfData['token_name']] = csrfData['hash'];
        }
...
function init_leads_status_sortable() {
    $("#kan-ban").sortable({
        ...
        update: function (e, t) {
            ...
            var s = {[csrfData['token_name']]: csrfData['hash']};
...
function sync_proposals_data(e, t) {
    var a = {[csrfData['token_name']]: csrfData['hash']},
...
function init_table_staff_tasks(e) {
    if (void 0 === e && $("body").hasClass("dashboard")) return !1;
    if (0 !== $("body").find(".table-staff-tasks").length) {
        var t = {},
            a = $("._hidden_inputs._filters.staff_tasks_filter input");
        $.each(a, function () {
            t[$(this).attr("name")] = '[name="' + $(this).attr("name") + '"]'
        }), initDataTable(".table-staff-tasks", admin_url + "tasks/staff_tasks", "undefined", "undefined", t, [2, "asc"])
    }
}

function init_table_staff_orders(e) {
    if (void 0 === e && $("body").hasClass("dashboard")) return !1;
    if (0 !== $("body").find(".table-staff-orders").length) {
        var t = {},
            a = $("._hidden_inputs._filters.staff_orders_filter input");
        $.each(a, function () {
            t[$(this).attr("name")] = '[name="' + $(this).attr("name") + '"]'
        }), initDataTable(".table-staff-orders", admin_url + "orders/staff_orders", "undefined", "undefined", t, [2, "asc"])
    }
}
...
function update_checklist_order() {
    ...
    if (0 !== t.length) {
        ...
        var i = {[csrfData['token_name']]: csrfData['hash']};
...
function init_tasks_checklist_items(e, t) {
    $.post(admin_url + "tasks/init_checklist_items", {
        taskid: t, [csrfData['token_name']]: csrfData['hash']
    }).done(function (t) {
...
function task_mark_as(e, t, a) {
    ...
    a += "?single_task=" + i, $("body").append('<div class="dt-loader"></div>'), requestGetJSON(a).done(function (a) {
        $("body").find(".dt-loader").remove(), !0 !== a.success && "true" != a.success || (reload_tasks_tables(), i && _task_append_html(a.taskHtml), 5 == e && "function" == typeof _maybe_remove_task_from_project_milestone && _maybe_remove_task_from_project_milestone(t), 0 === $(".tasks-kanban").length && alert_float((a.alert_type ? a.alert_type : "success"), a.message))
    })
}
...
function task_form_handler(e) {
    ...
    return $.ajax({
        ...
        // Enable lại nút submit sau khi hoàn thành
...
function timer_action(e, t, a, i) {
    ...
    if ("" !== t || !n.is(":visible")) {
        ...
        $(e).addClass("disabled");
        var o = {[csrfData['token_name']]: csrfData['hash']};
...
function tasks_kanban_update(e, t) {
    if (t === e.item.parent()[0]) {
        var a = $(e.item.parent()[0]).data("task-status-id"),
            ...
            n = {[csrfData['token_name']]: csrfData['hash']};
        n.order = [];
        var s = 0;
        $.each(i, function () {
...
function view_event(e) {
    ...
    void 0 !== e && $.post(admin_url + "utilities/view_event/" + e, data).done(function (e) {
...
var original_top_search_val, table_leads, table_activity_log, table_estimates, table_invoices, table_tasks,
...
$("body").on("loaded.bs.select change", "select.ajax-search", function (e) {
    ...
    side_bar.metisMenu(), setup_menu.metisMenu(), $(".hide-menu").click(function (e) {
        ...
        t.length < 2 && -1 === app.user_language.indexOf("chinese") && -1 === app.user_language.indexOf("japanese") || (i.html('<i class="fa fa-remove"></i>').addClass("search_remove"), delay(function () {
            t != original_top_search_val && $.post(admin_url + "misc/search", {
                q: t/*, csrf_token_name : csrfData['hash']*/
            }).done(function (e) {
    ...
    if (i && $("#search_input").val(i).trigger("keyup"), $("body").on("blur", "#timesheet_duration", function () {
        ...
        requestGet(admin_url + "tasks/checkbox_action/" + $(this).parents(".checklist").data("checklist-id") + "/" + (!0 === $(this).prop("checked") ? 1 : 0)).done(function (taskid) {
            if (taskid) {
    ...
    var l = {
        ...
        name_system_search: "[name='name_system_search']",
        fullcode_search: "[name='fullcode_search']",
        zcode_search: "[name='zcode_search']"
    };
    // if ((table_leads = $("table.table-leads")).length) {
    //     var d = table_leads.find("#th-consent"),
    //         r = [0],
    //         c = [0, table_leads.find("#th-assigned").index()];
    //     d.length > 0 && (r.push(d.index()), c.push(d.index())), _table_api = initDataTable(table_leads, admin_url +
    // "leads/table", c, r, l, [table_leads.find("th.date-created").index(), "desc"]), _table_api && d.length > 0 &&
    // _table_api.on("draw", function() { var e = table_leads.find("tbody tr"); $.each(e, function() {
    // $(this).find("td:eq(3)").addClass("bg-light-gray") }) }), $.each(l, function(e, t) { $("select" +
    // t).on("change", function() { $("[name='view_status[]']").prop("disabled", "lost" == $(this).val() || "junk" ==
    ...
    if ((table_leads = $("table.table-leads")).length) {
        var d = table_leads.find("#th-consent"),
            r = [0],
            c = [0, table_leads.find("#th-assigned").index()];
        d.length > 0 && (r.push(d.index()), c.push(d.index())), _table_api = initDataTableCustom(table_leads, admin_url + "leads/table", c, r, l, [table_leads.find("th.date-created").index(), "desc"], fixedColumns = {
            leftColumns: 5,
            rightColumns: 0
        }), _table_api && d.length > 0 && _table_api.on("draw", function () {
    ...
    if ($("body").on("change", 'input[name="contacted_today"]', function () {
        ...
        $("body").on("change", '[name="repeat_every"], [name="recurring"]', function () {
            ...
            "custom" == e ? $(".recurring_custom").removeClass("hide") : $(".recurring_custom").addClass("hide"),
                "custom_day" == e ? $(".custom_day_to_month").removeClass("hide") : $(".custom_day_to_month").addClass("hide"),
                "" !== e && 0 != e ? $("body").find("#cycles_wrapper").removeClass("hide") : (
        ...
        notifications_wrapper.find(".notifications").attr("data-total-unread") > 0 && $.post(admin_url + "misc/set_notifications_read", {[csrfData['token_name']]: csrfData['hash']}).done(function (e) {
            !0 !== (e = JSON.parse(e)).success && "true" != e.success || (document.title = doc_initial_title, $(".icon-notifications").addClass("hide"))
        })
    }), init_table_tickets(), init_table_announcements(), init_table_staff_projects(), init_table_staff_tasks(), init_table_staff_orders(), (table_activity_log = $("table.table-activity-log")).length) {
        var p = [];
...
//Cong
function C_formatNumber(nStr, decSeperate = ".", groupSeperate = ",") {
    nStr += '';
    x = nStr.split(decSeperate);
    x1 = x[0];
    x2 = x.length > 1 ? x[1] : '';
    if (x2.length > 0) {
        ...
        tam = (x2[0] == 0 ? '0' : '');
        x2 = tam + (parseFloat(x2.substring(0, 2)) + parseFloat(tosum));
        if (parseFloat(x2) > 0) {
            x2 = '.' + x2;
        } else {
            x2 = '';
...
function C_formatNumBerKeyUp(id_input) {
    key = "";
    money = $(id_input).val().replace(/[^\-\d\.]/g, '');
    a = money.split(" ");
    $.each(a, function (index, value) {
...
var groupSelectPicker = (data, el) => {
    setTimeout(function () {
        ...
        $.each(data, function (key, value) {
            ...
            } else if (value.value == 'group') {
                html += '</optgroup>';
                html += '<optgroup label="' + value.text + '">';
            } else {
                html_sub = '';
                if (typeof value.data != 'undefined') {
                    $.each(value.data, function (index, el) {
                        html_sub += 'data-' + index + '="' + el + '"';
                    });
                }
                html += '<option data-types=' + value.type_items + ' ' + html_sub + ' value="' + value.value + '">' + value.text + '</option>';
            }
        });
        html += '</optgroup>';
        $(el).html(html);
        $(el).selectpicker('refresh');
        if (count > 3) {
            $(el).parents().find('.status').addClass('hide');
        } else {
            $(el).parents().find('.status').removeClass('hide');
        }
    }, 1);
};

//end tnh
function initDataTableCustom(selector, url, notsearchable, notsortable, fnserverparams, defaultorder, fixedColumns = {
    leftColumns: 0,
    rightColumns: 0
}, ck = false, reHeight = true) {
    ...
    //hoàng crm bổ xung set height
    if (!reHeight) {
        if (Number(width_document) > 768) {
            var dataTables_scrollBody = document.getElementsByClassName("dataTables_scrollBody");
            var localtion_inPage = dataTables_scrollBody[0].getBoundingClientRect();
            var height = document.body.offsetHeight;
            var heightTable = Number(height) - Number(localtion_inPage.top) + 10;
            dataTables_scrollBody[0].style.height = heightTable + "px";
        }
    }
    //end
    return tableApi;
...
//tnh
//end tnh
//bổ xung load more activity log
var dem_more = 1;

function load_more_log(value) {
    var data = {};
    if (typeof (csrfData) !== 'undefined') {
        data[csrfData['token_name']] = csrfData['hash'];
    }
    data['value'] = value;
    data['dem_more'] = dem_more;
    $.post(admin_url + 'misc/load_more_log', data).done(function (response) {
        response = JSON.parse(response);
        var html = '';
        $.each(response.dataMain, function (i, v) {
            html += '<div class="feed-item">\
                        <div class="activity-text">\
                            ' + v.img + ' ' + v.name + '\
                        </div>\
                        <div class="activity-time">\
                            ' + v.date + ' <span class="activity-module">' + v.module + '</span>\
                        </div>\
                        <div>\
                            ' + v.content + '\
                        </div>\
        ...
        if (response.hide === true) {
            $('.more_log').addClass('hide');
        }
        dem_more++;
    });
}

$(document).on('change', '.date_space_activity', function (e) {
    var target = $(e.currentTarget);
    load_more_log_by_search(target.attr('data-module'));
});
$(document).on('change', '#staff_activity', function (e) {
    var target = $(e.currentTarget);
    load_more_log_by_search(target.attr('data-module'));
});

function load_more_log_by_search(module_activity) {
    var date = $('.date_space_activity').val();
    var staff = $('#staff_activity').val();
    if (date || staff) {
        ...
        data['module'] = module_activity;
        data['date'] = date;
        data['staff'] = staff;
        $.post(admin_url + 'misc/load_more_log_by_search', data).done(function (response) {
            response = JSON.parse(response);
            var html = '';
            if (typeof response.dataMain === 'undefined') {
                html = 'Không tìm thấy hoạt động!';
            } else {
                $.each(response.dataMain, function (i, v) {
                    html += '<div class="feed-item">\
                                <div class="activity-text">\
...
function initDataTableReport(selector, url, notsearchable, notsortable, fnserverparams, defaultorder) {
    var table = typeof(selector) == 'string' ? $("body").find('table' + selector) : selector;
    ...
    var buttonTwo = [{
        ...
        customize: function (xlsx) {
            ...
            var r3 = Addrow(1, [{ k: 'A', v: ($('head title').text()) }, { k: 'B', v: "" }, { k: 'C', v: "" }]);
            var r1 = Addrow(2, [{ k: 'A', v: (code.toUpperCase()) }, { k: 'B', v: "" }, { k: 'C', v: "" }]);
            var r2 = ''
            if($.isArray(list_name)) {
                var isStt = 0;
                var listRow = [];
                ...
                r2 = Addrow(3, [{ k: 'A', v: (list_name.toUpperCase()) }, { k: 'B', v: "" }, { k: 'C', v: "" }]);
            }
    ...
    //     var paymentReceivedReportsTable = $(tableApi).DataTable();
    //     console.log(1);
    //     var title_excel = paymentReceivedReportsTable.ajax.json().title_excel;
    //     if(title_excel) {
    //         $(tableApi).attr('title_excel', JSON.stringify(title_excel));
    //     }
    // });
...
//end tnh
function initDataTableCustomReport(selector, url, notsearchable, notsortable, fnserverparams, defaultorder, fixedColumns = {
    leftColumns: 0,
    rightColumns: 0
}, ck = false, reHeight = true) {
    var table = typeof (selector) == 'string' ? $("body").find('table' + selector) : selector;
