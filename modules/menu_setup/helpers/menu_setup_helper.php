        ...
        if (isset($options->{$item['slug']})) {
            ...
            foreach ($item['children'] as $childKey => $child) {
                if (isset($options->{$item['slug']}->children->{$child['slug']}))
                {
                    ...
                    foreach ($child['children'] as $_childKey => $_child) {
                        if (isset($options->{$item['slug']}->children->{$child['slug']}->children->{$_child['slug']})) {
                            if (isset($options->{$item['slug']}->children->{$child['slug']}->children->{$_child['slug']}->disabled)
                                && $options->{$item['slug']}->children->{$child['slug']}->children->{$_child['slug']}->disabled === 'true') {
                                // Is disabled
                                unset($items[$key]['children'][$childKey]['children'][$_childKey]);
                            } else {
                                // Has custom icon
                                if ($options->{$item['slug']}->children->{$child['slug']}->children->{$_child['slug']}->icon === false) {
                                    $items[$key]['children'][$childKey]['children'][$_childKey]['icon'] = '';
                                } elseif (!empty($options->{$item['slug']}->children->{$child['slug']}->children->{$_child['slug']}->icon)) {
                                    $items[$key]['children'][$childKey]['children'][$_childKey]['icon'] = $options->{$item['slug']}->children->{$child['slug']}->children->{$_child['slug']}->icon;
                                }
                            }
                        }
                    }
...
function _apply_menu_items_position($items, $options)
{
    ...
    if (!is_array($options)) {
        ...
        foreach ($options as $key => $item) {
            ...
            if ($newItem = $CI->app_menu->filter_item($items, $item->id)) {
                ...
                if (isset($item->children)) {
                    foreach ($item->children as $_key => $child) {
                        if ($newChildItem = $CI->app_menu->filter_item($items, $child->id)) {
                            $newItems[$key]['children'][$_key] = $newChildItem;
                            $newItemsAddedKeys[]          = $newChildItem['slug'];
                        }

                        if (isset($child->children)) {
                            foreach ($child->children as $_child) {
                                if ($new_ChildItem = $CI->app_menu->filter_item($items, $_child->id)) {
                                    $newItems[$key]['children'][$_key]['children'][] = $new_ChildItem;
                                    $newItemsAddedKeys[]          = $new_ChildItem['slug'];
                                }
                            }
                        }
                    }
        ...
        foreach ($items as $key => $item) {
            ...
            {
                foreach ($item['children'] as $childKey => $child) {
                    if (!empty($child['slug']) && !in_array($child['slug'], $newItemsAddedKeys)) {
                        $newItems[$key]['children'][$childKey] = $child;
                    }
                    if(!empty($child['children']))
                    {
                        foreach ($child['children'] as $_childKey => $_child) {
                            if (!empty($_child['slug']) && !in_array($_child['slug'], $newItemsAddedKeys)) {
                                $newItems[$key]['children'][$childKey]['children'][] = $_child;
                            }
                        }
                    }
                }
    ...
    foreach ($menu_options as $option) {
        if (isset($option->children)) {
            foreach ($option->children as $childKey => $child) {
                if (isset($child->id) && $child->id == $slug) {
                    return $child;
                }
            }
        }
