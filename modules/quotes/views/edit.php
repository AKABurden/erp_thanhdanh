...
<?php echo form_open('admin/quotes/edit/' . $quote['id'], array('id' => 'quotes')); ?>
<style type="text/css">
									...
									<table class="tnh-tb table-bordered table-hover">
										<tbody>
											<tr>
												...
												<td>
													<div class="form-group">
														<input type="text" name="reference_no" class="form-control" id="reference_no" value="<?= $quote['reference_no'] ?>" readonly="" aria-invalid="false">
													</div>
												</td>
												<td style="width: 10%;">
													<?= lang('date', 'date') ?>
												...
												<td style="width: 40%;">
													<?= form_input('date', set_value('date') ? set_value('date') : _dt($quote['date']), 'id="date" class="form-control datetimepicker" placeholder="' . lang('date') . '" required ') ?>
												</td>
											</tr>
											<tr>
												...
												<td>
													<div class="form-group">
														<input type="text" name="customers" data-placeholder="<?= lang('customers') ?>" id="customers" class="customers" required style="width: 100%;" value="<?= $quote['type_customer'] . '__' . $quote['customer_id'] ?>">
													</div>
												</td>
												<td><?= lang('tnh_person_contact', 'person_contact') ?></td>
												<td>
													<div class="form-group">
														<input type="text" name="person_contact" data-placeholder="<?= lang('tnh_person_contact') ?>" id="person_contact" class="person_contact" style="width: 100%;" value="<?= $quote['person_contact_id'] ? $quote['type_customer'] . '__' . $quote['person_contact_id'] : '' ?>">
													</div>
												</td>
											</tr>
											<tr>
												...
												<td>
													<div class="input-group">
														<input type="tel" name="address_delivery" id="address_delivery" data-placeholder="<?= lang('tnh_address_delivery') ?>" class="modal-select2" style="width: 100%;" value="<?= $quote['address_delivery_id'] ? $quote['type_customer'] . '__' . $quote['address_delivery_id'] : '' ?>">
												...
												<td>
													<textarea name="bale_parameters" placeholder="<?= lang('tnh_bale_parameters') ?>" id="bale_parameters" class="form-control" rows="3"><?= $quote['bale_parameters'] ?></textarea>
												</td>
											</tr>
											<tr>
												...
												<td>
													<select name="currencies" id="currencies" data-placeholder="<?= lang('tnh_currencies') ?>" class="currencies" style="width: 100%;" required>
														...
														<?php foreach ($currencies as $key => $value) : ?>
															<option <?= $value['id'] == $quote['currencies'] ? 'selected' : '' ?> data-amount_to_vnd="<?= $value['amount_to_vnd'] ?>" value="<?= $value['id'] ?>"><?= $value['name'] ?></option>
														<?php endforeach ?>
													</select>
												</td>
												<td><?= lang('amount_to_vnd', 'amount_to_vnd') ?></td>
												<td>
													<input type="text" name="amount_to_vnd" id="amount_to_vnd" placeholder="<?= lang('amount_to_vnd') ?>" class="form-control money-format" value="<?= $quote['amount_to_vnd'] ?>" required>
												</td>
											</tr>
											<tr>
												<td><?= lang('tnh_delivery_term', 'delivery_term') ?></td>
												<td colspan="1">
													<textarea name="delivery_term" id="delivery_term" placeholder="<?= lang('tnh_delivery_term') ?>" class="form-control delivery_term" rows="2"><?= $quote['delivery_term'] ?></textarea>
												</td>
												<td><?= lang('tnh_ship_to', 'ship_to') ?></td>
												<td colspan="1">
													<textarea name="ship_to" id="ship_to" placeholder="<?= lang('tnh_ship_to') ?>" class="form-control ship_to" rows="2"><?= $quote['ship_to'] ?></textarea>
												</td>
											</tr>
											<tr>
												<td><?= lang('tnh_payment_detail', 'payment_detail') ?></td>
												<td colspan="1">
													<textarea name="payment_detail" id="payment_detail" placeholder="<?= lang('tnh_payment_detail') ?>" class="form-control payment_detail" rows="2"><?= $quote['payment_detail'] ?></textarea>
												</td>
												<td><?= lang('tnh_payment_term', 'payment_term') ?></td>
												<td colspan="1">
													<textarea name="payment_term" id="payment_term" placeholder="<?= lang('tnh_payment_term') ?>" class="form-control payment_term" rows="2"><?= $quote['payment_term'] ?></textarea>
												</td>
											</tr>
											<tr>
												<td><?= lang('tnh_expiration_date', 'expiration_date') ?></td>
												<td colspan="1">
													<input type="text" placeholder="<?= lang('tnh_expiration_date') ?>" name="expiration_date" id="expiration_date" class="form-control expiration_date datepicker" value="<?= !empty($quote['expiration_date']) ? _d($quote['expiration_date']) : '' ?>">
												</td>
													...
													<select name="id_branch" id="id_branch" data-placeholder="<?= lang('h_branch') ?>" style="width: 100%;" class="">
														...
														<?php if (!empty($branch)) { ?>
															<?php foreach ($branch as $key => $value) : ?>
																<option <?= $quote['id_branch'] == $value['id'] ? 'selected' : '' ?> value="<?= $value['id'] ?>"><?= $value['name'] ?></option>
															<?php endforeach ?>
														<?php } ?>
													</select>
												</td>
											...
											<tr>
												...
												<td>
													<div class="form-group">
														<input type="text" name="quotation_request_id" data-placeholder="<?= lang('Yêu cầu báo giá') ?>" id="quotation_request_id" class="quotation_request_id" style="width: 100%;" value="<?= !empty($quote['quotation_request_id']) ? $quote['quotation_request_id'] : '' ?>">
													</div>
												</td>
												<td><?= lang('Báo giá lại', 'is_quote_again') ?></td>
												<td>
													<div class="checkbox checkbox-danger">
														<input type="checkbox" name="is_quote_again" id="is_quote_again" <?= $quote['is_quote_again'] == 1 ? 'checked' : '' ?> value="1">
									...
									<table class="tnh-tb table-bordered table-hover">
										<tbody>
											<tr>
												...
												<td colspan="3">
													<textarea name="note" id="note" class="form-control" placeholder="<?= lang('note') ?>" rows="2"><?= $quote['note'] ?></textarea>
												</td>
											</tr>
										</tbody>
									</table>
				...
				<table class="table tnh-tb table-bordered table-hover">
					<tbody>
						<tr>
							...
							<td style="width: 35%;">
								<select name="tax_id" id="tax_id" class="tax_id" data-placeholder="<?= lang('tax') ?>" style="width: 100%;">
									...
									<?php foreach ($taxs as $key => $value) : ?>
										<option <?= ($value['id'] == $quote['tax_id']) ? 'selected' : '' ?> data-rate="<?= $value['taxrate'] ?>" value="<?= $value['id'] ?>"><?= $value['name'] ?></option>
									<?php endforeach ?>
								</select>
							</td>
							<td></td>
							...
							<td class="hide" style="width: 35%;" class="td-grand-total-all text-right"><?= formatMoney($quote['grand_total']) ?></td>
						</tr>
					</tbody>
				</table>
			</div>
