                                            ...
                                            <tbody>
                                                    <?php $j_v2 = 0; if(!empty($attribution)){ ?>
                                                        <?php foreach ($attribution as $key => $value) { ?>
                                                            ...
                                                            <td ><input type="hidden" class="count" value="<?=$j_v2?>" /><div class="form-group" style="width: 100%"><select style="width: 100%" class="units_useds" id="units_useds_<?=$j_v2?>" name="attribution[<?=$j_v2?>][units_useds]" style="width: 200px;" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">\
                                                            <?php foreach($departments as $v) { ?>
                                                                <option <?=($v['departmentid']==$value['units_useds'])?'selected':''?> value="<?=$v['departmentid']?>"><?=$v['name']?></option>\
                                                            <?php }?>
                                                            </select></div></td>
                                                            <td style="text-align: center;"><input type="number" id="attribution_percentage" class="attribution_percentage H_input"  value="<?=$value['percent']?>"  name="attribution[<?=$j_v2?>][attribution_percentage]"></td>
                                                            <td class="delete"><a href="#" class="btn btn-danger pull-right" onclick="deleteTrItem_v2(this); return false;"><i class="fa fa-times"></i></a></td>
                                                            </tr>                                          
                                                        <?php $j_v2++; } ?>
                                                    <?php }?>
                                            </tbody>
                            ...
                            <table class="tnh-tb table-bordered table-hover dont-responsive-table m-group0" style="table-layout: fixed;">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php $value = (isset($items) ? $items->number_used_time : ''); ?>
                                            ...
                                            <tbody>
                                                    <?php $j = 0; if(!empty($items)){ ?>
                                                        <?php foreach ($items->items as $key => $value) { ?>
                                                            <tr class="sortable item">
                                                            <td class="dragger avatar" style="text-align: center;"><input type="hidden"  value="<?=$value['id_other_payslips']?>" /></td>
                            ...
                            <table class="tnh-tb table-bordered table-hover dont-responsive-table m-group0" style="table-layout: fixed;">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td >
                                            <label for="date" class="control-label">
                                                <?php echo _l('ch_warranty_period'); ?>
                                            </label>
                                        </td>
                                        <td colspan="3">
                                            <?php $value = (isset($items) ? $items->warranty_period : ''); ?>
                                            <?php echo render_input('warranty_period','',$value); ?>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td >
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        residual_value: "required",
            ...
            function add_quotes_client_s(form) {
                ...
                var total = 0;
                $.each(items, (index,value)=>{
                    total+=parseFloat($(value).find('.attribution_percentage').val().replace(/\,/g, ''));
                });
                if(total > 100)
                {
                    alert('<?=_l('Tổng phần trăm phân bổ không được lớn hơn 100')?>');return;
                }
                if(total < 100)
                {
                    alert('<?=_l('Tổng phần trăm phân bổ không được bé hơn 100')?>');return;
                }
    ...
    function countrow_v2()
    {
        if(!$('table.item-attribution tbody').find('input[value=hau]').length)
        {
            createTrItemfist_v3();
        }
    }
    ...
    var createTrItem = (item,currentQuantityInput) => {
        ...
        if( ($('table.item-record_increased tbody tr').find('input[value=' + item.id + ']#product_id').length > 0)) {
            alert_float('danger', "<?=_l('ch_document_has_been_added')?>");
    ...
    var createTrItem_v2 = (id,currentQuantityInput) => {
        if(typeof(id)=='undefined' || id.length==0) return;
        if( ($('table.item-attribution tbody tr').find('input[value=' + id + ']#attribution_id').length > 0)) {
            alert_float('danger', "<?=_l('Đơn vị này đã được thêm')?>");
            currentQuantityInput.select2('val','');
             var new_tr = currentQuantityInput.parents('tr');
             var count = new_tr.find('td > input.count').val();
             new_tr.find('td.avatar').html((Number(count)+1)+'<input type="hidden"  value="hau" />');
             return;
        }
        var new_tr = currentQuantityInput.parents('tr');
        var count = new_tr.find('td > input.count').val();
        new_tr.find('td.avatar').html((Number(count)+1)+'<input type="hidden" class="id" id="attribution_id" name="attribution[' + count + '][id]" value="'+id+'" />');
        new_tr.find('td.delete').html('<a href="#" class="btn btn-danger pull-right" onclick="deleteTrItem_v2(this); return false;"><i class="fa fa-times"></i></a>');
        // getTotalPrice();
        countrow_v2();
    }
    var uniqueArray = <?=$j?>;
    ...
    var uniqueArray_v2 = <?=$j_v2?>;
    var createTrItemfist_v2 = (id) => {

        var items = $('table.item-attribution tbody').find('tr.item');
        if($('table.item-attribution tbody').find('.dataTables_empty').length) {
            $('table.item-attribution tbody').find('.dataTables_empty').parents('tr').remove();
        }
        var newTr = $('<tr class="sortable item"></tr>');
        var td1 = $('<td class="dragger avatar" style="text-align: center;"><input type="hidden" id="attribution_id"  name="attribution[' + uniqueArray_v2 + '][id]" value="'+id+'" />'+(items.length+1)+'</td>');
        var td2 = $('<td><input type="hidden" class="count" value="'+uniqueArray_v2+'" /><div class="form-group" style="width: 100%"><select style="width: 100%" class="units_useds" id="units_useds_'+uniqueArray_v2+'" name="attribution[' + uniqueArray_v2 + '][units_useds]" style="width: 200px;" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">\
            <?php foreach($departments as $value) { ?>
                <option  value="<?=$value['departmentid']?>"><?=$value['name']?></option>\
            <?php }?>
            </select></div></td>');
        var td3 = $('<td style="text-align: center;"><input type="number" id="attribution_percentage" class="attribution_percentage H_input"  value="100"  name="attribution[' + uniqueArray_v2 + '][attribution_percentage]"></td>');

        newTr.append(td1);
        newTr.append(td2);
        newTr.append(td3);
        newTr.append('<td class="delete"></td');
        $('table.item-attribution tbody').append(newTr);
        $('#units_useds_'+uniqueArray_v2).select2();
        $('#units_useds_'+uniqueArray_v2).select2('val',id);
        // newTr.find('.selectpicker').selectpicker('refresh');
        uniqueArray_v2++;
        createTrItemfist_v3();
    }
    var createTrItemfist_v3 = () => {
        var items = $('table.item-attribution tbody').find('tr.item');
        var newTr = $('<tr class="sortable item"></tr>');
        var td1 = $('<td class="dragger avatar" style="text-align: center;"><input type="hidden"  value="hau" />'+(items.length+1)+'</td>');
        var td2 = $('<td><input type="hidden" class="count" value="'+uniqueArray_v2+'" /><div class="form-group" style="width: 100%"><select style="width: 100%" class="units_useds" id="units_useds_'+uniqueArray_v2+'" name="attribution[' + uniqueArray_v2 + '][units_useds]" style="width: 200px;" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">\
            <option></option>\
            <?php foreach($departments as $value) { ?>
                <option value="<?=$value['departmentid']?>"><?=$value['name']?></option>\
        ...
        var td3 = $('<td style="text-align: center;"><input type="number" id="attribution_percentage" class="attribution_percentage H_input"  value="0"  name="attribution[' + uniqueArray_v2 + '][attribution_percentage]"></td>');
