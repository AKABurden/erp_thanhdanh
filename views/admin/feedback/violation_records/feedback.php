<?php $folder = 'feedback/violation_records';?>
<?php $class = 'violation_records';?>
<?php $urlForm = 'feedback/add_feed_back_violation_records';?>
<?php $urlRemove = 'feedback/remove_feed_back_violation_records';?>
...
<button type="button" class="btn btn-info pull-right mbot20" autocomplete="off"  onclick="add_feedback(<?=!empty($violation_records) ? $violation_records->id : ''?>);">
    Thêm bình luận
</button>
<?php echo form_close(); ?>
<div class="clearfix"></div>
...
<div class="feedback-<?=$class?> data-feed-back-violation_records-<?=$violation_records->id?>">
    ...
    //init_editor('#<?//=$idComment?>//');
        ...
        function add_feedback(id) {
            ...
            $.ajax({
                ...
                if (data.success) {
                    if(data.html) {
                        if($('div[data-feed-back-violation_records="' + data.id + '"]').length == 0) {
        ...
        function removeFeedBack(id = "") {
            if(confirm('Xóa feedback sẽ không thể khôi phục bạn có chắc muốn xóa?')) {
                $.get(admin_url + '<?=$urlRemove?>/' + id, function(data) {
                    ...
                    if(data.success) {
                        $('div[data-feed-back-violation_records="' + id + '"]').remove();
                    }
                    alert_float(data.alert_type, data.message);
                })
            }
