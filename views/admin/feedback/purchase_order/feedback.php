<?php $folder = 'feedback/purchase_order';?>
<?php $class = 'purchase_order';?>
<?php $urlForm = 'feedback/add_feed_back_purchase_order';?>
<?php $urlRemove = 'feedback/remove_feed_back_purchase_order';?>
...
<button type="button" class="btn btn-info pull-right mbot20" autocomplete="off"  onclick="add_feedback(<?=!empty($items) ? $items->id : ''?>);">
    Thêm bình luận
</button>
<?php echo form_close(); ?>
<div class="clearfix"></div>
...
<div class="clearfix"></div>
<div class="feedback-<?=$class?> data-feed-back-purchase-order-<?=$items->id?>">
    ...
    function removeFeedBack(id = "") {
        if(confirm('Xóa feedback sẽ không thể khôi phục bạn có chắc muốn xóa?')) {
            $.get(admin_url + '<?=$urlRemove?>/' + id, function(data) {
                ...
                if(data.success) {
                    $('div[data-feed-back-purchase-order="' + id + '"]').remove();
                }
                alert_float(data.alert_type, data.message);
            })
        }
