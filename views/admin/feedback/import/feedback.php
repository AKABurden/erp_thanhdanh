...
<?php $folder = 'feedback/import';?>
<?php $class = 'import';?>
<?php $urlForm = 'feedback/add_feed_back_import';?>
<?php $urlRemove = 'feedback/remove_feed_back_import';?>
...
<div class="clearfix"></div>
<div class="feedback-<?=$class?> data-feed-back-import-<?=$items->id?>">
    ...
    function removeFeedBack(id = "") {
        if(confirm('Xóa feedback sẽ không thể khôi phục bạn có chắc muốn xóa?')) {
            $.get(admin_url + '<?=$urlRemove?>/' + id, function(data) {
                ...
                if(data.success) {
                    $('div[data-feed-back-import="' + id + '"]').remove();
                }
                alert_float(data.alert_type, data.message);
            })
        }
