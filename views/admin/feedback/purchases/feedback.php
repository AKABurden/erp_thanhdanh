<?php $folder = 'feedback/purchases';?>
<?php $class = 'purchases';?>
<?php $urlForm = 'feedback/add_feed_back_purchases';?>
<?php $urlRemove = 'feedback/remove_feed_back_purchases';?>
...
<button type="button" class="btn btn-info pull-right mbot20" autocomplete="off"  onclick="add_feedback(<?=!empty($purchase) ? $purchase->id : ''?>);">
    Thêm bình luận
</button>
<?php echo form_close(); ?>
<div class="clearfix"></div>
...
<div class="feedback-<?=$class?> data-feed-back-purchases-<?=$purchase->id?>">
        ...
        function removeFeedBack(id = "") {
            if(confirm('Xóa feedback sẽ không thể khôi phục bạn có chắc muốn xóa?')) {
                $.get(admin_url + '<?=$urlRemove?>/' + id, function(data) {
                    ...
                    if(data.success) {
                        $('div[data-feed-back-purchases="' + id + '"]').remove();
                    }
                    alert_float(data.alert_type, data.message);
                })
            }
