        ...
        <div class="panel-body _buttons">
            ...
            <div class="dropdown pull-right">
                ...
                <ul class="dropdown-menu pull-right" role="menu" aria-labelledby="dropdownMenu1" style="width: 200px;">
                    <li class="hide">
                        <?php if (has_permission('payroll_salary', '', 'export')) { ?>
                            ...
                            <table id="table-salary-new" class="table dt-tnh table-hover table-salary-new">
                                ...
                                <tr>
                                    ...
                                    <th rowspan="2" class="text-center" style="min-width: 100px;">
                                        <?= lang('Ngày vào làm') ?></th>
                                    <th rowspan="2" class="text-center" style="min-width: 100px;">
                                        <?= lang('Tổng lương') ?></th>
                                    <th rowspan="2" class="text-center" style="min-width: 100px;">
                                        <?= lang('Lương vị trí (LCB)') ?></th>
                                    ...
                                    <th colspan="<?= $colspanAllowance ?>" class="text-center"
                                        ...
                                        <?= lang('Phụ cấp') ?></th>
                                    <th rowspan="2" class="text-center" style="min-width: 80px;">
                                        <?= lang('Tổng phụ cấp') ?></th>
                                    <th colspan="3" class="text-center" style="min-width: 80px;">
                                        <?= lang('Số tiếng tăng ca') ?></th>
                                    <th rowspan="2" class="text-center" style="min-width: 80px;">
                                        <?= lang('Tổng tiền tăng ca') ?></th>
                                    <th rowspan="2" class="text-center" style="min-width: 80px;">
                                        <?= lang('Chênh lệch tiền tăng ca') ?></th>
                                    <th colspan="<?= $colspanReduce ?>" class="text-center" style="min-width: 80px;">
                                        <?= lang('Khấu trừ') ?></th>
                                    <th rowspan="2" class="text-center" style="min-width: 80px;">
                                        <?= lang('Khấu trừ khác(tạm ứng)') ?></th>
                                    <th rowspan="2" class="text-center" style="min-width: 80px;">
                                        <?= lang('Tổng khẩu trừ') ?></th>
                                    <th rowspan="2" class="text-center" style="min-width: 80px;">
                                        <?= lang('Thuế TNCN') ?></th>
                                    <th rowspan="2" class="text-center" style="min-width: 80px;">
                                        <?= lang('Tổng thực lãnh') ?></th>
                                </tr>
                                <tr>
                                    <th class="text-center" style="min-width: 50px;"><?= lang('Chức vụ') ?></th>
                                    <th class="text-center" style="min-width: 50px;"><?= lang('T.Nhiệm') ?></th>
                                    <th class="text-center" style="min-width: 50px;"><?= lang('Phép năm') ?></th>
                                    <th class="text-center" style="min-width: 50px;"><?= lang('Lễ tết') ?></th>
                                    <th class="text-center"
                                        style="min-width: 50px;"><?= lang('VR hưởng lương ( hiếu hỉ)') ?></th>
                                    <?php if (!empty($dtAllowance)) { ?>
                                        <?php foreach ($dtAllowance as $key => $value) { ?>
                                            <th class="text-center" style="min-width: 50px;"><?= $value['name'] ?></th>
                                        <?php } ?>
                                    <?php } ?>
                                    <th class="text-center"
                                        style="min-width: 70px;"><?= lang('Ngày cơm hành chính') ?></th>
                                    <th class="text-center"
                                        style="min-width: 70px;"><?= lang('Ngày cơm tăng ca') ?></th>
                                    <th class="text-center" style="min-width: 70px;"><?= lang('Tiền cơm') ?></th>
                                    <th class="text-center"
                                        style="min-width: 50px;"><?= lang('Ngày thường (1.5)') ?></th>
                                    <th class="text-center" style="min-width: 50px;"><?= lang('Chủ nhật (2.0)') ?></th>
                                    <th class="text-center"
                                        style="min-width: 50px;"><?= lang('Ngày lễ tết (3.0)') ?></th>
                                    <?php if (!empty($dtReduce)) { ?>
                                        <?php foreach ($dtReduce as $key => $value) { ?>
                                            <th class="text-center" style="min-width: 50px;"><?= $value['name'] ?></th>
                                        <?php } ?>
                                    <?php } ?>
                                    <th class="text-center" style="min-width: 70px;"><?= lang('8% BHXH') ?></th>
                                    <th class="text-center" style="min-width: 70px;"><?= lang('1,5% BHYT') ?></th>
                                    <th class="text-center" style="min-width: 70px;"><?= lang('1% BHTN') ?></th>
                                    <th class="text-center" style="min-width: 70px;"><?= lang('1% Đoàn phí') ?></th>
                                </tr>
                                </thead>
                                <tbody>
                                <tr>
        ...
        '<?= site_url('admin/payroll/getPayroll') ?>', {
            ...
            "ajax": {
                ...
                "dataSrc": function (json) {
                    ...
                    $('.table-salary-new tfoot tr td:nth-child(6)').html(`<div class="text-right">${tnhFormatMoney(json.footer_salary)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(7)').html(`<div class="text-right">${tnhFormatMoney(json.footer_salary_bhxh)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(8)').html(`<div class="text-right">${tnhFormatMoney(json.footer_salary_position)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(9)').html(`<div class="text-right">${tnhFormatMoney(json.footer_salary_responsibility)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(10)').html(`<div class="text-right">${tnhFormatMoney(json.footer_sales)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(11)').html(`<div class="text-right">${tnhFormatMoney(json.footer_gasonline_cars)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(12)').html(`<div class="text-right">${tnhFormatMoney(json.footer_phone)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(13)').html(`<div class="text-right">${tnhFormatMoney(json.footer_motel)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(14)').html(`<div class="text-right">${tnhFormatMoney(json.footer_concurrently)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(15)').html(`<div class="text-right">${tnhFormatMoney(json.footer_business_fee_staff)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(16)').html(`<div class="text-center">${tnhFormatNumber(json.footer_day_number)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(17)').html(`<div class="text-center">${(json.footer_day_number_new)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(18)').html(`<div class="text-center">${(json.footer_day_number_holiday)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(19)').html(`<div class="text-center">${(json.footer_day_number_lt)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(20)').html(`<div class="text-center">${(json.footer_day_number_ch)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(21)').html(`<div class="text-center">${(json.footer_day_number_off)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(22)').html(`<div class="text-center">${(json.footer_day_number_off_new)}</div>`);
                    $('.table-salary-new tfoot tr td:nth-child(23)').html(`<div class="text-right">${tnhFormatMoney(json.footer_salary_income)}</div>`);
                    keyNew = 23;
                    <?php if (!empty($dtAllowance)){ ?>
                    <?php foreach ($dtAllowance as $key => $value){ ?>
                    keyNew++;
                    ...
                    // $(`.table-salary-new tfoot tr td:nth-child(${keyNew})`).html(`<div class="text-right">${tnhFormatMoney(footer_total_allowance_new)}</div>`);
                    <?php } ?>
                    <?php } ?>
                    keyNew++;
                    $(`.table-salary-new tfoot tr td:nth-child(${keyNew})`).html(`<div class="text-center">${tnhFormatMoney(json.footer_allowance_rice)}</div>`);
                    keyNew++;
                    $(`.table-salary-new tfoot tr td:nth-child(${keyNew})`).html(`<div class="text-center">${tnhFormatMoney(json.footer_allowance_rice_tc)}</div>`);
                    keyNew++;
                    ...
                    $(`.table-salary-new tfoot tr td:nth-child(${keyNew})`).html(`<div class="text-center"></div>`);
                    keyNew++;
                    $(`.table-salary-new tfoot tr td:nth-child(${keyNew})`).html(`<div class="text-right">${tnhFormatMoney(json.footer_allowance_business_fee)}</div>`);
                    keyNew++;
                    ...
                    keyNew++;
                    idcheck = <?= $value['id'] ?>;
                    // $(`.table-salary-new tfoot tr td:nth-child(${keyNew})`).html(`<div class="text-right">${tnhFormatMoney(json.arrFooter.footer_total_reduce_3)}</div>`);
                    <?php } ?>
                    <?php } ?>
                    keyNew++;
