...
<div id="setup-menu-wrapper" class="animated <?php if($this->session->has_userdata('setup-menu-open')
    ...
    <ul class="nav metis-menu" id="setup-menu">
       ...
       <li class="menu-item-<?php echo !empty($item->slug) ? $item->slug : ''; ?>">
         <a href="<?php echo (!empty($item->children) ? '#' : admin_url($item->url)) ?>" aria-expanded="false">
         ...
         <?php if(!empty($item->children)){ ?>
             <ul class="nav nav-second-level collapse" aria-expanded="false">
                <?php foreach($item->children as $submenu){
					if (!is_admin()) {
						if ($item->name == 'tnh_categories' && $submenu->url != 'categories/machines') {
							continue;
						}
					} ?>
                   <li class="sub-menu-item-<?php echo $submenu->slug; ?>"><a href="<?php echo admin_url($submenu->url); ?>">
