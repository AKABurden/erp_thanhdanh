...
$aColumns = [
	...
	'( ' . $server . ' + 
		COALESCE(SUM(tblimport.total),0) + 
		COALESCE(
			(
...
foreach ($rResult as $key => $aRow) {
	...
	if ($aRow['type'] == 0) {
		...
		$row[7] = formatNumber($subtotal - $amount_paid - $amount_paid_invoice + $services);
	} else {
		$whereJoin = array();
		$whereJoin['where'] = array(
		...
		$whereJoin1['field'] = 'tbl_orders.price_other_expenses_delivery';
		$amount_paid = sum_from_table_join('tbl_orders', $whereJoin1);
		$row[7] = formatNumber($subtotal - $amount_paid);
	}


	if ($aRow['type'] == 0) {
		$row[8] = formatNumber($aRow['total_import'] - $aRow['amount_paid_import']);
		$footer_data['debt_total'] += $aRow['total_import'] - $aRow['amount_paid_import'];
	} else {
		$row[8] = formatNumber($aRow['total_import'] - $aRow['price_other_expenses_import']);
