...
$join = [
    ...
    'LEFT JOIN tblclients as customers ON customers.userid = tbltickets.customer_id',
    'LEFT JOIN tbl_orders ON tbl_orders.id = tbltickets.order_id',
];
...
if (count($_statuses) > 0) {
    array_push($filter, 'AND tbltickets.status IN (' . implode(', ', $_statuses) . ')');
}
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == '1') {
            ...
            // }
        } elseif ($aColumns[$i] == 'tbltickets.status') {
            $all_status = get_table_where('tbltickets_status');
            $_data = '<span class="pointer inline-block label task-menu-status" style="border:1px solid ' . $aRow['statuscolor'] . '; color:' . $aRow['statuscolor'] . '">
                        <span class="inline-block label" style="border:1px solid ' . $aRow['statuscolor'] . '; color:' . $aRow['statuscolor'] . '">'
                            .ticket_status_translate($aRow['tbltickets.status']).
                        '</span>';
            $_data .= '<a class="trigger task-menu-options manual-popover">
                        <i class="fa fa-cog font-medium-xs" aria-hidden="true"></i></a>
                        <div class="content-menu hide">
                            <ul>';
            foreach($all_status as $keyL => $text)
            {   
                if(is_numeric($text['ticketstatusid']) && $text['ticketstatusid'] != $aRow['tbltickets.status'])
                {
                    $_data .= '<li><a onclick="mark_as_status('.$text['ticketstatusid'].','.$aRow['tbltickets.ticketid'].')">'.$text['name'].'</a></li>';
                }
            }
            $_data .= '</ul></div></span>';
            ...
            $_data = _dt($_data);
        } elseif ($aColumns[$i] == 'tbltickets.priority') {
            $_data = '<span class="inline-block label label-info">'.ticket_priority_translate($aRow['tbltickets.priority']).'<span>';
        } else {
