...
var OrgChartNode = function (parent, boss)
{
	...
	this.getCell(1, 1).setText("Chức vụ:");
         ...
         success: function(data)
         {
            ...
            setY += 40;
           	if(data[0].CEO) {
				...
				ceoNode.setTitle(data[0].title);
				ceoNode.setFullName(data[0].CEO);
				...
				diagram.addItem(ceoNode);
              	...
              	ceoNode.resize();
              	$.ajax({
			        url: "<?=admin_url().'flow_chart/getData_sub'?>",
			        success: function(data2)
			        {
			            data2 = JSON.parse(data2);
			            $.each(data2.departments, (i,v) => {
							...
							$.each(v.role, (i_child_two, v_child_two) => {
			                  	var ctoNode2 = new OrgChartNode(diagram, ctoNode);
			                    ctoNode2.setBounds(new Rect(setX, setY, 60, 25));
								ctoNode2.setBoss(ctoNode);
								ctoNode2.setTitle();
								ctoNode2.setFullName(v_child_two.name);
								ctoNode2.resize();
								diagram.addItem(ctoNode2);
			                  	setX += 70;
			                  	var link2 = diagram.getFactory().createDiagramLink(ctoNode, ctoNode2);
								styleLink(link2);
								diagram.resizeToFitItems();
								diagram.arrangeAnimated(tree);
								ctoNode2.resize();
