				...
				<div class="panel_s">
				 	...
				 	<?php 
										...
										<tbody>
												...
												<td style="width: 35%">
													<div class="form-group">
										                <div class="input-group">
										                    ...
										                    <?php echo (isset($items) ? ($items->prefix) : get_option('prefix_return_suppliers'));?></span>
															<?php 
																	$number = sprintf('%06d', ch_getMaxID('id', 'tblreturn_suppliers') + 1);
											...
											<tr>
												...
												<td>
													...
													?>
													<input type="text" class="hide" name="suppliers_idd" id="suppliers_idd" value="<?=$value?>">
												</td>
												<td>
											...
											<tr>
												...
												<td rowspan="2">
													...
													<?php echo render_textarea('reason', '', $value,array('rows'=>3)); ?>
												</td>
											</tr>
											<tr>
												<td>
													<label for="type_items" class="control-label">
														<?php echo _l('ch_treatment_methods'); ?>
													</label>
												</td>
												<td >
													<?php $value = (isset($items) ? $items->treatment_methods : 1); ?>
													<?php
														$array = array(
															array(
																'id'=>1,
															...
															array(
																'id'=>2,
																'name'=>_l('ch_debt_deduction'),
															)
														);
											         	echo render_select('treatment_methods', $array, array('id', 'name'),'',$value,array(),array(),'','',false);
													?>
												</td>
											</tr>
										</tbody>
						...
						<?php
		                    ...
		                    if(total_rows(db_prefix().'customfields',array('fieldto'=>'return_suppliers','active'=>1)) > 0 ){
								...
								<div class="panel-body">
				                   	...
				                   	<div role="tabpanel" class="tab-pane" id="custom_fields">
				                    	<?php $value_id = (isset($items) ? $items->id : '');?>
				                      	<?php echo render_custom_fields('return_suppliers',$value_id); ?>
				                   	</div>
										...
										<table style="table-layout: fixed;" class="dt-tnh table item-import table-bordered table-hover">
											<thead>
												<tr>
													...
													<th style="width: 100px;" class="text-center"><?php echo _l('Số lượng thực nhập'); ?></th>
													<th style="width: 100px;" class="text-center"><?php echo _l('Tồn kho'); ?></th>
	                                                <th style="width: 100px;" class="text-center"><?php echo _l('Số lượng trả'); ?></th>
											...
											<?php
	                                            ...
	                                            if(isset($items) && count($items->items) > 0) {
	                                                ...
	                                                <tr class="sortable item">
													    ...
													    <td>
													    	<div class="form-group">
													             <select style="width: 150px;" data-id="<?=$value['localtion_warehouses_id']?>" class="localtion_warehouses_id" id="localtion_warehouses_id_<?=$i?>" name="items[<?=$i?>][localtion_warehouses_id]" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
													             </select>
													        </div>
													    </td>
													    <td>
													    ...
													    <td class="text-center">
													    	<div class="mainQuantity_supp_text"><?=formatNumber($value['quantity_warehoue'])?></div><input class="mainQuantity_supp height_auto hide" type="text" name="items[<?=$i?>][mainQuantity_supp]" value="<?=formatNumber($value['quantity_warehoue'])?>" />
													    </td>
													    <td class="text-center">
													    	<div class="mainQuantity_text"><?=formatNumber($value['quantity'])?></div><input class="mainQuantity height_auto hide" type="text" name="items[<?=$i?>][quantity]" value="<?=number_format($value['quantity'])?>" />
													    </td>
													    <td>
													    	<input onkeyup="formatNumBerKeyUpCus(this)"  class="mainQuantityNet H_input height_auto"  type="text" name="items[<?=$i?>][quantity_net]" value="<?=formatNumber($value['quantity_net'])?>" />
													    </td>
													    <td >
...
<script>
    ...
    function appendtype()
    {
        ...
        $.each(items, (index,value)=>{
            ...
            $(value).find('td:nth-child(1)').find('div#type_name').html(name_type);
          	...
          	ajaxSelectCallBack($('#custom_item_select_'+index), "<?=admin_url('return_suppliers/SearchItems')?>", ID,type);
    ...
    $(document).on('change', '.custom_item_select', (e)=>{
		...
		if(id == '') {
			...
			$.post(admin_url + 'return_suppliers/get_items/'+id+'/'+type+'/'+suppliers_id,{[csrfData['token_name']] : csrfData['hash']}, function(item){
    ...
    var createTrItem = (item,currentQuantityInput,type) => {
        ...
        console.log(item.quantity_warehoue);
	...
	var createTrItemfist = () =>{
        ...
        var td3 = $('<td><div class="form-group ">\
								             <select class="localtion_warehouses_id" id="localtion_warehouses_id_'+uniqueArray+'" name="items[' + uniqueArray + '][localtion_warehouses_id]" style="width: 150px;" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">\
								             </select>\
								        </div></td>');
        var td4 = $('<td class="unit_name"></td>');
        var td12 = $('<td class="text-center"><div class="mainQuantity_supp_text"></div><input class="mainQuantity_supp height_auto hide" type="number" name="items[' + uniqueArray + '][mainQuantity_supp]" value="1" /></td>');
        var td5 = $('<td class="text-center"><div class="mainQuantity_text"></div><input class="mainQuantity height_auto hide" type="number" name="items[' + uniqueArray + '][quantity]" value="1" /></td>');
        var td6 = $('<td><input onkeyup="formatNumBerKeyUpCus(this)" class="mainQuantityNet H_input height_auto"  type="text" name="items[' + uniqueArray + '][quantity_net]" value="1" /></td>');
        var td7 = $('<td ><input onkeyup="formatNumBerKeyUpCus(this)" class="height_auto H_input align_right price" type="text" name="items[' + uniqueArray + '][price]" value="0" /></td>');
        ...
        ajaxSelectCallBack($('#custom_item_select_'+uniqueArray), "<?=admin_url('return_suppliers/SearchItems')?>", 0);
        uniqueArray++;
        getTotalPrice();
        reset_item_select();
	}
	$(document).on('change', '.mainQuantityNet', (e)=>{
    	...
    	if(parseInt(currentQuantityInput.val()) > mainQuantity){
            ...
            mainQuantity_s=currentQuantityInput.parents('tr').find('.mainQuantity_supp').val();
		    	if(parseInt(currentQuantityInput.val()) > mainQuantity_s){
		            currentQuantityInput.attr("style", "width: 100px;border: 1px solid red !important");
		            currentQuantityInput.attr('data-toggle', 'tooltip');
		            currentQuantityInput.attr('data-trigger', 'manual');
		            currentQuantityInput.attr('title', '<?=_l('ch_limit_items')?>');
    ...
    {   
        ...
        if(!$('table.item-import tbody tr.item').find('input[value=hau]').length)
        {
        	if(items.length > 0)
	        {
	        	$('#suppliers_id').attr('disabled', true);
        		$("#id_order").select2('readonly',true);
	        }else
	        {
	        	$('#suppliers_id').attr('disabled', false);
        		$("#id_order").select2('readonly',false);
	        }
        }else
        {
        	if(items.length > 1)
    ...
    $('#items-form').on('submit', (e)=>{
   	...
   	function loadLocaltion_warehouses(warehouse_id,id){
       	...
       	var type = localtion_warehouses.parent().parent().parent().find('td:nth-child(1)').find('input#type').val();
       	...
       	if(localtion_warehouses.length) {
			// getLocationLotByItem
			// list_localtion_return
       		$.post(admin_url+"warehouse/getLocationLotByItem",{type:type,items:items,warehouse:warehouse_id,checked:checked,[csrfData['token_name']] : csrfData['hash']},function(data){
	...
	$(document).on('change', '.localtion_warehouses_id', (e)=>{
		...
		if(id == '') {
				...
				currentQuantityInput.parent().parent().parent().find('td:nth-child(6)').find('div.mainQuantity_text').html(formatNumber(0));
				currentQuantityInput.parent().parent().parent().find('td:nth-child(7)').find('input').change();
		}
		else {
			// $.post(admin_url + 'return_suppliers/get_quantity/'+items+'/'+warehouse_id+'/'+id+'/'+type+'/'+id_return,{[csrfData['token_name']] : csrfData['hash']}, function(item){
			$.post(admin_url + 'return_suppliers/getQuantity/'+items+'/'+type+'/'+id,{[csrfData['token_name']] : csrfData['hash']}, function(item){
				var item = JSON.parse(item);
				currentQuantityInput.parent().parent().parent().find('td:nth-child(6)').find('input').val(item);
				currentQuantityInput.parent().parent().parent().find('td:nth-child(6)').find('div.mainQuantity_text').html(tnhFormatNumber(item));
				currentQuantityInput.parent().parent().parent().find('td:nth-child(7)').find('input').change();
		    });
		}
	});
