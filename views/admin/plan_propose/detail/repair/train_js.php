<script>
    ...
    $(document).ready(function() {
        if (edit_train == 0) {} else if (edit_train == 1) {
            for (i = 0; i < counter; i++) {
                ...
                getCost(i, cost);
                getSubstituteQuota(i, substitutequota);
            }
    ...
    $('.add-row').on('click', function(event) {
        ...
        tdSubstituteQuota = '<div class="text-center mbot10">\
                <select name="train[' + counter + '][substitutequota]" style="width: 100%;" data-placeholder="Định Mức Thay Thế"  id="substitutequota_' + counter + '" class="substitutequota modal-select2">\
                </select>\
                </td>';
    ...
    function getSubstituteQuota(counter, substitutequota) {
        optionWh = '';
        optionWh = '<option value=""></option>';
        optionWh += `<option value="1">Vượt</option>`;
        optionWh += `<option value="2">Kém</option>`;
        optionWh += `<option value="3">Đạt</option>`;
        $('#substitutequota_' + counter + '').html(optionWh);
        $('#substitutequota_' + counter + '').select2({
            formatResult: repoFormatHtml,
            formatSelection: repoFormatHtml,
            dropdownCssClass: "bigdrop",
            escapeMarkup: function(m) {
        ...
        $('#substitutequota_' + counter + '').select2("val", substitutequota);
        $('#substitutequota_' + counter + '').change();
    }
</script>