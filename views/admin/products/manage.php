...
<!--                        <li>-->
<!--                            <a href="--><?//= base_url('admin/products/export_excel_products') ?><!--" class="tnh-modal" data-tnh="modal" data-toggle="modal" data-target="#myModal">-->
...
<!--                        </li>-->
			...
			<?php if ($this->perExportProducts): ?>
                <a href="<?= base_url('admin/products/export_excel_products') ?>" class="tnh-modal btn btn-info pull-right mright5 H_action_button " data-tnh="modal" data-toggle="modal" data-target="#myModal">
                    <i class="fa fa-file-excel-o" aria-hidden="true"></i>
                    <?php echo lang('tnh_export_excel'); ?>
                </a>

                <a onclick="export_product_stage()" href="#" class="btn btn-info pull-right mright5 H_action_button">
                    <i class="fa fa-file-excel-o" aria-hidden="true"></i>
                    ...
                    <div class="panel-body">
                        ...
                        <div class="">
                            <table id="table-products" class="table table-hover table-products" style="min-width: 100%;">
                                <thead>
                                    <tr>
                                        <th class="text-center"><div class="checkbox mass_select_all_wrap text-center"><input type="checkbox" id="mass_select_all" data-to-table="products"><label for="mass_select_all"></label></div></th>
...
<script type="text/javascript">
	...
	var fnserverparams = {status_table: '#status_table', category_search: '#category_search', products_search: '#products_search',code_bom_search : '#code_bom_search'};
	var oTable = '';
    var iDt = 0;
</script>
<script type="text/javascript">
		...
		oTable = tnhDatatable(
            '#table-products',
            {
                ...
                "columnDefs": [
                    ...
                    {
                    	"targets": 1, "name": 'images', 'width': '50px',
                    	"render": function (data, type, row) {
                    ...
                    {
                        "targets": 3, "name": 'type_products', 'width': '130px',
                        "render": function(data, type, row) {
                            str = '';
                            if (data == "products") {
                    ...
                    },
                    {
                        ...
                        "targets": 5, "name": 'name', 'width': '150px'
                    },
                    {"targets": 6, "name": 'unit_name', 'width': '80px', 'className': 'text-center'},
                    {
                        "render": function(data, type, row) {
                        ...
                        "targets": 7, "name": 'status', 'width': '80px'
                    },
                    {
                        "render": function(data, type, row) {
                            detailQty = row[14];
                        ...
                        "targets": 8, "name": 'quantity_inventory', 'width': '120px'
                    },
                    {
                        "targets": 9, "name": 'bm', 'width': '100px',
                        "render": function(data, type, row) {
                            sl = '';
                            if (data) {
                                data = data.split(':::');
                    ...
                    {
                        "targets": 10, "name": 'st', 'width': '100px',
                        "render": function(data, type, row) {
                            sl = '';
                            if (data) {
                                data = data.split(':::');
                    ...
                    },
                    {
                        ...
                        "targets": 11, "name": 'note', 'width': '200px'
                    },
                    {"targets": 12, "name": 'versions', 'visible': false},
                    {"targets": 13, "name": 'versions_stage', 'visible': false},
                    {"targets": 14, "name": 'qty_ws_inventory', 'width': '100px', 'visible': false, 'orderable': false, 'searchable': false},
                    <?= $script ?>
                    {"targets": <?= $targetsId ?>, "name": 'id_sort', 'visible': false},
                    {"targets": <?= $targets ?>, "name": 'actions', 'orderable': false, 'searchable': false, 'width': '160px', 'visible': false}
                ]
            }
        );
  ...
  //       ]);
        ...
        $(document).on('change', '#category_search, #products_search', function(event) {
            event.preventDefault();
            oTable.draw();
        });
