<?php echo form_open('admin/manufactures/add_production_planning_orders/' . $id, array('id' => 'created-detail')); ?>
<div class="modal-dialog modal-lg" style="width: 90%;">
    <div class="modal-content">
        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
            <h4 class="modal-title">
                <?= lang('tnh_add_production_planning') ?>
            </h4>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <?= $order['status'] == 'un_approved' ? '<span class="label label-danger">' . lang($order['status']) . '</span>' : '<span class="label label-success">' . lang($order['status']) . '</span>' ?>
                    </div>
                </div>
                <div class="row col-md-12">
                    <div class="col-md-6">
                        <div class="form-group">
                            <?= lang('tnh_location', 'location') ?>
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        <?= lang('Chọn nhanh hạn chót', 'flash_date') ?>
                        <input type="text" name="flash_date" id="flash_date" placeholder="Chọn nhanh hạn chót" class="form-control flash_date datepicker" value="">
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="form-group">
                        <?= lang('Chọn nhanh phòng ban', 'flash_derpartment') ?>
                        <select name="flash_derpartment" data-placeholder="<?= lang('Chọn nhanh phòng ban') ?>" id="flash_derpartment" autocomplete="off" multiple class="flash_derpartment modal-select2" style="width: 100%;">
                            <option value=""></option>
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        <?= lang('Chọn nhanh nhân viên', 'flash_staff') ?>
                        <select name="flash_staff" data-placeholder="<?= lang('Chọn nhanh nhân viên') ?>" id="flash_staff" autocomplete="off" multiple class="flash_staff modal-select2" style="width: 100%;">
                            <option value=""></option>
                            <?php foreach ($staffs as $k => $val) : ?>
                                <option value="<?= $val['staffid'] ?>"><?= $val['firstname'] ?> <?= $val['lastname'] ?></option>
                            <?php endforeach ?>
                        </select>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('select.flash_derpartment').select2();
        $('select.flash_staff').select2();
        ...
        $('#flash_date').change(function(event) {
            flash_date = $(this).val();
            if (flash_date) {
                $('.deadline').val(flash_date);
            }
        });

        $('#flash_derpartment').change(function(event) {
            flash_derpartment = $(this).val();
            if (flash_derpartment) {
                $.ajax({
                        url: site.base_url + 'admin/manufactures/getEmployeesByDeparment',
                        type: 'GET',
                        dataType: 'JSON',
                        data: {
                            departmentId: flash_derpartment,
                            csrf_token_name: hash
                        },
                    })
                    .done(function(data) {
                        if (data) {
                            $('select.departments').select2('val', flash_derpartment);
                            $('select.employess').html(loadEmployees(data.employees));
                        }
                    })
                    .fail(function() {
                        console.log("error");
        ...
        $('#flash_staff').change(function(event) {
            flash_staff = $(this).val();
            $('select.employess').select2('val', flash_staff);
        });
