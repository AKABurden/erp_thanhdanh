        ...
        <div class="div-box-menu">
            ...
            <?php if (!empty($items)) : ?>
                <?php foreach ($items as $i => $v) : ?>
                    ...
                    ?>
                    <?php if (empty($is_sub)) : ?>
                        <div style="<?= $is_not_click ? 'pointer-events: none;' : '' ?>" class="uppercase div-label p-relative <?= $activeCategory ?>" data-value="<?= $i ?>">
                            <?= $v['name'] ?>
                            <?php if ($is_not_click) : ?>
                    ...
                    <?php else : ?>
                        <div class="uppercase p-relative">
                            ...
                            <div class="div-sub-label div-label" data-value="<?= $i ?>">
                                <?= $v['name'] ?>
                                <div class="div-sub-name"><?= $sub_name ? '('.$sub_name.')' : '' ?></div>
                            </div>
                        </div>
        ...
        <?php if (!empty($items)) : ?>
            <?php foreach ($items as $i => $v) : ?>
                ...
                <div class="tab-sub-menu cl-<?= $i ?>" id="<?= $i ?>" style="<?= $style ?>">
                                ...
                                <div class="div-box-sub-menu">
                                    ...
                                    <?php if (!empty($sub_menu_one)) : ?>
                                        <?php foreach ($sub_menu_one as $kS => $vS) : ?>
                                            <div class="uppercase div-title"><?= $vS['name'] ?></div>
                                            <?php
                                            ...
                                            <?php if ($sub) : ?>
                                                <?php foreach ($sub as $kSub => $vSub) : ?>
                                                    ...
                                                    ?>
                                                    <div class="sub-div flex-center <?= empty($is_permission) ? 'not-permission' : '' ?>">
                                                        <?php
                    ...
                    <?php } else { ?>
                        <div class="row">
                            <div class="uppercase h3 text-center">Đánh Giá KPI Tháng/Năm <?= date('Y') ?></div>
                            <div class="col-md-3">
                                <?php $thisMonth = date('m') ?>
                                <select name="filter_month" id="filter_month" onchange="view_kpi_evaluation()" data-width="100%" data-none-selected-text="Không có mục nào được chọn" data-live-search="true" tabindex="-98" data-placeholder="<?= lang('month') ?>" class="selectpicker">
                                    <?php for ($i = 1; $i <= 12; $i++) : ?>
                                        <option value="<?php echo $i; ?>" <?= ($i == $thisMonth) ? ' selected' : '' ?>>Tháng <?php echo $i; ?></option>
                                    <?php endfor; ?>
                                </select>
                            </div>
                            
                            <div class="col-md-12" id="view_kpi_evaluation">
                                <div class="col-md-6">
                                    <div class="div-box-sub-menu">
                                        <div class="mtop20"></div>
                                        <?php $troubleViolationList = getTroubleViolationList(['this_month'=>true]);?>
                                        ...
                                        <table class="table table-hover dataTable dont-responsive-table">
                                            ...
                                            <tbody>
                                                <?php
                                                $total_count_trouble_violation = 0; $total_point = 0;
                                ...
                                <div class="col-md-6">
                                    <div class="mtop20"></div>
                                    <?php $productionReport = getProductionReport(['this_month'=>true]);?>
                                    ...
                                    <table class="table table-hover dataTable dont-responsive-table">
                                        <tbody>
                                            <?php foreach($productionReport as $th => $thData) { ?>
                                                ...
                                                ?>
                                                    <tr><td><div class="col-md-10"><div class="col-md-10"><?= $img .' '. $trData['object_name'] .'</div><div class="col-md-2 text-right bold" style="color:red; font-size: 20px">'.formatMoney($point_kpi).'</div></div> '. $strTypeKpi ?></td></tr>
                                                <?php } ?>
                                            <?php } ?>
                                        </tbody>
                                    </table>
...
<script>
    ...
    //         $.ajax({
    //             type: 'POST',
    //             url: admin_url+'/misc/view_kpi_evaluation',
    //             data: {
