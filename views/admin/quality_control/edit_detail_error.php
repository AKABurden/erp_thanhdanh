<?php echo form_open('admin/quality_control/edit_detail_error/'.$id,array('id'=>'edit-detail-error')); ?>
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
			<h4 class="modal-title"><?= _l('tnh_edit_detail_error'); ?></h4>
		</div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="category_id" id="category_id" data-placeholder="<?= lang('category_errors') ?>" style="width: 100%;" required="required">
                            ...
                            <?php foreach ($categories as $key => $value): ?>
                                <option <?= $value['id'] == $category['category_error_id'] ? 'selected' : '' ?> value="<?= $value['id'] ?>"><?= $value['name'] ?></option>
                            <?php endforeach ?>
                        </select>
                    </div>
                </div>
...
<script>
    $(function(){
        $('#category_id').select2();
       	appValidateForm($('#edit-detail-error'), {
