<?php echo form_open('admin/quality_control/edit_category_error/'.$id,array('id'=>'edit-category-error')); ?>
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
			<h4 class="modal-title"><?= _l('tnh_edit_category_error'); ?></h4>
		</div>
                ...
                <div class="col-md-12">
                    <div class="form-group">
                        ...
                        <select name="parent_id" id="parent_id" data-placeholder="<?= lang('tnh_group_parent') ?>" class="modal-select2" style="width: 100%;">
                            <option value=""></option>
                            <?= recursiveCategoryErrors($category['id']) ?>
                        </select>
                    </div>
                </div>
				<div class="col-md-12">
...
<script>
    $(function(){
        ...
        $('#parent_id').val(<?= $category['parent_id'] ?>).trigger('change');
       	appValidateForm($('#edit-category-error'), {
           code: 'required',
           name: 'required'
        }, addCategory);
