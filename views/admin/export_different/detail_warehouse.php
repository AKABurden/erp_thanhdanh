                                ...
                                <div class="panel-body">
                                    <table class="tnh-tb table-bordered table-hover dont-responsive-table m-group0">
                                        <tbody>
                                            ...
                                            <tr>
                                                <td><?= lang('Chi nhánh xưởng', 'id_branch') ?></td>
                                                <td>
                                                    ...
                                                    $branch_id = (isset($items) ? $items->id_branch : $staff['id_branch']);
                                                    ?>
                                                    <select name="id_branch" id="id_branch" class="id_branch  " required="required" data-placeholder="<?= lang('Chi nhánh xưởng') ?>" style="width: 100%;">
                                                        <option value=""></option>
...
<?php init_tail(); ?>
<script>
    var items_warehouse = <?= !empty($items_warehouse) ? json_encode($items_warehouse) : '{}' ?>;
    ...
    function load_report() {
        $.each(items_warehouse, (index, value) => {
            createTrItemfist_warehouse(value);
        });
    }
    var createTrItemfist_warehouse = (items) => {
        ...
        var td12 = $('<td><div class="form-group " style="width: 100%">\
								             <select class="warehouses_id" data-id="' + items.id + '"  id="warehouses_id_' + uniqueArray + '" name="items[' + uniqueArray + '][warehouses_id]" style="width: 100%;" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">\
								             </select>\
								        </div></td>');
        ...
        var td5 = $(
            ...
            uniqueArray + '][quantity]" value="' + items.product_quantity + '" /></td>');
        var td6 = $('<td><input class="mainQuantityNet H_input height_auto"  type="text" name="items[' + uniqueArray +
            '][quantity_net]" value="' + items.product_quantity + '" /></td>');
        var td7 = $(
        ...
        ajaxSelectCallBack($('#custom_item_select_' + uniqueArray), "<?= admin_url('purchases/SearchItems') ?>", items.id_items, items.type_items);
        $('#custom_item_select_' + uniqueArray).change();
        uniqueArray++;
        getTotalPrice();
    }
    ...
    $(document).on('change', '.warehouses_id', (e) => {
        ...
        if (id == '') {
            ...
            $.post(admin_url + 'export_different/get_quantity/' + items + '/' + id + '/' +
                ...
                function(item) {
                    var item = JSON.parse(item);
                    currentQuantityInput.parent().parent().parent().find('td:nth-child(6)').find('input').val(
                        item);
                    currentQuantityInput.parent().parent().parent().find('td:nth-child(6)').find(
                        'div.mainQuantity_text').html(tnhFormatNumber(item));
                    currentQuantityInput.parent().parent().parent().find('td:nth-child(7)').find('input')
                        .change();
                });
        }
    });
    ...
    function loadLocaltion_warehouses_id(id, value = '') {
        ...
        var checked = localtion_warehouses.attr('data-id');
        console.log(localtion_warehouses)
