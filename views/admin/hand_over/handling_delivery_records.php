                    ...
                    <table class="tb-view table table-hover dataTable">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style="width: 35%;">
                                    <input type="text" name="date" id="date_delivery_records" class="form-control datetimepicker" value="<?= !empty($delivery_records) ? _d($delivery_records['date'])  : date('d/m/Y H:i:s') ?>" required="required">
                                </td>
                            </tr>
                            <tr>
                                <td><?= lang('Hạng mục bàn giao', 'type_object') ?></td>
                                <td>
                                    <select name="type_object" data-placeholder="<?= lang('Liên quan đến') ?>" data-live-search="true" id="type_object" class="selectpicker form-control " data-none-selected-text="Không có mục nào được chọn" tabindex="-98">
                                        <option value=""></option>
										<?php if (!empty($type_object_internal_proposal)) : ?>
											<?php foreach ($type_object_internal_proposal as $key => $value) : ?>
                                                ...
                                                ?>
                                                <option <?= !empty($delivery_records) && $delivery_records['type_object'] == $value['key_object'] ? 'selected' : '' ?> data-type="<?=$value['type']?>" value="<?= $value['key_object'] ?>"><?= $value['name'] ?></option>
											<?php endforeach; ?>
										<?php endif; ?>
                                    </select>
                                </td>
                                <td class="type_id_object <?=empty($type) ? 'hide' : ''?>"><label for="id_object"><?=!empty($delivery_records['name_type_object']) ? $delivery_records['name_type_object'] : ''?></label></td>
                                <td class="type_id_object <?=empty($type) ? 'hide' : ''?>" id="rel_id_select">
                                    <select name="id_object[]" data-live-search="true" id="id_object" class="selectpicker form-control" data-actions-box="true" multiple data-none-selected-text="Không có mục nào được chọn" tabindex="-98">
										<?php if (!empty($id_object)) : ?>
											<?php foreach ($id_object as $key => $value) : ?>
                                                <option <?= !empty($delivery_records) && !empty($delivery_records['id_object_' . $value['id']]) ? 'selected' : '' ?> value="<?= $value['id'] ?>" data-subtext="<?=$value['subtext']?>"><?= $value['name'] ?></option>
											<?php endforeach; ?>
										<?php endif; ?>
                                    </select>
                                </td>
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="staff" data-placeholder="<?= lang('staff') ?>" id="staff_delivery_records" class="modal-select2 <?= $disabled ?>" style="width: 100%;" required="required">
                                        <option value=""></option>
                                        <?php if (!empty($staffs)) : ?>
                                            <?php foreach ($staffs as $key => $value) : ?>
                                                <option <?= !empty($delivery_records) && $delivery_records['staff'] == $value['staffid'] ? 'selected' : (!empty($id_import) && ($staffc == $value['staffid']) ? 'selected' : '') ?> data-department="<?= $value['name_department'] ?>" data-role="<?= $value['name_role'] ?>" value="<?= $value['staffid'] ?>"><?= $value['fullname'] ?></option>
                                            <?php endforeach; ?>
                                        <?php endif; ?>
                                    </select>
                                </td>
                                ...
                                <td> 
                                    <select name="receiver" data-placeholder="<?= lang('Người nhận bàn giao') ?>" id="receiver" class="modal-select2 <?= $disabled ?>" style="width: 100%;">
                                        <option value=""></option>
										<?php if (!empty($staffs)) : ?>
											<?php foreach ($staffs as $key => $value) : ?>
                                                <option <?= !empty($delivery_records['receiver']) && $delivery_records['receiver'] == $value['staffid'] ? 'selected' : '' ?> value="<?= $value['staffid'] ?>"><?= $value['fullname'] ?></option>
											<?php endforeach; ?>
										<?php endif; ?>
                                    </select>
                                </td>
                            ...
                            <tr>
                                ...
                                <td colspan="3">
                                    ...
                                    ?>
                                    <select name="category_hand" id="category_hand" data-live-search="true" <?=!empty($kt_active_delivery_records_task) ? 'disabled' : ''?> data-none-selected-text="<?= lang('Loại bàn giao') ?>" class="form-control selectpicker" required="required">
                                        <option></option>
                                        <?php if (!empty($category_hand)) : ?>
                                            <?php foreach ($category_hand as $key => $value) : ?>
                                                <option <?= (!empty($delivery_records) && $delivery_records['category_hand'] == $value['id'] ? 'selected' : '') ?> value="<?= $value['id'] ?>"><?= $value['name'] ?></option>
                                            <?php endforeach; ?>
                                        <?php endif; ?>
                                    </select>
                                </td>
                                    ...
                                    <table id="tb-task" class="table tb-view dataTable">
                                        <thead>
                                            <tr style="background: #cedae6;">
                                                ...
                                                <td style="width: 25%;" class="text-center"><?= lang('Nội dung bàn giao') ?></td>
                                                <td style="width: 15%;" class="text-center"><?= lang('Tiêu chuẩn') ?></td>
                                                <td style="width: 25%;" class="text-center"><?= lang('Phương thức') ?></td>
                                                <td style="width: 10%;" class="text-center checkTab"><?= lang('Đạt') ?></td>
                                                <td style="width: 10%;" class="text-center checkTab"><?= lang('Không đạt') ?></td>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <?php
