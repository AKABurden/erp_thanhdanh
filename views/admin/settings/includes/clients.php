...
?>
<div class="form-group">
    <label for="visible_customer_profile_tabs" class="control-label">
        <?php echo _l('visible_tabs'); ?>(<?php echo _l('client_add_edit_profile'); ?>)
    </label>
    <select name="settings[visible_customer_profile_tabs][]" id="visible_customer_profile_tabs" multiple class="form-control selectpicker" data-none-selected-text="<?php echo _l('all'); ?>" data-actions-box="true">
        <?php foreach ($tabs as $tabKey => $tab) {
            ...
            <option value="<?php echo $tabKey; ?>"<?php if ((is_array($current_tabs) && array_key_exists($tabKey, $current_tabs) && $current_tabs[$tabKey] == true) || (is_array($current_tabs) && !array_key_exists($tabKey, $current_tabs))) {
                echo ' selected';
            } ?>>
                <?php echo $tab['name']; ?>
            </option>
...
<hr/>
<!--Công-->
<?php echo render_input('settings[leadtime_client]', 'cong_leadtime_advisory_client', get_option('leadtime_client'))?>
<!--Công-->
<hr/>
