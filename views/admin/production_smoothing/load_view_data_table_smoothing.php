<table id="table-production-smoothing" class="table dt-tnh table-production-smoothing-new"
    ...
    <tr>
        ...
        <th class="text-center"><?= lang('Chi tiết lệnh') ?></th>
        <?php if(!empty($result)){ ?>
...
<script>
    ...
    function loadTableNew() {
        oTableVs1 = tnhInitDataTable('#table-production-smoothing',
            '<?= site_url('admin/production_smoothing/getProductionSmoothingVs1') ?>', {
                'order': [
                    [0, 'desc'],
                ],
                // scrollY: height_body,
                ...
                "ajax": {
                    "url": '<?= site_url('admin/production_smoothing/getProductionSmoothingVs1') ?>',
                    "type": "POST",
                    "data": function (d) {
                    ...
                    "dataSrc": function (json) {
                        $.each(json.arrCheck,function (k,v){
                            if (v != -1){
                                oTableVs1.columns(v).visible(false, false);
                            }
                        })
                        return json.aaData;
                    }
